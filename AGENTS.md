...
6. Open every PR as Draft and use ordinary pushes for impact-selected feedback while batching the work. After each successful `PR Feedback` run, the trusted default-branch Draft Codex Review workflow automatically requests one exact-head Codex review and writes a rolling status comment with the settled head state; a maintainer can also post a `pageroot-draft-review-command:v1` comment to request or close a round manually. Batch accepted P0/P1 fixes in Draft, and only freeze the final head on current `main` and mark it Ready once the latest Draft round is clean. Ready is the sole final-review trigger. While an exact-head probe marker is open `review-policy` refuses every completion, so a Draft probe can never satisfy the final gate; the probe closes its own marker once the round settles while Draft, and a PR must never be marked Ready while a probe round is still open. `review-policy` binds Codex evidence to the exact live head/base, requires a post-Ready exact-commit Codex review, immutable clean exact-commit comment or post-Ready Codex Bot `+1`, waits 30 seconds after its final response, blocks only active P0/P1 findings or P0/P1 `CHANGES_REQUESTED` reviews, and records P2/P3/unclassified findings as debt instead of blocking regardless of reviewer. `branch-policy` and `baseline-policy` run independently of review; source and macOS lanes start after the dependency baseline so review and tests overlap. A later commit or base update invalidates promotion: return to Draft, batch any P0/P1 fix, then Ready the new final pair once. PR size is advisory only, never a mechanical merge limit. Squash-merge only with explicit authorization.
...
The release and artifact gates require a clean committed tree. Any source change invalidates earlier release evidence. Every ordinary Pull Request update runs impact-selected feedback plus a non-blocking review-advisory snapshot; returning to Draft alone runs nothing. Only `ready_for_review` starts the final candidate workflow. `review-policy` requires a post-Ready Codex review, immutable clean exact-commit comment or Codex Bot `+1` on the frozen current pair, revalidates head/base, waits 30 seconds, blocks only P0/P1 findings and P0/P1 changes requests, and writes P2/P3/unclassified findings to review debt regardless of reporter. Dependency advisory and packaged-runtime closure remain hard deterministic gates in `baseline-policy`; source build, Browser and macOS lanes wait only for that baseline, not for review. `candidate-context` calls credential-free `Release Dry Run` only when packaging risk is present; a skipped dry run is valid for source-only candidates. `release-gate` joins every required result, immediately revalidates review policy, refreshes the deterministic baseline and attests the exact tree. A new commit cancels the stale candidate and leaves the new SHA without `release-gate` until it is batched and promoted again. `main` verifies exact Tree/version/PR attestation and does not rerun source tests. The manual `Release Candidate` workflow may use the internal `gate:artifact-only:auto` lane only when CI has authenticated a fresh successful PR gate for the identical Git tree and version. The separate `Release` workflow then verifies and publishes those exact candidate bytes and creates the immutable tag; it never rebuilds during publication. Release only from reviewed `main` using `docs/RELEASING.md`.
The release and artifact gates require a clean committed tree. Any source change invalidates earlier release evidence. Every ordinary Pull Request update runs impact-selected feedback; after its success, the trusted Draft Codex Review Auto workflow requests one exact-head Codex review, and returning to Draft alone runs nothing. Only `ready_for_review` starts the final candidate workflow. `review-policy` requires a post-Ready Codex review, immutable clean exact-commit comment or Codex Bot `+1` on the frozen current pair, revalidates head/base, waits 30 seconds, blocks only P0/P1 findings and P0/P1 changes requests, and writes P2/P3/unclassified findings to review debt regardless of reporter. Dependency advisory and packaged-runtime closure remain hard deterministic gates in `baseline-policy`; source build, Browser and macOS lanes wait only for that baseline, not for review. `candidate-context` calls credential-free `Release Dry Run` only when packaging risk is present; a skipped dry run is valid for source-only candidates. `release-gate` joins every required result, immediately revalidates review policy, refreshes the deterministic baseline and attests the exact tree. A new commit cancels the stale candidate and leaves the new SHA without `release-gate` until it is batched and promoted again. `main` verifies exact Tree/version/PR attestation and does not rerun source tests. The manual `Release Candidate` workflow may use the internal `gate:artifact-only:auto` lane only when CI has authenticated a fresh successful PR gate for the identical Git tree and version. The separate `Release` workflow then verifies and publishes those exact candidate bytes and creates the immutable tag; it never rebuilds during publication. Release only from reviewed `main` using `docs/RELEASING.md`.
