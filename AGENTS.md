...
6. Open every PR as a draft. Ordinary pushes receive impact-selected feedback only. After freezing the final head on current `main`, request Codex review in Draft with the exact 40-character head/base marker documented in `docs/CODEX_WORKFLOW.md`; fix P0-P2 findings and repeat for every new head or base. Only after that Draft review is complete, and only when no other PR is being promoted, mark it Ready once. Ready is the single final-review trigger; do not post another review command while promotion is running. `review-settled` waits for Codex's post-Ready completion on the current commit and its settle window, then `baseline-policy` must pass before any complete source or macOS lane starts. Any later commit or base update invalidates the promotion: return the PR to Draft, update a changed base into the branch so the head changes, finish and review the current exact pair, then mark it Ready again. Squash-merge only with explicit authorization.
...
The release and artifact gates require a clean committed tree. Any source change invalidates earlier release evidence. Every ordinary Pull Request update runs impact-selected feedback, whether the PR is draft or ready; returning to Draft alone runs nothing. Only an explicit `ready_for_review` transition can promote a Draft-reviewed exact head/base pair and trigger its final Codex review. The complete lanes remain skipped until `review-settled` observes a non-blocking substantive Codex review bound to the exact commit, an immutable exact-commit clean completion comment or a phase-correct clean reaction, ignores empty reviews and `EYES`, treats current-commit `CHANGES_REQUESTED` as blocking even without an inline thread, waits 180 seconds, rejects active P0-P2 threads and revalidates the current head and base; dependency advisories and the packaged-runtime closure then pass in `baseline-policy` before Linux build or macOS Electron work begins. A new commit cancels an in-flight stale gate and leaves the new head without `release-gate` until it is reviewed and promoted again; a base update also invalidates the pair. `main` verifies that exact Tree/version/PR attestation and does not rerun source tests. The manual `Release Candidate` workflow may use the internal `gate:artifact-only:auto` lane only when CI has authenticated a fresh successful PR gate for the identical Git tree and version. The separate `Release` workflow then verifies and publishes those exact candidate bytes and creates the immutable tag; it never rebuilds during publication. Release only from reviewed `main` using `docs/RELEASING.md`.
