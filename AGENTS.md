...
6. Open every PR as Draft and use ordinary pushes for impact-selected feedback while batching the work. `PR Feedback` also mirrors the final review policy as a non-blocking advisory, so Draft-phase Codex findings surface before promotion. When the final head is frozen on current `main`, mark it Ready once; Ready is the sole final-review trigger. `review-policy` binds Codex evidence to the exact live head/base, accepts an exact-commit review or clean comment written after the frozen base commit (Draft included), requires post-Ready root `+1` reactions, waits 30 seconds after its final response, blocks only active P0/P1 findings or P0/P1 `CHANGES_REQUESTED` reviews, and records P2/P3/unclassified findings as debt instead of blocking regardless of reviewer. `branch-policy` and `baseline-policy` run independently of review; source and macOS lanes start after the dependency baseline so review and tests overlap. A later commit or base update invalidates promotion: return to Draft, batch any P0/P1 fix, then Ready the new final pair once. PR size is advisory only, never a mechanical merge limit. Squash-merge only with explicit authorization.
