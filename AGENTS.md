...
6. Open every PR as a draft. Ordinary pushes receive impact-selected feedback only. After freezing the final head on current `main`, request Codex review in Draft with the exact 40-character SHA marker documented in `docs/CODEX_WORKFLOW.md`; fix P0-P2 findings and repeat for every new head. Only after that Draft review is complete, and only when no other PR is being promoted, mark it Ready once. `review-settled` waits for the Ready-triggered final Codex result and its settle window, then `baseline-policy` must pass before any complete source or macOS lane starts. Any later commit or base update invalidates the promotion: return the PR to Draft, finish and review the new exact SHA, then mark it Ready again. Squash-merge only with explicit authorization.
