...
6. Open every PR as Draft and use ordinary pushes for impact-selected feedback while batching the work. `PR Feedback` also runs a non-blocking review-advisory snapshot of the final review policy, so current Draft-phase P0/P1 findings surface before promotion. When the final head is frozen on current `main`, mark it Ready once; Ready is the sole final-review trigger. A trusted `github-actions[bot]` Draft-phase probe request is allowed, but while its exact-head marker is open `review-policy` refuses every completion, so a probe can never satisfy the final gate; the probe closes its own marker once the round settles while Draft, and a PR must never be marked Ready while a probe round is still open. `review-policy` binds Codex evidence to the exact live head/base, requires a post-Ready exact-commit Codex review, immutable clean exact-commit comment or post-Ready Codex Bot `+1`, waits 30 seconds after its final response, blocks only active P0/P1 findings or P0/P1 `CHANGES_REQUESTED` reviews, and records P2/P3/unclassified findings as debt instead of blocking regardless of reviewer. `branch-policy` and `baseline-policy` run independently of review; source and macOS lanes start after the dependency baseline so review and tests overlap. A later commit or base update invalidates promotion: return to Draft, batch any P0/P1 fix, then Ready the new final pair once. PR size is advisory only, never a mechanical merge limit. Squash-merge only with explicit authorization.
