...
6. Open every PR as a draft. Ordinary pushes receive impact-selected feedback only. After freezing the final head on current `main`, request Codex review in Draft with the exact 40-character head/base marker documented in `docs/CODEX_WORKFLOW.md`; fix P0-P2 findings and repeat for every new head or base. Only after that Draft review is complete, and only when no other PR is being promoted, mark it Ready once. After GitHub records Ready, post the documented final exact-head/base request. `review-settled` requires a completion bound to that request comment or exact commit and its settle window, then `baseline-policy` must pass before any complete source or macOS lane starts. Any later commit or base update invalidates the promotion: return the PR to Draft, finish and review the current exact pair, then mark it Ready again. Squash-merge only with explicit authorization.
