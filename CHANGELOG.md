...
- 设置页的 AI 服务列出内置 AI、Qoder 和 Codex 三行，一次展开一个接入面板；浏览某行不改变默认服务。更多菜单区分断开（保留凭据）和移除 API Key。侧栏可选择服务并原位打开同一接入面板；Key 失效直达更换表单，修复后需明确重新发送，不会对另一文件误发。断开的默认服务不会被静默换成另一家。普通设置页不再套用模态 Tab 循环。
