...
- Finder 在同一受管项目目录内改名当前打开的 Working Copy 或同父目录的项目文件夹后，PageRoot 会按稳定身份自动同步新路径，并允许立刻在顶栏继续改名。重命名不创建 Project、Document、Working Copy 或 Version；跨磁盘、副本和多候选仍失败关闭。顶栏真实失败后不再强制聚焦，避免无法退出编辑。macOS 上 `/var` 与 `/private/var` 视为同一活动路径；进程被杀掉后仍可从已同步路径恢复。
