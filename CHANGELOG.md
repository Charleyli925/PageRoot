...
- 源 HTML 写入失败不再锁死整个工作台。源页会为当前 revision 写入并读回校验 Main Process 恢复日志；得到精确恢复或导出凭证后可切换标签、关闭文档和退出，同时仍明确显示原文件未更新。活动标签改为先保护、再关闭、后打开后继；标签布局写入不再阻止退出，桌面关闭最多自动重试一次并会释放精确关闭冻结。失败横幅现在占用工作区自身布局，可展开和复制诊断。
