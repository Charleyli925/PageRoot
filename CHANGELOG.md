...
- The first time a real local HTML opens in the verified edit canvas, a
  non-modal “快速开始” card explains click to select, double-click to edit
  text, and Preview for page-button interaction. Pointer hover uses the same
  three captions. Click selection range is unchanged, and there is no
  Preview-mode hover state. The card is remembered per install, not per
  project, and never appears on the built-in welcome page.
