...
- Agent 设置页现在只做无副作用的连接诊断，正式发送时才预检并冻结模型配置。DeepSeek 改为 SSE 增量接收，HTTP 与 ACP 都只在连续 45 分钟无有效协议数据时中断；运行区只显示等待时间、已接收大小和当前可执行操作。Agent 失败会立即替换“处理中”，允许原任务直接重试或结束，不生成半成品 Candidate；历史对话按日期与轮次分组。
