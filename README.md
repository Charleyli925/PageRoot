...
**Qoder CLI 可以通过源页受管的 ACP 会话自动执行；Claude Code、Codex、WorkBuddy、Qoder 以及其他能够读取本地文件并执行命令的 AI Agent 仍可使用剪贴板交接。** 两种方式都基于同一份受管本地任务，不需要把整份 HTML 复制进对话。
...
遇到生成内容、跨区域调整或需要更多判断的修改，源页会准备完整的本地任务包。你可以让经过校验的独立 Qoder CLI 通过受管 ACP 会话自动执行，也可以只复制一条简洁指令交给其他本地 Agent。AI Agent 读取冻结 HTML 和声明过的上下文，把一份完整 HTML 写入指定输出位置并执行随附的最终化命令；源页校验后先把结果独立保留，再把只读审阅作为首要入口，让你直接对照真实页面，决定最后使用哪一版。
...
- **选择受管执行或通用复制。** 源页可以启动、显示进度并停止经过校验的独立 Qoder CLI；Claude Code、Codex、WorkBuddy、Qoder 及其他具备文件能力的 AI Agent 继续通过剪贴板方式使用。
...
2. **评论并交给 AI Agent。** 标出需要较大修改的准确位置，补充说明或附件，由源页冻结本轮内容，再选择受管 Qoder 自动执行或复制可直接执行的交接信息。
