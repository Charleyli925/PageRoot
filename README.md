...
For larger or generative changes, PageRoot prepares a complete local task package and copies one concise handoff instruction. Your AI agent reads the frozen HTML and declared context, writes one complete HTML result to the designated output path, and runs the included finalizer. PageRoot validates the result and keeps it separate, then makes a read-only side-by-side review the primary next step before you choose which version to use.
...
遇到生成内容、跨区域调整或需要更多判断的修改，源页会准备完整的本地任务包，并复制一条简洁的交接指令。AI Agent 读取冻结 HTML 和声明过的上下文，把一份完整 HTML 写入指定输出位置并执行随附的最终化命令；源页校验后先把结果独立保留，再把只读审阅作为首要入口，让你直接对照真实页面，决定最后使用哪一版。
...
- **在页面上审阅 AI 修改，而不是硬看代码 Diff。** 修改前与 AI 修改后并排呈现，文案、结构和视觉变化分别标出；你原来写下的评论仍停在对应位置。
...
AI 结果看起来合理，也可能藏着遗漏。源页把“审阅对比”放在结果返回后的首要位置：默认从第一处变化开始，修改前后同步滚动，未改区域降低存在感，让真正需要判断的内容更醒目。你可以只看文案、结构或视觉变化，打开内容地图，切换单页，也可以回看当初提出要求的评论。
...
确认满意后，选择“打开 AI 修改后”；不满意，就回到修改前继续编辑。无论如何，AI 候选都安全保留，审阅本身不会静默替换当前 HTML。
...
4. **决定当前使用哪一版。** 满意就打开 AI 新版，不满意就回到修改前继续；任何一侧都不会被静默覆盖。
