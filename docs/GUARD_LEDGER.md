...
| `RUN_SUBMISSION_TARGET_UNSAFE` | Current comment must resolve exact by stable element ID on a complete managed Working Copy; whole-page body remains exact; unmanaged ID-less comments may still be exact/rebound | authority | cannot send; relink card | rail `canLocateTarget` | explicit relink then continue send | keep; extra Toast → degrade |
