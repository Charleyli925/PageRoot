...
minimize it again. Automatically triggered native dialogs are suppressed and
logged instead of popping up. Set `PAGEROOT_E2E_FOREGROUND=1` only when
explicitly debugging the native window. The hosted-macOS environment preflight
uses a visible inactive accessory window because that suite must prove
WindowServer painting without stealing keyboard focus.
