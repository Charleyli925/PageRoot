...
Every ordinary Pull Request open, update or reopen runs the separate impact-selected `PR Feedback` workflow, whether the PR is draft or ready; returning to Draft alone starts no run. Before promotion, a trusted collaborator requests Codex review for the frozen full head SHA and current base SHA while the PR is Draft. The Ready transition starts `review-settled` and is the sole final-review trigger; no additional review command is posted. The job continuously revalidates both live SHAs, requires a substantive exact-commit review, immutable exact-commit clean comment or phase-correct clean reaction strictly after its trigger, preserves Draft evidence when Ready replaces its provisional thumbs-up, fails closed when second-resolution timestamps cannot prove ordering, ignores empty reviews and `EYES`, waits 180 seconds and rejects active P0-P2 threads. `baseline-policy` then checks dependency advisories and the packaged-runtime closure. Only after both barriers pass do parallel Node, three-shard Chromium, real HTML, native Electron and deterministic AI groups start. `release-gate` refreshes that same baseline before attestation so a delayed failed-job retry cannot rely on an old advisory result. Any later commit or base update invalidates the pair; the PR remains unmergeable until it is returned to Draft, updated, reviewed and promoted again. Promote only one PR at a time so a merge cannot force several parallel candidates through the complete matrix again. Linux builds and shares only the Web renderer used by Node and Browser. Each macOS job builds the Electron renderer locally, runs the hosted-window preflight, then owns either the native Electron suite or the AI suite; those jobs can be rerun independently. Dependency, Playwright and Electron downloads are cached by lockfile identity.
