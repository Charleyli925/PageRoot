...
- 用户也可以在产品内直接重命名项目：这是被支持的主动路径，语义结果与上一条的被动同父改名完全一致——PageRoot 真的把项目文件夹改名，再原子更新 Registry 登记路径，`projectId`、版本、草稿和所有受管 HTML 名称都不变。产品内不引入任何独立的项目显示名字段。
- 主动重命名只接受用户明确输入的名字：目标名被其他项目占用时直接拒绝并说明，不静默改成 `<名称> (2)`；只有导入才自动分配同名序号。
...
项目的显示名不是一个另行的权威字段：它始终是登记项目文件夹名的显示投影。配置项目目录内完成可信文件夹改名后，Registry 原子更新登记路径，再从新目录名投影显示值。产品内的主动重命名走同一条事务：先在 Registry 落盘 pending 改名标记，再改名文件夹，最后更新登记路径与文件身份并清除标记；两侧崩溃都在下次启动时按标记与文件身份收口，因此显示名永远只有文件夹这一个来源。
...
- [ ] 在产品内主动重命名项目后，Finder 中项目文件夹名已变、项目内 HTML 文件名未变，同一 `projectId`/`documentId`/`workingCopyId`、版本与草稿全部保留，Registry 登记路径与列表显示名同时更新。`[PR 2B]`
- [ ] 主动重命名遇到已被占用的项目名时拒绝并说明，不改名、不改 Registry；名称非法或过长时同样拒绝。`[PR 2B]`
- [ ] 在主动重命名的文件夹移动前后各注入一次崩溃，重启后都收口到唯一可解释状态：移动前只丢弃标记，移动后按文件身份把登记路径修正到新目录，两种情况都不丢版本与草稿。`[PR 2B]`
