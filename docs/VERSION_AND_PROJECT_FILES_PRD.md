...
- 用户可以在 Finder 中重命名或移动项目文件夹，也可以重命名项目内的可见 HTML；重新打开后仍能恢复原项目和版本关系。
...
- MVP 不自动合并两个具有相同 `projectId` 的项目副本，也不在整个磁盘中后台搜索丢失项目。
...
- 项目文件夹名就是用户看到的项目名；用户可以在 Finder 中重命名或移动整个项目文件夹。PageRoot 识别成功后更新界面项目名和当前位置，但 `projectId` 不变。
- `.pageroot/project.json` 中的 `projectId` 是项目稳定身份；文件夹名和绝对路径都只是可变位置，不能用来判定项目身份。
- Registry 只缓存项目上次位置。用户从新位置打开项目内 HTML 时，PageRoot 向上找到 `.pageroot/project.json`，确认 `projectId` 后更新缓存，不能让旧 Registry 路径覆盖文件夹内的身份事实。
- 重命名项目内的可见 HTML 不自动重命名项目文件夹，也不创建 Version；HTML 所属版本由 manifest 中的 `versionId` / `workingCopyId` 确定，不能从 `A-V2.html` 这类文件名推断。
...
- 打开项目根目录中的可见 HTML 时，PageRoot 向上读取 `.pageroot/project.json`，再通过 `.pageroot/manifest.json` 中稳定的 `versionId` / `workingCopyId` 确认它是哪一份工作文件；`<名称>-V2.html` 只是默认可读名称，不承担身份判断。
- 用户可以在 Finder 中重命名可见 HTML。重新打开后仍进入同一 Working Copy，并把新相对路径写回 manifest；重命名不创建新项目或新版本。
- 用户可以重命名或移动整个项目文件夹。从新位置双击其中任一受管 HTML 时，PageRoot 确认同一 `projectId`，更新项目当前位置并恢复关系。
- 打开隐藏不可变快照时，只允许历史只读查看。
- 单独移出项目目录、无法向上找到对应 `.pageroot/project.json` 的 HTML，按外部文件精确打开；第一次持久化时建立新项目，不自动并入原项目或另一个项目。
- 如果 manifest、文件身份和用户选择不能唯一确认改名后的 HTML，只提示一次让用户确认；不能按文件名或 Hash 猜测，也不能自动跳到别的文件。
...
- 项目列表以当前项目文件夹名为用户项目名，不显示内部 `projectId`；用户在 Finder 中改名后，重新识别时同步更新列表名称。
...
- `projectId`：来自项目目录内 `.pageroot/project.json` 的稳定身份，不随文件夹或 HTML 改名、移动而改变。
- `displayName`：当前项目文件夹名的显示投影，可以随用户在 Finder 中改名而更新，不承担身份职责。
- `projectRootPath`：当前 Session 已解析的可变位置；Registry 只保存它的上次值，不能把它当作项目身份。
...
`.pageroot/manifest.json` 以 `versionId` / `workingCopyId` 映射当前相对路径和受控文件身份。`sourceRelativePath` 可以随用户改名而更新，但稳定 ID 不变；系统不得反向解析文件名来生成或猜测这些 ID。

manifest 可记录平台文件标识（例如 device、inode、birthtime）作为改名或同卷移动后的重新关联证据，但它不是业务身份。文件标识缺失、变化或同时命中多个对象时，必须由用户确认，不能退化为按文件名或 Hash 自动认领。
...
请求打开具体路径
    ↓
向上读取 .pageroot/project.json，并读取 manifest
    ↓
解析稳定 projectId / versionId / workingCopyId
    ↓
取消或隔离上一轮过期异步加载
    ↓
读取该路径精确字节并计算 Hash
...
- Registry 和旧绝对路径只能辅助定位；项目目录内的稳定 ID 与 manifest 才能确认身份。
- 上一文件的迟到回调不能覆盖新文件状态。
- 同 Hash 文件仍保持不同路径和用户选择身份。
- 无法唯一识别改名或移动后的对象时，请用户确认一次，不能按文件名或 Hash 自动认领。
...
- 项目文件夹或可见 HTML 改名、移动后，通过项目内 `projectId`、manifest 和受控文件身份重新绑定，并更新 Registry 的上次位置。
- 当前已经打开的项目在 Finder 中被改名或移动时，PageRoot 暂停向旧路径写入，重新定位同一 `projectId` 后继续保存；若不能唯一定位，只要求用户选择一次新位置，不在整个磁盘后台搜索。
- 复制整个项目文件夹导致两个位置具有相同 `projectId` 时，明确询问“作为新项目打开”还是“重新关联原项目”。前者为副本生成新的项目身份，后者只更新原项目的位置；系统不得静默合并或任选一个位置。
- 单个 HTML 被移到另一个项目时，不自动合并两个项目。
- 不允许全盘按同名文件自动认领。
...
现有 `~/Documents/PageRoot/项目记录/projects`、旧可读目录名、`working/*-V1.x.html`、冻结 Request 和 Attempt 可能已经被协议路径引用，因此 PageRoot 的迁移程序不得自动移动、重命名或批量改号。这里限制的是系统批量迁移，不把用户主动改名或移动定义为异常；旧项目完成稳定 ID 与 manifest 升级后，遵循本 PRD 的同一重新识别规则。
...
- [ ] 在 Finder 中重命名或移动整个项目文件夹后，从新位置双击受管 HTML，恢复同一 `projectId`、版本和草稿并更新项目列表名称。
- [ ] 重命名项目内可见 HTML 后重新打开，仍进入同一 `workingCopyId`，不创建新项目或新版本。
- [ ] 当前已打开项目被改名或移动后，PageRoot 不再写旧路径，并能重新定位后继续保存。
- [ ] 复制整个项目文件夹形成重复 `projectId` 时，系统要求用户选择“作为新项目打开”或“重新关联原项目”，不静默处理。
