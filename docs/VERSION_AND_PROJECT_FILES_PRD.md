...
| 阶段 | PR 范围 | 不在本阶段扩张的内容 |
|---|---|---|
| 第一阶段（PR 1） | 稳定项目/文档身份、原子导入、持续保存、Candidate 与用户采纳晋升、文件/文件夹改名和移动、精确打开状态机 | 历史继续编辑、完整项目列表、多文件可见性 UI |
| 第二阶段（PR 2） | 历史继续编辑、正式版本独立工作文件、可见 AI 任务与附件、完整顶部状态和项目体验 | 旧项目物理迁移、完整项目克隆、多文件网站 |
| 后续 | 经过独立计划和证据验证后再做旧项目物理迁移、完整项目克隆、资源包/多文件网站 | 不在 PR 1 或 PR 2 中隐式实现 |
...
PR 1 不引入并行的全局 Store。下表是每类状态的唯一权威位置；Registry 只是位置缓存，不能覆盖项目目录内的事实。

| 状态 | 权威位置 |
|---|---|
| `projectId` / `documentId` | `.pageroot/project.json` |
...
| `projectRootPath` | 当前 `ProjectSession`；Registry 只缓存上次位置 |

项目的显示名不是一个另行的权威字段：它始终是项目文件夹名的显示投影。每次重新解析项目根目录时都需从该目录重新读取这个值。
...
不得重新引入一个同时承担四种含义的 `currentVersionId`，也不得用缓存的绝对路径判定身份。
...
一个正式版本在 MVP 中最多有一份受管理的版本工作文件。重复“基于 V2 继续编辑”时打开同一份 V2 工作文件，避免产生多个无法区分的 V2 草稿。这份状态文件是 Hash、draft 和保存状态的唯一权威；`manifest.json` 只保留稳定 ID、Version 和相对路径的结构映射。
...
- [ ] 在 Finder 中重命名或移动整个项目文件夹后，从新位置双击受管 HTML，恢复同一 `projectId`、版本和草稿并更新项目列表名称。`[第一期]`
- [ ] 重命名项目内可见 HTML 后重新打开，仍进入同一 `workingCopyId`，不创建新项目或新版本。`[第一期]`
- [ ] 当前已打开项目被改名或移动后，PageRoot 不再写旧路径，并能重新定位后继续保存。`[第一期]`
- [ ] 复制整个项目文件夹形成重复 `projectId` 时，系统要求用户选择“作为新项目打开”或“重新关联原项目”，不静默处理。`[第一期]`
...
- [ ] 不支持的相对资源依赖失败关闭，不生成看似成功但资源断裂的项目。`[第一期]`
- [ ] 所有可见路径与隐藏路径均防止 `..`、软链接逃逸和跨项目写入。`[第一期]`

### 16.6 不随前两期隐式交付的范围

- [ ] 旧 `项目记录/projects` 物理迁移、清单、断点恢复与回滚证据。`[后续]`
...
1. PR 1：先修正文件打开 Session 与“Hash 不得改变导航”的边界，再建立稳定身份、原子导入、改名/移动恢复、Candidate 与 Promotion 分离。
...
4. 最后单独设计旧项目迁移、完整克隆和多文件网站；没有完整证据前不移动旧数据。
