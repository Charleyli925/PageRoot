...
- 文件留在项目外部并被再次打开时，只作为外部 HTML 精确查看；第一次持久化动作按新外部来源建立新项目，不回写、合并或重新关联原项目。单个 HTML 被移动到另一个项目根目录时同样不自动合并。
...
| 留在外部并重新打开 | 精确外部预览；第一次持久化时导入为新项目 | 不弹“重新关联” |
...
现有 `~/Documents/PageRoot/项目记录/projects`、旧可读目录名、`working/*-V1.x.html`、冻结 Request 和 Attempt 可能已经被协议路径引用，因此 PageRoot 的迁移程序不得自动移动、重命名或批量改号。旧项目继续在原兼容路径打开、保存和恢复，不做物理迁移；只有进入新模型且明确登记根目录后，才适用本 PRD 的配置目录与受管文件边界。
...
5. 旧项目根目录一旦登记，同样只允许该登记位置写入；把整个旧项目复制或移出登记范围不触发自动重新关联。
6. [ADR 0022](decisions/0022-user-owned-project-root-identity.md) 已将 Registry 定义为 v4 写入白名单，并把根目录文件身份限制为同父改名线索；它不改变旧项目的兼容入口。
...
| 旧项目与 ADR | 旧项目兼容；ADR 0022 曾采用用户目录身份优先、Registry 缓存模型 | 旧项目仍不物理迁移；v4 登记根目录遵守唯一写入位置，ADR 0022 已改为 Registry 白名单模型 | 兼容逻辑不得成为绕过登记根目录的第二身份系统 |
