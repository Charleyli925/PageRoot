...
| --- | --- | --- | --- |
| `bridge/project-context-service.mjs`, its standalone test, and package/impact-map entries | `DELETE` | No production import remains. Only its test and package/artifact allowlists reference it; v4 Project File owns new project mutation identity. | Remove as one closure. Never rewrite or delete a user's v3 registry/project tree. |
