...
变化分析先保存叶子级文案增删证据，再生成一份供框选和虚化共同读取的可读 footprint：小范围修改使用短语或独立行矩形，单字框在所属文字行内获得最小可读宽度，密集多行改写提升到最小语义文本块；不再把字词碎片描成锯齿多边形，也不跨越完整稳定句、未改文字块、卡片或 section。修改前继续使用红色虚线框与红色虚线删除线，修改后继续使用绿色虚线框且不增加下划线；每个语义变化组只显示一个“文本调整 / 句子改写 / 段落改写”等说明。结构 footprint 是最小受影响节点；视觉变化额外记录属性的影响 owner，内容属性使用细粒度内容框，背景、边框、圆角、阴影、尺寸和布局等盒子级属性使用完整组件 border box，并支配同一 owner 内的视觉子框。静态 footprint 仍是主事实；仅当源码空图表宿主能唯一配对、相关脚本确有变化且该宿主尚未被静态框覆盖时，审阅页才在 500ms 初始预算内用两次稳定的宿主内 HTML/SVG/Canvas 指纹补充一项视觉变化。受管 bootstrap 在作者脚本之前绑定取证所需原生能力，并记录每个 key 最初对应的源码宿主，只认冻结分析声明的精确 key/宿主组合；未声明属性被忽略，已声明宿主缺失、重复、被转移或替换、采样中漂移或取证异常时整批补充静默回到静态 Diff。宿主或内部区域由可见变为透明算主体变化，但任一透明层内部的不可见节点变化不单独取证，SVG 包裹组也不例外；页面绝对位移、动画、不稳定、单侧、超时或迟到结果均不增加框，也不显示提示；已有变化复用原 `changeId`。当前筛选 footprint 之外的整页内容按可见度虚化；导航焦点只负责滚动和活动说明。全部变化把同一位置的文案/结构/视觉框融合，只删除自身未变化且可由后代解释的祖先重复框；不同 `ownerKey` 的邻近组件不跨列或空白融合。内容地图只定位并揭示 Tab，不改变审阅显示逻辑。两页安全动作使用成对稳定 key 双向同步，且不受同步/独立滚动选择影响；无法安全对应时静默降级。所有运行态都不改变候选。通过左上角项目标识退出审阅会回到同一待处理页面；确认“返回 AI 修改前”则结束 active run 并直接恢复修改前 HTML 的编辑状态，但不删除评论、编辑记录、候选 Version、working HTML 或本轮记录，确认提示中的链接直接在 Finder 定位该候选 HTML。“打开 AI 修改后”需要确认；审阅层保持覆盖到候选编辑画布完全就绪后再一次性移除，不得闪现等待 AI 页面，完成三侧 Hash 校验后回到 `editing`。

受管会话对创建后，两侧审阅 iframe 若未在 1.5 秒内完成注册，或会话创建失败，本轮直接静默使用静态 Diff，迟到加载不再追加框。运行态图表补充的独立 500ms 决策预算只在两侧审阅 iframe 均完成加载后计时，不把正常的非对称资源加载算作匹配耗时。
该补充只在桌面端受管预览运输中启用。候选页面尝试替换审阅子 frame 时，主进程必须拦截；首次加载完成前的尝试还必须将同一临时会话一次性重载为只保留受管 bootstrap 的无作者脚本副本，审阅静默回退到静态 Diff，加载完成后的尝试则不应破坏当前页面；内联或纯浏览器审阅始终只使用静态 Diff。
