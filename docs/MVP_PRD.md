...
工作台不负责新建 HTML。每次打开本地 HTML 时，系统先只读分类：有效 v4 项目文件直接打开；已绑定的外部原文件显示“已经导入”确认，主操作继续当前项目；真正全新的外部 HTML 显示首次导入确认，用户确认后才建立新的 v4 Project 与初始 V1。导入不覆盖或改写原始 HTML；只有用户勾选「成功导入后，同意将原文件移至废纸篓。」且新画布已确认后，才把该原稿移入废纸篓。v4 以前的项目状态不迁移、不恢复，也不作为读取回退。V1 是初始只读基线，不是一次手动保存。详见 [首次打开导入确认](IMPORT_CONFIRMATION_PRD.md)。
...
变化分析先保存叶子级文案增删证据，再在真实布局中生成一份供框选和虚化共同读取的可读 footprint。比对范围是整页作者内容：`<main>` 内部逐层展开为区域，`<body>` 直属的页脚、页头、导航等同级内容各自作为一个区域，不得当作站点装饰排除。字符证据与范围框完全解耦：修改前只在精确删除字符上显示红色横虚线，修改后只在精确新增字符下显示绿色实点；二者都以「字」为颗粒度，且不得改变作者颜色、字号、字重或行距（见交互流程中的「字符证据标记（冻结合同）」）。范围框在两侧统一为低强调紫色实线。默认使用短语矩形；同一实际文字行至少有三个变化短语，或首尾字符证据跨度达到该行可见文字宽度 60% 时提升为完整行矩形；同一 owner 至少三行且其中至少 75% 已提升为完整行时，再提升为最小段落矩形。所有范围都是干净、轴对齐的长方形，必须包含其全部字符证据，并与遮罩透明孔使用完全相同的坐标；不再保留密度、稳定句或事实层 block 的第二套判断，也不把字词碎片描成锯齿轮廓或扩大到卡片/section。每个语义变化组只显示一个“文本调整 / 段落改写”等说明。结构 footprint 是最小受影响节点；视觉变化额外记录属性的影响 owner，内容属性使用细粒度内容框，背景、边框、圆角、阴影、尺寸和布局等盒子级属性使用完整组件 border box，并支配同一 owner 内的视觉子框。静态 footprint 始终是主事实：两张审阅页先完成静态投影，桌面可信父级才把冻结 HTML、完整 source SHA、视口和 source-backed bindings 交给主进程唯一的 RuntimeSnapshotOwner。SourceHostResolver 只接受直接 Canvas/SVG 与 source-empty 稳定宿主；owner 先重验源码绑定，再在隔离世界确认同一宿主和可见 Canvas/SVG paint，每候选最多一次 rect 与 PNG，并执行像素和字节预算。作者页没有 Bridge、Node、文件、网络、运行态消息或截图读取能力。可信 renderer 验证 PNG bytes、尺寸和 Hash 后只比较一次 before/after pair；缺失、重复、被转移、替换、取证异常、超预算、单侧、超时或迟到只使相关候选保留静态 Diff，不显示提示，也不会启动确认轮。已有变化复用原 changeId，原来未变化的区域最多新增一项视觉调整。审阅页没有退回本轮处理页的入口，左上角 HTML 图标与编辑态一样只打开“关于源页”；确认“返回 AI 修改前”则结束 active run 并直接恢复修改前 HTML 的编辑状态，但不删除评论、编辑记录、候选 Version、working HTML 或本轮记录，确认提示中的链接直接在 Finder 定位该候选 HTML。“打开 AI 修改后”需要确认；审阅层保持覆盖到候选编辑画布完全就绪后再一次性移除，不得闪现等待 AI 页面，完成三侧 Hash 校验后回到 `editing`。

静态审阅一旦就绪即可交互；静态页 ready 后只发起一个 before/after owner pair，并受 1.5 秒主进程总 deadline 约束。只有两侧合法已捕获 PNG 的 Hash、尺寸或字节数不同，才补充局部运行态 marker；任何一侧失败、不可用、取消或迟到都只省略该 marker，不能回写静态事实，也不能在静态审阅完成后迟到插入新框。
该补充只在桌面端受管 owner capture 中启用。owner 拒绝作者页的导航、弹窗、下载、webview、权限和非预览网络请求，完成后销毁窗口并撤销预览会话；内联或纯浏览器审阅始终只使用静态 Diff。
...
正式签名的 macOS App 在启动约 5 秒后检查 stable 更新，并在应用持续打开期间每 4 小时再次检查；macOS 应用菜单中的“关于源页”同时提供用户主动触发的“检查更新”、常驻的“查看更新内容”、固定 GitHub 仓库入口，以及安装包内固定“用户声明与免责声明”文本文件的本地打开入口。页面不能为该入口提供任意文件路径。自动与手动检查必须合并到同一个主进程更新控制器，不得并发建立两套下载或安装状态。检测到 stable 新版本时，顶部 HTML 图标下缘显示小号红色斜体 `New!`；当前版本或自动检查失败时不占位，手动检查的结果和失败原因则在“关于源页”原位显示。只有用户点击 `New!` 或“关于源页”中的“下载更新”后才开始下载，优先使用差分传输，失败时允许回退完整 ZIP；下载期间顶部文案保持不变，不显示百分比、进度条或动画。下载完成后弹出“现在重启 / 稍后”确认；选择稍后时顶部变为 `New! 重启更新`，再次点击打开同一确认，Canvas 不显示下载完成横幅。只有编辑器写入、草稿和 Bridge 关闭排空全部成功后才重启安装。普通退出不得暗中安装，GitHub 仓库只作为项目与人工后备入口。用户必须能看到“更新了什么”：“查看更新内容”在任何更新状态下常驻更新卡，只能请求主进程持有的固定 Release 地址，页面没有打开时在同一位置说明原因；正式 Release 的说明正文必须是 `CHANGELOG.md` 中该版本的小节，不得使用自动生成的提交或 Pull Request 列表，缺少该小节时发布流程在打 tag 之前就失败。
