...
正式签名的 macOS App 检测到 stable 新版本时，顶部发送按钮上方显示紧凑更新状态；当前版本或检查失败时不占位。新版本自动下载并优先使用差分传输，失败时允许回退完整 ZIP。下载完成后用持久提示和“重启更新”入口要求用户明确决定；只有编辑器写入、草稿和 Bridge 关闭排空全部成功后才重启安装。普通退出不得暗中安装，发布页只作为下载或安装失败时的人工后备入口。
