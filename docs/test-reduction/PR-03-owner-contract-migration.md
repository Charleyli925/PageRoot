...
| empty mixed-island sibling-text deletion keeps the host session and frozen descendants | `native-dom-structural-text.spec.mjs` observes the parent remaining editable after deleting only the bare text. |
...
| handoff fails closed before locking when a comment target is unsafe | `source-patch-engine.test.mjs`, Bridge Request submission integration, and Native/AI Electron relink and handoff flows own the observable rejection and recovery path. |
