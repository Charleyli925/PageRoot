...
| Discussion Turn short-lived read-only snapshot, its context Hash and the completed/interrupted turn outcome | Bridge-owned `runDiscussionTurn` in `scripts/discussion-turn-runner.mjs`, using the restricted discussion host in `scripts/qoder-acp-client.mjs`; the caller keeps command, executable identity and lease authority | none; the snapshot is one owner-only directory under `.pageroot/discussion-snapshots/` that exists only for the turn and is deleted on every exit path, and an unconfirmed deletion fails the turn. It never becomes Request, Candidate, Version or export authority, and the Working Copy stays the sole source authority | the Bridge discussion route and, through it, the conversation surface; the Agent receives only the snapshot directory and never the Working Copy path |
