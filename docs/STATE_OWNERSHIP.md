...
| Comment-rail disclosure, card measurement, virtualization, paired reveal/focus, Canvas selection and source-tagged target geometry | `CommentRailContainer` and `commentCanvasPort` | none; disposable React/adapter state only, never a comment fact | comment rail view; Workbench commands may read current target status without subscribing the root render |
