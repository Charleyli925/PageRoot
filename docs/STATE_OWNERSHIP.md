...
| Live source-file change early warning (`fs.watch` debounce, IPC `source-file-may-have-changed`, `/source-stat` hash compare) | Desktop `source-file-watch` plus renderer `DocumentWorkflow.observeExternalSourceChange` | none; watcher never writes persistence. `DocumentSession.persistState` is the only conflict projection | Workbench conflict banner |
| Force-unlock of a Working Copy conflict (adopt disk Hash as `saved`, no HTML write) | `ProjectFileRepository.forceUnlockWorkingCopy` via `POST /conflict/resolve` `force-unlock` | Working Copy state record only | `DocumentWorkflow.forceUnlockConflict` and the conflict banner |
| Current-source commit, same-directory two-state CAS replacement (`prepared` → atomic rename → `committed`) and Project File settlement | `ProjectFileRepository` | source HTML working copy and `.pageroot` runtime/autosave records; the save journal is two-state CAS owned by `ProjectFileRepository` | `/autosave` route adapter and restart recovery |
