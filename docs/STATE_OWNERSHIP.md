...
| Initial AI review runtime-chart snapshot batch, one conditional same-document confirmation batch for scope-only candidates, frame-registration/comparison deadlines and accepted supplemental host markers | Parent `AiReviewWorkspace` through `ReviewRuntimeVisualCoordinator`, behind the main-process managed-preview navigation fence | none; one bounded in-memory decision bound to the frozen document pair and declared host keys; only a first-pair difference without direct script causality reloads once, and session/load failure, navigation fallback and inline/browser review are static-only | effective review changes/outline and both isolated frame projections |
