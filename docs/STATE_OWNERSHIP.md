...
| Renderer Agent provider availability, selected provider/runtime/model/reasoning and selection-keyed preflight cache | `AgentCatalogState` | providers are keyed by provider id; selected selection is frozen; execution tickets are keyed by provider/runtime/model/reasoning/installation/trust and deleted when handed to execution | `WorkspaceController`, About, conversation sidebar and `RunWorkflow` |
