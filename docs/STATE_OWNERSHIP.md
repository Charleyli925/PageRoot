...
| One-shot Edit author-runtime identity, presentation-gated direct prepare grant, phase and final outcome | `EditAuthorRuntimeSession`, composed by `WorkspaceController` | none; one disposable `(sourcePath, canvasGeneration)` attempt and one revokeable resource session | Workbench loading-surface acknowledgement, initial-frame choice and `HtmlCanvasEditor` load/settle callbacks |
