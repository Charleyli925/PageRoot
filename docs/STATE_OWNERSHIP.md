...
| Pure semantic document revision, accepted operation lineage and generated in-process inverse | `SemanticOperationKernel` | none in PR4; complete input/output HTML and Hashes are caller-owned values, while SourcePatch is an internal exact-range materializer | focused kernel tests only until PR5 switches existing edit/history callers; no Workbench, Repository, Desktop, Runtime, AI or Review write path consumes this state yet |
