...
| Live source-file change early warning (`fs.watch` debounce, IPC `source-file-may-have-changed`, `/source-stat` hash compare; armed after every successful active-project read, including reopen) | Desktop `source-file-watch` plus renderer `DocumentWorkflow.observeExternalSourceChange` | none; watcher never writes persistence. `DocumentSession.persistState` is the only conflict projection | Workbench conflict banner |
