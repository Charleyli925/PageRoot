...
| Read-only tab display projections, hot/warm LRU order and per-tab Canvas mode/PageViewContext/scroll restoration | Controller-owned `DocumentSurfaceCacheSession`; `WorkbenchNavigationWorkflow` only touches/removes entries and Workbench only renders the frozen snapshot | none; bounded process memory only, maximum three hot iframes, eight HTML entries and 48 MiB; persisted tabs remain identity-only | pending tab presentation may show a script-disabled cached frame while canonical registered-project open validates the sole editable authority |
...
| Packaged Codex App Server installation and preflight evidence | `codex-provider.mjs` owns exact bundled package identity; `codex-app-server-client.mjs` owns the bounded JSONL probe and confirmed process cleanup; the artifact verifier owns raw source/package byte equality except for signed Mach-O files, whose executable content is compared after deterministic signature normalization, plus the native `--version` smoke | process-only version, installation digest, auth class and provider-namespaced model catalog; no account details, thread, Request, Candidate or Conversation write | default Provider catalog, installed app and exact-architecture package verification |
| Gated Codex execution process, ephemeral Provider binding and visible progress | `codex-app-server-runtime.mjs` owns the verified executable snapshot, JSONL thread/turn, sandbox, process cleanup and bounded events; Coordinator continues to own ticket, lease, cancel and session projection | thread/turn ids and Agent text are disposable process evidence. Only the fixed finalizer plus Repository validation creates a pending-review Candidate; Working Copy and Version authority never move on `turn/completed` | existing Conversation/sidebar projection and Candidate Review. `codexExecution=true` exposes this path; `codexDiscussion=false` still forbids pure discussion, and flipping execution false removes the Provider/runtime without migrating durable data |
