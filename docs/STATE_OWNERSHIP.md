...
| Staged Codex App Server installation and preflight evidence | `codex-provider.mjs` owns exact bundled package identity; `codex-app-server-client.mjs` owns the bounded JSONL probe and confirmed process cleanup | process-only version, installation digest, auth class and provider-namespaced model catalog; no account details, thread, Request, Candidate or Conversation write | provider contract tests and the manual safe preflight command; the default provider registry does not expose Codex in PR3 |
| Gated Codex execution process, ephemeral Provider binding and visible progress | `codex-app-server-runtime.mjs` owns the verified executable snapshot, JSONL thread/turn, sandbox, process cleanup and bounded events; Coordinator continues to own ticket, lease, cancel and session projection | thread/turn ids and Agent text are disposable process evidence. Only the fixed finalizer plus Repository validation creates a pending-review Candidate; Working Copy and Version authority never move on `turn/completed` | existing Conversation/sidebar projection and Candidate Review. `codexExecution` remains a source-owned false build fact until packaged evidence enables it |
