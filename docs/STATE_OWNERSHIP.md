...
| Registered project folder-rename transaction and its pending marker | `ProjectFileRepository` Registry writer | Registry `pendingProjectRenames[projectId]` holds the target path plus the root file identity recorded under the same write lock before the move; the record's own `registeredProjectRootPath` / `rootFileIdentity` become authoritative once the marker commits. The folder name on disk stays the only display-name source, so no separate project display-name field exists | Bridge `/registered-project/rename`, trusted desktop rename-project IPC, the `ProjectWorkflow` rename command, and startup recovery through `#recoverPendingProjectRenames` plus identity-based `#recoverRegisteredRootRename` |
...
| Product Qoder disk availability, use-time readiness ticket, shared delivery/About projection and copied guidance fact | Bridge-owned `AgentBridgeService` performs every uncached disk read and owns tickets; renderer `RunWorkflow` owns the current presentation snapshot and ticket reuse | availability and copied-guidance facts are process-local only; command tickets expire in Bridge memory and never become Request authority | `QoderAvailabilityCard` on delivery/About through the aggregate `WorkspaceController` snapshot; renderer never receives command, version, path, npm prefix or model count |
| Product Qoder task-keyed child/session lifetime, bounded public progress, stop/restart disposition and process cleanup | Bridge-owned `AgentBridgeService` in `scripts/agent-bridge-service.mjs`, using the restricted host in `scripts/qoder-acp-client.mjs` | live sessions stay in process memory; an exclusive `.pageroot/agent-bridge-leases/` record is only a crash/orphan fence and never task authority. Immutable Request `agentDelivery` authorizes mode/policy; runtime/Request/Attempt/Candidate remain the sole durable task authority | typed Bridge routes, `RunWorkflow` and `RunSession`; renderer never receives command, cwd, env, prompt or filesystem policy |
| Renderer discussion turn projection, one in-flight turn per Document, poll lifetime and the discussion drain disposition | Renderer `DiscussionTurnSession` owns the projection; `DiscussionTurnWorkflow`, composed by `WorkspaceController`, owns start/poll/cancel ordering; `RunWorkflow` remains the sole owner of Qoder availability and the consent-backed ticket and hands one over per turn | none; the Bridge turn is the only authority, and a status-read failure keeps the last projection instead of inventing an outcome. A turn is published as complete only when the Bridge says so; `interrupted` stays visible on its own | the aggregate `WorkspaceController` snapshot under `discussionTurn`, and the conversation surface through it |
| Discussion Turn session lifetime, bounded public turn projection, one in-flight turn per Document, and the short-lived read-only snapshot with its context Hash | Bridge-owned `DiscussionBridgeService` in `scripts/discussion-bridge-service.mjs` owns the session and its outcome mapping; `runDiscussionTurn` in `scripts/discussion-turn-runner.mjs` owns the snapshot; the one-use command ticket and executable identity stay with `AgentBridgeService` | none; live turns are process memory only, and the snapshot is one owner-only directory under `.pageroot/discussion-snapshots/` that exists only for the turn and is deleted on every exit path, with an unconfirmed deletion failing the turn. A discussion turn never becomes Request, Candidate, Version or export authority, never touches `activeRequest`, and the Working Copy stays the sole source authority | `POST /discussion/start`, `GET /discussion/status` and `POST /discussion/cancel`; the renderer receives turn state and the context Hash only, never command, cwd, snapshot path, prompt or page bytes |
| Development synthetic Qoder ACP fixture and sanitized live-probe report | `scripts/qoder-acp-spike.mjs`, reusing `scripts/qoder-acp-client.mjs` only against its disposable synthetic Project File | no product authority; ignored `output/qoder-acp-spike/report.json` is diagnostic evidence only | developers evaluating live account/network compatibility; never a release gate or Candidate authority |
