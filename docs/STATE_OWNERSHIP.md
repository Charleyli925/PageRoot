...
| Product Agent provider dispatch, Qoder disk availability, use-time readiness ticket, shared delivery/About projection and copied guidance fact | Bridge-owned `AgentBridgeService` owns ticket lifetime; the sole provider registry resolves legacy `qoder-acp`; `qoder-provider.mjs` performs uncached disk reads, installation/version/login/model checks and error normalization; renderer `RunWorkflow` owns the current presentation snapshot and ticket reuse | availability and copied-guidance facts are process-local only; tickets bind provider/runtime, opaque installation digest and capabilities, expire in Bridge memory and never become Request authority | `QoderAvailabilityCard` on delivery/About through the aggregate `WorkspaceController` snapshot; renderer never receives provider/runtime IDs, installation, digest, command, version, path, npm prefix or model count |
| Product Agent task-keyed child/session lifetime, bounded public progress, stop/restart disposition and process cleanup | Bridge-owned `AgentBridgeService` in `scripts/agent-bridge-service.mjs`, dispatching only through the provider/runtime registries; `scripts/agent/policies/` owns the single branded frozen policy and `scripts/agent/hosts/` owns the permission-separated execution/discussion Host Ports and managed terminal cleanup | live sessions stay in process memory; an exclusive `.pageroot/agent-bridge-leases/` record is only a crash/orphan fence and never task authority. Immutable Request `agentDelivery` authorizes mode/policy; runtime/Request/Attempt/Candidate remain the sole durable task authority | typed Bridge routes, `RunWorkflow` and `RunSession`; renderer and provider/runtime code never receive authority to choose command, cwd, env, prompt, filesystem paths or success criteria |
...
| Development synthetic provider/runtime fixture and Qoder ACP live-probe report | `tests/fixtures/agent-provider/qoder-provider.mjs` provides path-free registry evidence; `scripts/qoder-acp-spike.mjs` reuses `scripts/qoder-acp-client.mjs` only against its disposable synthetic Project File | no product authority; the fixture is process-only and ignored `output/qoder-acp-spike/report.json` is diagnostic evidence only | provider contract tests and developers evaluating live account/network compatibility; never a release gate or Candidate authority |
