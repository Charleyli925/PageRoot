...
| Development-only synthetic Qoder ACP session, protocol allowlist, child lifetime and sanitized probe result | `scripts/qoder-acp-spike-client.mjs` for one process-local session; `scripts/qoder-acp-spike.mjs` owns the disposable synthetic fixture | no product authority; ignored `output/qoder-acp-spike/report.json` is diagnostic evidence only | developers evaluating ACP v1 compatibility; never renderer, Bridge runtime or package consumers |
