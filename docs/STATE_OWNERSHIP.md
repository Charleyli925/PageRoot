...
| Renderer Agent provider availability, selected provider/runtime/model/reasoning and selection-keyed preflight cache | `AgentCatalogState`; `RunWorkflow` exposes legacy Qoder method names only as compatibility facades | providers are keyed by provider id; selected selection is frozen; tickets are keyed by provider/runtime/model/reasoning/installation/trust/purpose and deleted when handed to execution or discussion | `WorkspaceController`, About, conversation sidebar and `RunWorkflow` |
...
| Product Agent dispatch, use-time ticket, execution/discussion session, cancellation, timeout, lease, restart projection, bounded canonical events and shutdown drain | Bridge-owned `AgentRuntimeCoordinator`; `AgentBridgeService` and `DiscussionBridgeService` are route compatibility façades with no maps or process authority. Provider-specific availability/preflight/copy stays in the registry/provider | tickets bind provider/runtime/security profile/purpose/installation digest, are one-use and expire in coordinator memory. Sessions and event reductions are process-local; runtime/Request/Attempt/Candidate remain durable authority | unchanged typed Bridge routes and renderer projections; internal provider/runtime/security fields never cross them |
...
| Renderer discussion turn projection, one in-flight turn per Document, frozen Agent selection, poll lifetime and the discussion drain disposition | Renderer `DiscussionTurnSession` owns the projection; `DiscussionTurnWorkflow`, composed by `WorkspaceController`, owns start/poll/cancel ordering; `AgentCatalogState` hands it one purpose-bound ticket | none; the Bridge turn is the only authority, and a status-read failure keeps the last projection instead of inventing an outcome. Discussion tickets cannot execute | the aggregate `WorkspaceController` snapshot under `discussionTurn`, and the conversation surface through it |
| Discussion Turn projection, one in-flight turn per Document, and short-lived read-only snapshot | `AgentRuntimeCoordinator` owns session/seal/drain ordering; `runDiscussionTurn` owns the snapshot. A recorded question whose ticket cannot redeem is sealed failed/interrupted before returning | terminal conversation reply is written by `ConversationRepository`. A discussion ticket cannot execute and the turn never becomes Request/Candidate/Version authority | unchanged discussion routes/projection; `DiscussionBridgeService` only delegates |
