...
| Read-only tab display projections, hot/warm LRU order, five-entry frozen Canvas residency and per-tab Canvas mode/PageViewContext/scroll restoration | Controller-owned `DocumentSurfaceCacheSession` owns source projections; Workbench owns only the five mounted, inert iframe presentations and `WorkbenchNavigationWorkflow` only touches/removes entries | none; bounded process memory only, maximum five static display iframes, five frozen Canvas iframes, 20 HTML entries and 32 MiB of source projections; persisted tabs remain identity-only | pending tab presentation may show a script-disabled cached frame while canonical registered-project open validates the sole editable authority; an exact source-Hash Canvas hit cancels duplicate runtime preparation but never commits Session facts |
...
| Renderer Agent provider availability, selected provider/runtime/model/reasoning, installable/installSource/installState overlay and selection-keyed preflight cache | `AgentCatalogState` | providers are keyed by provider id; selected selection is frozen; execution tickets are keyed by provider/runtime/model/reasoning/installation/trust and deleted when handed to execution; install jobs are not owned here | `WorkspaceController`, About, conversation sidebar and `RunWorkflow` |
| Product ACP allowlist, managed Agent inventory, in-flight install jobs and install drain | Bridge `AgentCatalog` / `AgentInstaller` | managed bytes live under Electron `userData/agents/<providerId>/<version>/` (or `HTML_AI_AGENTS_ROOT`); jobs are process-local; user-global npm is never spawned | `GET /agent/providers`, `POST /agent/install`, `POST /agent/install/cancel`, Qoder provider discovery and Coordinator shutdown |
| AI Request freeze, pre-Request Agent use-time check, persisted-boundary verification, safely fenced same-Request Agent start/retry, unknown-POST authority reconciliation, polling lifecycle, cancellation ordering and conflict command sequence | Renderer `RunWorkflow`, composed by `WorkspaceController` | the user intent synchronously freezes selection; after Request publication every path reads the durable Request selection and never the mutable catalog selection | Workbench intent/Drawer/Toast adapter, typed Bridge client and Bridge run lifecycle |
...
| Product Agent dispatch, use-time execution ticket/session, cancellation, timeout, lease, restart projection, bounded canonical events and shutdown drain | Bridge-owned `AgentRuntimeCoordinator`; `AgentBridgeService` is the route façade. Provider-specific availability/preflight/copy stays in the registry/provider | tickets bind provider/runtime/security profile/execution purpose/installation digest, are one-use and expire in coordinator memory. Sessions and event reductions are process-local; runtime/Request/Attempt/Candidate remain durable authority | typed Agent routes and renderer projections; internal provider/runtime/security fields never cross them |
...
| Packaged-unregistered Codex App Server installation evidence | `codex-provider.mjs` still owns exact bundled package identity for leftover extraResources; it is not registered. Codex ACP discovery/preflight is owned by `codex-acp-provider.mjs` | process-only version, installation digest, auth class and provider-namespaced model catalog; no account details, thread, Request, Candidate or Conversation write | default Provider catalog, managed `userData/agents/codex/` closure and leftover packaged natives |
| Gated Codex ACP execution process, ephemeral Provider binding and visible progress | `codex-acp-provider.mjs` plus the shared `acp` runtime own launch, ACP session and process cleanup; packaged-unregistered `codex-app-server-runtime.mjs` is not a new-start authority. Coordinator continues to own ticket, lease, cancel and session projection | Agent text is disposable process evidence. Only the fixed finalizer plus Repository validation creates a pending-review Candidate; Working Copy and Version authority never move on ACP completion | existing Conversation/sidebar projection and Candidate Review. `codexExecution=true` registers Codex ACP; `codexDiscussion=false` still forbids pure discussion, and flipping execution false removes the Provider without migrating durable data |
...
| Current source-backed comment resolution, visibility, coordinates, marker eligibility and natural document height | `html-canvas-comment-layout` measurement owned by `HtmlCanvasEditor`, stabilized by `commentCanvasPort` | none; disposable snapshot tagged by rendered source Hash, applied page-view generation and exact target-ID set | `CommentRailContainer` and the inherited Canvas-height CSS variable; Workbench root does not subscribe |
...
- Interactive-preview sessions, page-view context and the direct one-shot Edit
  author-runtime session are disposable. They do
