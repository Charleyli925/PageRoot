...
| Browser-workbench navigation admission, receipt and tab order/active/pending/mounted/runtime-owner identity | Renderer `WorkbenchNavigationSession` owns the transaction phase/receipt and `WorkbenchTabsSession` owns the tab projection; the Controller-owned `WorkbenchNavigationWorkflow` is the only coordinator | validated `workbench-tabs.json` stores only `tabId + projectId + documentId` and the active document tab; it is restart-convenience metadata written best-effort, with no close veto and no path, title, HTML, Hash, Request, Candidate, Version or Conversation authority | Startup/restore, local/recent, registered/sidebar/tab, browser-file, OS-external and confirmation all enter one ordered admission stream; ProjectWorkflow applies the tab mutation synchronously through the correlated application receipt before its presentation event |
| Read-only tab display projections, hot/warm LRU order, five-entry script-disabled Canvas residency and per-tab Canvas mode/PageViewContext/scroll restoration | Controller-owned `DocumentSurfaceCacheSession` owns source projections; Workbench owns only the five mounted inert static iframe presentations and `WorkbenchNavigationWorkflow` only touches/removes entries | none; bounded process memory only, maximum five static display iframes, five script-disabled Canvas iframes, 20 HTML entries and 32 MiB of source projections; Script-enabled Edit pages are discarded on deactivation | pending tab presentation may show a script-disabled cached frame while canonical registered-project open validates the sole editable authority; Runtime DOM never enters this cache contract |
...
| Renderer Agent provider availability, selected provider/runtime/model/reasoning, bounded public model catalog, installable/installSource/installState overlay and selection-keyed preflight cache | `AgentCatalogState` | providers are keyed by provider id; selected selection is frozen; execution tickets are keyed by provider/runtime/model/reasoning/installation/trust and deleted when handed to execution; public models are a bounded projection from preflight, never account records; install jobs are not owned here | `WorkspaceController`, Settings, conversation sidebar and `RunWorkflow` |
| 源页 Agent session Token | Bridge `AgentRuntimeCoordinator` | process memory only; injected only as `PAGEROOT_API_KEY` / `PAGEROOT_API_VENDOR` / `PAGEROOT_API_BASE_URL` for `pageroot`/`http`; never written to disk, `ui-preferences.json`, logs or GET responses | Settings connection card; Renderer posts `POST /agent/session-credential` and never rereads the secret |
...
| Disposable Edit author-runtime program identity, scoped prepare grant, public phase/load outcome and at-most-once compatible recovery | `EditAuthorRuntimeSession`, composed by `WorkspaceController`; Main owns exact-source admission and immutable compatible/exact resource sessions derived from one preparation | none; bounded process memory only. One resource closure is keyed by `(sourcePath, canvasGeneration)` and may serve repeated disposable frames while exact authored script identity is unchanged. It does not own or cache physical last-known-good availability | Workbench loading acknowledgement and the candidate controller's identity-bound settlement result; Runtime DOM never enters persistence |
| Disposable Runtime candidate identity, latest-candidate phase, last-known-good identity, ignored stale callbacks and Native Edit/IME promotion gate | Pure `EditRuntimeCandidateController`; `HtmlCanvasEditor` owns only the corresponding iframe, viewport and Selection effects | none; bounded renderer memory only. `superseded` is coordination, never authored-program failure | candidate load/activation/deadline/rAF/microtask/position callbacks and `EditAuthorRuntimeSession`; only the latest identity can publish a terminal result |
