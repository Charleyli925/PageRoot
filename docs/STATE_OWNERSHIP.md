...
| Packaged Codex App Server installation and preflight evidence | `codex-provider.mjs` owns exact bundled package identity; `codex-app-server-client.mjs` owns the bounded JSONL probe and confirmed process cleanup; the artifact verifier owns source/package byte equality and the native `--version` smoke | process-only version, installation digest, auth class and provider-namespaced model catalog; no account details, thread, Request, Candidate or Conversation write | default Provider catalog, installed app and exact-architecture package verification |
