...
| Current preview/edit display context, safe reveal transition and capture generation | Workbench page-view context state | none; source-backed in-memory projection only | `HtmlCanvasEditor` presentation layer and toolbar |
...
- Interactive-preview sessions and page-view context are disposable. They do
  ...
  second copy of the source HTML. Toolbar and Option-click actions may propose
  a new context for the current document key, but do not own or persist it.
