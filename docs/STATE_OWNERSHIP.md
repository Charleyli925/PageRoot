...
| Current Edit Runtime degradation presentation (`none`, `static-preparing`, `static-visible`, `last-known-good-readonly`) | `HtmlCanvasEditor`; Workbench only derives `direct-static-visible` when `EditAuthorRuntimeSession` reports a direct static fallback before any Editor failure transition | none; disposable renderer presentation. usable static states are quiet; retry stays in the More menu. A verified newly loaded static frame retires the previous frame's degradation. `last-known-good-readonly` retains the top reload/export recovery notice | Workbench notice and read-only projection; it cannot change Working HTML, Runtime outcome or slot identity |
