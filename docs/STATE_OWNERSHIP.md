...
| Agent recovery intent (origin surface, project/document/request identity, target provider/field, error kind, configuration generation) | Renderer Workbench memory via `agent-recovery-intent.js` | none; never Keys, tokens or login URLs; discarded when the round ends or the user dismisses recovery | Settings focus, conversation recovery bar and sidebar `AgentSetupPanel` |
