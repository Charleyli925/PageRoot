...
- 顶部栏为两行文件信息、编辑/预览、项目、上轮处理与发送按钮保留完整垂直空间；“添加全局评论”仅放在右侧评论标题行，顶栏不显示或保留独立入口，全局评论的数据、恢复与排序语义保持不变。HTML 图标是“关于源页”的入口。图标在无更新、`New!` 和 `New! 重启更新` 三种状态中始终使用同一几何中心，不因预留标签而上移。存在更新时，较小的标签以绝对定位局部叠在图标下缘，点击语义仍是下载或重启确认；图标、标签、标题、快捷操作和焦点轮廓都不得互相遮挡或越过底部分隔线。审阅画布约束在顶栏下方的内容行内，收起态工具条不得溢出覆盖顶栏“关于源页”等共享控件。
- 当前 HTML 画布。
- 直接编辑工具。
- 评论与本轮要求面板。
- 自动写回状态。
...
已登记项目的打开由 Repository 一次返回完整 `Project + Document + OpenTarget + HTML + Hash`
...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；正式审阅页复用正式工作台顶栏并移除所有 Demo 标记。顶栏左上角的 HTML 图标在审阅页与编辑页语义完全一致，都是“关于源页”入口；同形控件不得在两个页面里指向不同动作，因此审阅页不再提供“退回本轮处理页”入口，离开审阅只能通过“返回 AI 修改前”或“采纳 AI 修改”这两个明确决定。审阅浮层不得铺满整个工作台去盖住顶栏；收起态审阅工具条可以上探露出手柄，但必须裁切在内容行内，不得拦截顶栏“关于源页”等共享控件的点击。采纳会把项目当前源晋升到新的工作文件路径，但仍然可见的审阅浮层必须继续持有准备审阅时冻结的源路径与文件标题：两页预览会话不得因中途路径切换而重建（页面不得空白重载、滚动位置不得丢失），浮层拆除是采纳过程中唯一一次视觉切换。
