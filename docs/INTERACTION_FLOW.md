...
- `pagePresentationState`：两页共享的 Tab、折叠、业务按钮与 input/select/textarea 状态。它与滚动模式无关，用户从任一侧操作都向另一侧同步；无法建立安全对应时静默保持当前侧结果，不增加打扰提示。
...
2. 分析器产出的细粒度 rect 先删除可由后代解释的祖先框，再按连续关系形成最终可见 footprint。指定筛选下，框与虚化遮罩必须消费这份完全相同的最终矩形集合；遮罩以矩形为透明孔，因此整个框内都保持清晰，框外才按 `contextVisibility` 虚化。不得再通过 DOM 祖先/后代 opacity 另算一份上下文。
3. 相邻或跨行但连续的 rect 可在同一变化、同一语义色内融合；相隔列、大片空白或不同区域不跨越合并。`全部变化`只在 footprint 强重叠时融合不同类型，避免把邻近但独立的变化并成大框。
4. 文案新增在修改后页使用绿色虚线框并标注“新增文案”；文案删除在修改前页使用红色虚线框并保留红色删除线；文案修改的两页对应框分别标注“文案修改前/后”。结构变化在两页统一使用蓝色虚线框；视觉变化在两页统一使用紫色虚线框。新增文字本身继续保留作者颜色、字重和装饰。`全部变化`中的单类型框仍保留语义色，同处多类型只显示一个紫色融合框和类型并集说明，不叠出重复框。
...
两页的 panel 与 action key 在冻结的修改前/修改后文档之间成对生成，优先使用显式目标身份，再使用同一 panel 内的控件类型、语义与位置，避免 AI 改文案或调整顺序后左右失配。Tab、折叠、业务按钮以及 input/select/textarea 的值与选中态始终双向镜像；导航、提交、弹窗、下载和宿主 IPC 仍被隔离层阻止。每次本地或镜像动作、DOM 尺寸变化、字体完成和安全 panel 切换后都重新投影框选，内容地图不是触发正确显示的前置条件。同步滚动的横向位置直接联动，纵向按相同内容区域及区内相对进度联动；独立滚动只关闭这两种滚动跟随，不关闭动作同步。
