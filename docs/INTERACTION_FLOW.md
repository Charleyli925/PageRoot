...
无法证明时立即准备 Candidate。带 `is` 的内建自定义元素、无法证明文本/注释
插入边界的混合内容父级，以及 `html`/`head` 作为目标父级，都继续重建。
...
| 复制、删除、插入、跨父移动 | 已证明的纯源码子树先提交语义操作；身份、当前节点和局部结果均可验证时在当前 iframe 原地完成，副本或恢复对象获得本次事务的受控编辑权限。无法证明时走既有 Candidate。含运行内容的选中子树不允许复制 | 拒绝时源码、历史和 DOM 均不变。源码已接受而投影失败时保留源码和历史，进入画布恢复，不把结果改回“编辑被拒绝” |
...
- 页面应尽量保持当前 Document、共享滚动位置和 Selection。结构修改在源码身份、
  ...
  Script 重新计算的修改，仍在语义操作/checkpoint 边界自动重建。无法证明
  文本/注释边界的混合内容插入与移动、以及带 `is` 的内建自定义元素，不得
  报告原地成功。`body` 可以作为普通子元素的目标父级，但其自身仍不可作为
  删除或移动对象。局部同步失败、源码目标失效或已经存在的必要 Candidate 被新源码过期时，仍只能请求
