...
- 当前项目入口。
- 当前文件名、版本与保存状态；文件名右侧常驻粗体“+”和斜向右上的外部打开图标，分别用于快速“打开本地HTML”和“在默认浏览器中打开”当前已知源文件。两者在鼠标悬浮或键盘聚焦时于图标右上方显示小号浅色浮层提示，不参与顶部栏布局计算；外部打开会先围栏刚送达的原生输入并等待该精确编辑 revision 安全写回，写回中保持禁用，失败时不启动浏览器。纯浏览器预览没有本地文件权限，因此仅展示禁用态的外部打开图标。桌面本地文件在安全保存时还可从文件名原位重命名，并提供“在文件夹中打开”快捷入口。Finder 显示源文件、默认浏览器打开、项目记录、本轮 Request 和历史 Version 都是一击一次的本地副作用：失败显示可恢复错误但不锁住项目，也不会自行重放；用户再次点击才发起下一次调用。
...
1. “编辑”对普通页面仍直接构建禁用作者脚本的静态画布。若桌面端发现明确的
   ECharts 初始化、唯一稳定且源码原本为空的图表/表格宿主和可判定的 classic script，则复用既有
   的画布验证/加载门槛等待一次独立隐藏探测；不兼容、超时或失败时不显示
   新提示，直接显示这份静态画布。源码内联 SVG、静态 HTML 和 PNG 按原样显示；
   脚本生成的 Canvas/SVG、动态表格、表单值、Tooltip 和滚动状态均不冒充可编辑源码。
2. 探测通过后，系统直接在正式编辑 iframe 中用同一批固定脚本字节再运行一次，
   在固定且受限的 ECharts 最终帧等待窗口后，立即冻结计时器、监听器、Observer、动画和图表交互，并复核宿主外的源码节点、
   文字和属性。获准的空宿主可扩展为图表或静态表格并改变布局；其运行时内容始终
   只读，不能双击编辑或成为评论 TargetRef，但整块原始宿主仍可像其他模块一样留评论。
   验证完成前该 iframe 不显示也不接入编辑事件；通过后直接正常显示，
   失败则用同一权威源码静默回退静态画布。没有隐藏 staging iframe，也没有等待用户
   停止编辑或 IME 后的后台替换。运行时节点只能留在获准空宿主内，永不写回源码。
3. 点击“预览”后，当前 HTML 在独立隔离文档中像普通页面一样运行。
   页面脚本、相对资源、Tab、表单、SVG、Canvas 和动态表格可以工作；
   页面不能导航宿主窗口或获得 PageRoot 的文件、编辑、评论和 AI 权限。
4. 用户在预览中切换 Tab、展开详情或改变同类显示状态。
5. 点击“编辑”时，系统不捕获运行时图像或页面状态；它只把仍能唯一映射到
   源码节点的 Tab/显示状态临时应用到画面，因此用户看到并编辑刚才选择
   的页面。该临时状态不写回 HTML。

编辑画布继续复用既有的链接、按钮、表单、输入与键盘默认行为拦截，保留
单击选择和双击文字编辑；不增加全页 `inert`、第三种模式、弹窗或另一套通用
事件层。保存、导出、撤销/重做、版本对比和发给 AI 的基础文件始终是完整、
原样的权威 HTML；编辑路径没有 runtime capture API、PNG/Blob 缓存、位图
属性或运行时 DOM 投影回写。
