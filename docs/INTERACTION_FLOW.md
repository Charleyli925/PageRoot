...
- 左侧栏是全局项目上下文，固定分为标题栏、品牌区、可滚动项目列表和底部固定操作区。项目列表包含“当前项目”和 Registry 中的其他已导入项目；当前项目行只展示上下文，版本树摘要仍可从左侧展开并进入既有安全导航。品牌区打开 About，底部齿轮在标签栏中打开设置。
...
- 设置是一个普通标签页，不是弹窗，包含“AI Agent”和“软件更新”两个区块。齿轮默认把焦点放到关闭按钮；AI 助手中的“设置 Agent”打开同一标签页并聚焦 Agent 操作。设置页打开时触发 Agent 可用性检查，并在未解决状态返回前台时复查；更新后台控制器、侧栏 New! 和重启确认保持原有生命周期。
