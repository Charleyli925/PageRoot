...
轮廓或文案。说明只出现在目标轮廓旁的 Hover 标签上，不出现在工具条上。

单击选择范围不变：仍走 `findCanvasSelectionElement` 与 `selectElement`，
不改成「选择该文字或文字宿主」。没有「预览模式下可操作」这一档 Hover；
编辑模式里部分 Tab 已经能操作，不为此增加眼睛徽章或工具栏「去预览」按钮。
...
1. 普通单击继续只选择元素并显示选框；选择算法与 Hover 分类器独立，
   第二次普通单击仍然只是选择，
