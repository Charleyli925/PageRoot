...
1. 点击“预览”后，当前 HTML 在独立隔离文档中像普通页面一样运行。
   页面脚本、相对资源、Tab、表单、SVG、Canvas 和动态表格可以工作；
   页面不能导航宿主窗口或获得 PageRoot 的文件、编辑、评论和 AI 权限。
2. 用户在预览中切换 Tab、展开详情或改变同类显示状态。
3. 点击“编辑”时，系统无额外提示地采集一次有限页面上下文；不要求用户
   再点按钮，也不根据预览滚动位置定位编辑画布。
4. 编辑器继续从当前权威源码构建禁用脚本的画布，只把仍能唯一映射到
...
需要在编辑状态完整可见的报告内容必须有源码静态回退：图表优先内联
SVG，表格行直接写入静态 HTML，不能表达为 SVG/HTML 的视觉内容再使用
PNG。JavaScript 只负责增强交互或更新这些静态节点。这样预览可以正常
交互，编辑画布可以静态显示和选择真实源码内容；仅存在于运行时的内容
只能在预览中看到。
...
审阅显示只有一个互斥的七状态模型，但视觉上组织为两个同样式的分段控件：

- `页面预览`：`双页 / 左页 / 右页`。进入审阅默认选中“双页”，以无差异标记的修改前/AI 修改后完整画布展示；左页按钮以弱提示标明“修改前”，右页标明“修改后”，且不改变三个按钮的既有宽度。“左页”或“右页”只显示对应一页；必须点击明确的“双页”才能回到双页，不使用“再次点击当前单页”的隐藏手势。
- `变化审阅`：`全部变化 / 文案 / 结构 / 视觉`。选择任一变化类型都会恢复双页画布并自动聚焦该类第一处变化；某类没有结果时显示明确空态。

内容地图使用稳定的大区域作为导航锚点，但可见变化边界只能来自小粒度 marker，禁止直接给导航区域画框。“全部变化”把同一变化中相邻、横向重叠的 marker 合并为适度虚线框，不得把中间大量未变化内容纳入；“文案”不显示外层大框，先按逗号、顿号、冒号、分号、句号、问号、感叹号或换行截成有意义短句，再只给字符级删除和新增片段加删除线/下划线，完全相同的文字不得标记；“结构”标出具体新增、删除、移动或属性变化节点，并在内容地图说明发生了什么；“视觉”覆盖 inline style、class、可视属性和适用 CSS 规则，在具体受影响元素上使用蓝紫虚线并说明圆角、间距、颜色等变化。“上下文可见度”只调节非聚焦区域，从双页视图直接拖动时先自动进入全部变化并聚焦第一处变化；数值按 0–100 线性映射到 opacity，0 完全不可见，100 完全无遮挡。

内容地图按页面区域与原页面 Tab 分组，同时列出已修改和未修改区域；点击地图项或上一处/下一处定位隐藏 Tab 内区域时，两侧先安全切换到对应页签，再各自定位。双页且同步滚动时，用户直接点击任一侧安全 Tab 控件也镜像到另一侧；普通链接、表单和业务按钮绝不镜像。同步滚动的横向位置直接联动，纵向按相同内容区域及区内相对进度联动，不按两页原始像素高度硬对齐；程序化跟随端在本轮同步完成前不能反向成为主导端，页面顶端与底端必须精确钳制。仍可切换独立滚动和适应/100% 缩放。

两侧审阅画布在无同源、无表单提交、无导航、无弹窗、无下载、无宿主 IPC 的隔离沙箱中运行原页面交互，因此原页面 Tab、折叠区和纯页内控件可点击审阅，但运行态变化不会写回 HTML、候选 Version 或当前项目。“返回 AI 修改前”先逐行说明不会采用本次 AI 返回、继续以修改前版本为基线，以及 AI HTML 会保留；第三行可直接在 Finder 打开本轮文件夹。确认后结束当前待打开选择，直接回到原 HTML 的可编辑状态，原评论和编辑记录继续保留，候选 Version/working HTML 仍留在历史与原位置。“打开 AI 修改后”确认后保持处理抽屉关闭，先挂载编辑画布，再于下一帧进入 9.1 的正式激活事务；不得闪现等待 AI 页面。两条确认中，“继续审阅”是建议保留的紫色操作，“返回修改前版本”为灰色操作；打开候选的最终按钮文案为“确认并打开”。
