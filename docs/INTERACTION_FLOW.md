...
[导出当前编辑] [预览外部版本] [重新载入外部文件] [强制解锁项目]
```

- **预览外部版本** 只读展示磁盘当前 HTML，不写入任何持久化状态。用户可“返回我的编辑”或“接受此版本（丢弃我的编辑）”。
- **重新载入外部文件** 会二次确认后用磁盘版本覆盖当前编辑，行为与既有 `reload-external` 一致。
- **强制解锁项目** 仅在用户确认后清空 Working Copy 锁状态并采用磁盘 Hash，不写回 HTML。这是冲突死锁的确定恢复路径，避免手动删除 `.pageroot/`。
