...
- 当前项目入口。
- 当前文件名、版本与保存状态。顶部状态徽章默认沉默，只在正在保存、保存失败、正在等 AI、有 AI 修改待查看或正在看历史（只读）时出字；“基于 Vn / 项目最新 Vm / 本地修改 / 候选身份”留在版本列表、历史与处理面板，不常驻顶栏。文件名右侧常驻粗体“+”和斜向右上的外部打开图标：粗体“+”在其正下方展开锚定浮层“打开 HTML”（浮层左缘与“+”左缘对齐、无背景遮罩，内含最近打开列表与“打开本地 HTML”入口），斜向右上图标“在默认浏览器中打开”当前已知源文件。两者在鼠标悬浮或键盘聚焦时于图标正上方水平居中显示小号浅色浮层提示，不参与顶部栏布局计算；提示只跟随指针与键盘聚焦，鼠标点击后不因按钮保留焦点而常驻，按钮自己的浮层在场时也不出现；外部打开会先围栏刚送达的原生输入并等待该精确编辑 revision 安全写回，写回中保持禁用，失败时不启动浏览器。纯浏览器预览没有本地文件权限，因此仅展示禁用态的外部打开图标。桌面本地文件在安全保存时还可从文件名原位重命名，并提供“在文件夹中打开”快捷入口。Finder 显示源文件、默认浏览器打开、经验证项目根、可见 Version Working Copy 和派生 AI任务都是一击一次的本地副作用：失败显示可恢复错误但不锁住项目，也不会自行重放；用户再次点击才发起下一次调用。
...
- 顶部栏为两行文件信息、编辑/预览、项目、上轮处理与发送按钮保留完整垂直空间；“添加全局评论”仅放在右侧评论标题行，顶栏不显示或保留独立入口，全局评论的数据、恢复与排序语义保持不变。HTML 图标是“关于源页”的入口。图标在无更新、`New!` 和 `New! 重启更新` 三种状态中始终使用同一几何中心，不因预留标签而上移。存在更新时，较小的标签以绝对定位局部叠在图标下缘，点击语义仍是下载或重启确认；图标、标签、标题、快捷操作和焦点轮廓都不得互相遮挡或越过底部分隔线。
- 当前 HTML 画布。
- 直接编辑工具。
- 评论与本轮要求面板。
- 自动写回状态。
...
顶部“项目”按钮打开的是**当前项目工作台**：只呈现当前这一个项目的身份、状态、项目规则与版本历史，不再混入其他项目。打开或切换到别的项目由文件名右侧的“+”负责（见下），形成“+ = 打开/切换，项目 = 管理当前”的清晰分工。

“项目规则”（`PROJECT.md`）是用户在项目里长期维护的核心内容：整个项目以后每次 AI 修改都会使用的长期规则；新项目默认留空，用户按需填写；项目空闲时可修改，处理时只查看。它在工作台顶部是一行**就地展开的披露区**（不跳转、不离开版本树），小字说明为“每次 AI Agent 修改本项目 HTML 都会读取”；展开即自动准备项目记录（登记项目），不要求用户先发送一次 AI 任务。保存状态默认沉默：只有用户真实修改过且已写回磁盘，才显示绿点与“项目规则已保存”。规则读取完成前不可编辑；未保存时折叠或切换项目会被阻止，并明确要求保存或还原。中文输入法处于组词阶段时暂停规则自动保存；用户明确“还原修改”时先废弃持有 marked text 的旧输入框，再恢复已保存内容，迟到的 composition/input 不能把拼音写回。

`runtime-state.json`、`edit-audit.jsonl` 等内部记录仍在本地保存，但不作为产品入口暴露给用户；用户需要的每轮要求、AI 返回与本地改动都在右侧版本详情里以友好方式呈现。

只打开顶部“项目”面板不登记项目。若项目记录建立失败，失败原因和“重试建立”显示在规则行下方；若 `PROJECT.md` 读取失败，编辑器不使用占位文案冒充文件内容，用户可以重试读取；失败时版本树仍在旁边，不会把用户带离工作台。

当前文件名右侧的“+”在其正下方展开锁定浮层“打开 HTML”（左缘与“+”对齐、无背景遮罩，点击浮层以外任何位置或按 Escape 即关闭），提供：

- 最近打开（默认最多 6 条，排除当前文件，可逐条移除记录）。
- 打开本地 HTML。

浮层在场期间，承接“点击外部关闭”的透明层覆盖整个窗口并显式退出窗口拖拽，因此顶部栏也算“浮层以外”：点“+”左右的顶栏空白、再点一次“+”都直接关闭浮层，不会被系统当成拖动窗口。

顶部“项目”按钮打开居中的当前项目工作台：尺寸、圆角、磨砂玻璃质感与页脚节奏与 AI 交接面板一致（点击遮罩或“关闭”退出），分为三层：

- **顶部身份**：项目名与一行安静状态（保存状态 · 版本数），右侧是可点按的“在文件夹中打开”（定位源 HTML 所在位置）与“导出副本”。整个面板只有这一个文件夹入口，不在版本层面重复。
- **项目规则**：就地展开的规则编辑区。
- **版本树（主从双栏）**：左侧是按时间从旧到新排列的版本轨道图，右侧是选中版本的详情；页脚右下角固定“在画布中预览”。

工作台不再展示其他项目列表；切换到别的项目统一走“+”浮层。

### 版本树与版本详情

版本树把“每一版基于哪一版”画出来：连续修改沿同一条轨道向下；从一个不再是轨道末端的版本继续修改时，会开一条新轨道，并以直角折线从父版本拐出。轨道只往外展开、不回收，因此连接线永不交叉；分支颜色只用靛紫家族的四个深浅，不引入第二种强调色。

版本行的标题是**用户当时写下的要求**。优先级为：该版随版保留的第一条评论（“目标：要求”，多条时追加“等 N 条”）→ 该轮冻结的要求→ “本地编辑 · N 处” → 分叉头的“从 V16 分出” → 留空；V1 固定为“原始导入”。当前存储的版本条目本身不带评论，因此要求文本由 Bridge 从该版所属轮次的冻结请求中读出（该字符串由用户自己的评论文本拼成），随版本行一并返回：单行收敛、限长 120 字，按轮次缓存（版本一旦晋升即不可变），读不到则静默回退、不影响工作台。不使用版本 manifest 的 `summary` 作为标题：它不是 AI 返回的内容（completion / candidate 协议里没有该字段），也不保证是人类可读的修改总结；文件名在同一项目内又全部相同。

右侧详情以中性的“版本 N”为标题（避开同一句要求在一屏内重复两次），依次呈现：最新版本 / 当前编辑基础等状态标签、“基于 版本 N · 该版要求”（可点击跳转到父版）、本轮要求或我留的评论（含附件）、AI 对话补充（没有则不显示）、本地编辑（修改前→修改后）。不展示附件数、画布类型、目标定位状态等对用户无意义的统计；候选校验只在被标记为需审阅或未通过时出一行提示。

“打开 HTML”浮层选中最近项或“打开本地 HTML”时先关闭浮层再进入同一安全项目切换与导入确认流程；首次导入确认框出现时浮层已不在场，不会与其堆叠。
...
指针下的目标、轮廓、文案和单击结果来自同一个 `resolveCanvasPointerHit`。
点在子内容上选小框；点在有内容的模块自己的 padding、边框或分栏间隙上选该模块；
...
1. 普通单击继续只选择元素并显示选框；Hover 与单击共用 `resolveCanvasPointerHit`。
...
当前文件名右侧始终保留独立的“+”按钮，点击后在其正下方展开锁定浮层“打开 HTML”（左缘对齐、无背景遮罩）而不直接弹出系统文件选择器；浮层内的“打开本地 HTML”与最近项才真正进入同一文件选择与项目切换流程。标题的小铅笔仍只在鼠标悬停或键盘聚焦时出现；“+”与重命名入口的点击热区互不嵌套。用户在文件选择器取消时，当前项目与画布完全不变。浏览器预览路径的隐藏文件选择器必须在浮层内“打开本地 HTML”的同一次点击里打开（该点击本身即用户手势），包括编码错误后画布“重新选择”直接打开选择器的情形；不得先 drain 当前项目再请求选择器。内存中打开的 HTML 会记下内容 Hash，以便下一次选择能通过画布切换围栏。若当前编辑、评论、附件或项目规则还没有安全收口，系统只在用户确认会切换项目的动作之后，才沿用项目切换边界完成保存或阻止切换。系统文件选择器始终从受管项目根（“文稿 › PageRoot › 项目”）起步；该目录不可用时回退到“文稿”，不记忆上一次选择过的目录。
...
在画布中单击内容只更新选择框和对应评论的高亮，不主动改变共享滚动位置。点在子内容上选择该小框；点在有内容的模块自己的 padding、边框或分栏间隙上选择该模块。完全空的模块（没有元素子节点、也没有非空白文字）不能选中。取消选择走 `Esc`、顶部栏留白和评论栏未被评论卡片或输入框占用的区域，以及 `html` / `body` 页面根；选择工具栏、评论卡片、评论输入框及其操作仍保留选区直至对应操作完成。已保存评论的 TargetRef 不随后来的单击改绑到祖先或子孙。只有用户从评论卡片、未保存评论入口或重新定位流程明确要求前往目标时，系统才滚动到对应内容。
...
提交准备意图在任何异步检查或登记之前同步占位；快速双击、键盘快捷操作或重复事件只能复用/退出，不能并行建立两个 Request。`preparing` 只去重、不锁 Canvas，期间的后续编辑由最终 `freezeNow()` 捕获。自动模式的使用前检查验证版本、登录和当前可用性，并产生一次性短时 ticket；成功后同一次提交直接复用该 ticket，不连续检查两次。About 的主动检查只更新共享状态，不授权稍后的发送；发送仍在当次点击重新取得 ticket，避免两次操作间 CLI 身份变化后先创建 Request。若用户已打开过“项目资料”，提交沿用同一项目登记结果。
...
用户每次发送前只打开一个“怎样交给 AI？”弹窗。弹窗不跳 About、不再叠加第二个弹窗，
只保留关闭按钮与两张卡：

- “Qoder CLI”：自动执行并在 PageRoot 显示进度。选择即确认本轮采用
  `trusted-local-agent-v1`；更完整的本机权限与第三方处理说明继续由用户声明承担，主流程只
  保留“Qoder 会读取本轮 HTML、评论和附件；结果先进入审阅”。
- “复制任务”：保留通用本地文件交接，用户把一条简洁指令粘贴给 Qoder 或任意能够读取
  本机文件并执行命令的 Agent；该卡始终可用。

弹窗与 About 共用同一份 `checking / ready / not-installed / auth-required /
...
选择 Qoder 或点击“检查并继续”才执行完整使用前检查。成功即关闭原弹窗并直接继续冻结、
建 Request 和启动处理，不再二次确认；失败仍留在原 Qoder 卡内：
...
处理面板标题、状态徽章、摘要和底部操作必须读取同一份阶段投影：
...
| 候选已安全保留并等待用户决定 | `AI返回结果` | `可在审阅中对比查看修改差异` | `等待确认打开`；连续性需确认时为 `请先审阅` | `审阅对比`，符合条件时同时显示 `采纳并打开` |

发送结果待确认、复制失败、校验失败、无变化和外部冲突必须覆盖上述正常流程文案，
并显示各自的确认、重试、返回编辑或冲突选择操作；不得沿用“直接粘贴”或“可在审阅中”提示。
...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；正式审阅页复用正式工作台顶栏并移除所有 Demo 标记。顶栏左上角的 HTML 图标在审阅页与编辑页语义完全一致，都是“关于源页”入口；同形控件不得在两个页面里指向不同动作，因此审阅页不再提供“退回本轮处理页”入口，离开审阅只能通过“返回 AI 修改前”或“采纳 AI 修改”这两个明确决定。采纳会把项目当前源晋升到新的工作文件路径，但仍然可见的审阅浮层必须继续持有准备审阅时冻结的源路径与文件标题：两页预览会话不得因中途路径切换而重建（页面不得空白重载、滚动位置不得丢失），浮层拆除是采纳过程中唯一一次视觉切换。
...
- `changeFilter`：`全部变化 / 文案 / 结构 / 视觉`，任一时刻恰好选中一个。筛选按钮只改变框选与虚化所读取的变化集合，不强制返回双页、不改变上下文可见度或缩放；三个类型按钮各带一个与画布标注同色的差异色点（文案为删除红与新增绿，结构为蓝，视觉为紫与琥珀「疑似」），色值与页内标注同源。切换筛选时，若当前定位目标在新筛选下已不再匹配，则一并把定位目标移到该筛选的第一处变化；当前目标仍匹配时保持用户所在位置不动。没有匹配变化时保留所选筛选、不移动定位并显示空态。
...
进入审阅的默认组合是 `双页 + 全部变化 + 上下文可见度 18% + 同步滚动 + 适应`，并可把第一处变化设为导航目标；“导航到第一处”不等于只显示第一处，当前筛选中的全部变化仍保持清晰并显示边界。默认选择“适应”是为了让双页对比第一眼就完整可见，不出现横向裁切；用户可自行切到 100%，单双页共用同一个缩放轴。页面预览三个按钮使用紧凑的固有宽度。单双页都让文档视口尽量铺满 Canvas：除 pane 边框和双页分隔外只留极小间隙，不保留装饰性大留白。

审阅工具栏是浮层，不占布局位置：进入审阅时默认展开并浮在两页之上，页面始终从 Canvas 顶部开始铺满。点中间的把手收起，再点一次展开；只有这一次点击能改变展开状态，鼠标经过不再偷偷展开或收起。工具栏展开与收起都只让浮层自身滑动，两页的位置、尺寸、滚动位置和框选测量都不因此变化，也不产生任何重排。为了不让浮层压住面板标题，内容地图抽屉在工具栏展开时同步下移一段，收起时随同一条曲线回到原位。

变化数量使用统一的两级口径：工具栏与内容地图都以“变化区域”为单位，且同一个筛选下的数字必须可互相对应：全部变化时工具栏显示 `N 个变化区域`，选中某一类型时显示 `N/M 个变化区域`（N 为当前筛选命中、M 为全部），并与逐处导航的 `当前/总数` 共用同一个 N；内容地图分组在全部变化时显示 `N/M 个区域有变化`，筛选时显示 `N/M 个区域含某类变化`，不得拿筛选后的 0 冒充“该分组没有任何变化”；抽屉标题显示 `M 个区域`。画布上的标注徽章是区域内的明细事实，不参与这些数字。修改前页头保持中性标识，AI 修改后页头额外使用紫色顶边与极浅紫底，让横向滚动后仍能立刻分辨哪一侧是新版本。
...
运行态 Snapshot 补充只在桌面 Electron 的受管 `RuntimeSnapshotOwner` 中启用。候选作者脚本永远不能获得取证通道、截图或宿主权限；只有 parser-blocking 的受管首个 bootstrap 可在闭包中持有精确 Element 绑定与一次性私有投影端口。内联/srcDoc 或纯浏览器审阅不生成运行态候选，只使用静态结果。owner 为每个请求创建新的非持久 `pageroot-preview` 会话和隐藏窗口，拒绝导航、弹窗、下载、webview、权限和非预览请求，完成或失败后立即销毁并撤销该会话。唯一例外是白名单图表库脚本：捕获前由主进程在有界预热预算内把冻结 HTML 中声明的白名单 `<script src>`（与 Edit 一次性 ECharts 会话共用同一主机白名单与有界抓取器）抓取并冻结为字节，隔离会话仅从冻结字节应答这些钉住的 URL，捕获页面自身永不触网；同一捕获会话的前后两侧必见到相同字节或相同缺失，抓取失败/超时/超大只让相关图表保持无法核实。

1. 分析器先建立高置信度节点配对，再从实际结果生成类型，禁止仅凭“同标签 + 同位置”把两个区域配成一对。文案先按混合行内节点中的语义文本流计算叶子级精确增删证据，直接文本与嵌套的 `span/strong` 不得断开；事实层明确区分 `insert/delete/replace/layout`，并把每侧的 `evidenceRanges`、`phraseGroups` 与 `anchorOffset` 分开保存。`evidenceRanges` 是唯一字符级变化证据；`phraseGroups` 只保存可读短语边界，不能提前决定行框或段落框。纯新增只在修改后页显示新增字符证据，修改前页只保留不可见定位锚点；纯删除只在修改前页显示删除字符证据，修改后页只保留不可见定位锚点。定位锚点不得进入框选、遮罩透明孔、标签或变化类型统计。正文相同而 authored `br` 偏移变化时使用 layout operation，并归入既有视觉变化，不生成红绿文字 evidence。文本配对先建立语义层级：表格以 `tr` 为行单元并用逻辑列边界约束 cell，列表以 `li` 为项单元，连续 `br` 文本在每个非空行都具有编号标记时以编号行为单元，普通 `p/标题/直接文字块` 才作为文字单元；直接的无文字媒体、图形、画布和表单控件也是独立原子单元，不能被丢入空文字流。不同父级、单元类型或 HTML 标签不得互配。每组已配对父级内的兄弟单元按 DOM 顺序做有界单调配对，重复类目、重复短句、低置信或多解候选保持 unmatched；超出矩阵预算时只允许有限前瞻的单调退化，不得恢复为全局贪心。未配对的编号行只框该行，未配对的 `li/tr` 以自身为结构 owner，后代不再与其他父级配对。范围计算先按句末、换行和有意义的标点建立语义边界，相同标点前后的独立改动不跨界合并；句内只在共享词或连续短语足以构成稳定锚点时继续细化。超长且无标点的文本超过计算预算时，只能用前后均唯一的连续词组建立递归分块边界，不得把两处修改之间的大段稳定文字一起作为字符证据。短中文块可用至少两个共同汉字补充配对证据，但前后不同词组中偶然出现的单个相同汉字不得抵消删除与新增。句子配对是 1:1 的，因此它无法表达“四句合写成一句”或“一句拆成四句”；句层配对之后必须再做一次幸存词对齐，把两侧已标记 token 流互相比较，删去仍然出现在页面上的文字，不得因为整句未配对就把幸存文字整段划为删除或新增。该幸存对齐是单向收缩的：它只能减少标记，永不新增标记；只有连续相邻、含词且至少两个可见字符的词串才算幸存，单个汉字或单个标点不算；若两侧将同时被清空，则保留原范围，以免隐藏纯顺序调整。只剩标点的残留差异在任一侧仍有实词差异时丢弃，以免一个逗号自己占一个框；当标点是唯一差异时仍然上报。仅承担排版的行内标签和 `br` 不构成结构事实；文字与结构读取同一 semantic pair graph，视觉事实保持独立但不得越过已配对祖先边界。新增/删除结构本身不得顺带被判成视觉变化。稳定且自身未变化的大区域只作导航锚点，永不直接成为字符证据。
   静态分析完成后，`SourceHostResolver` 只从 `SourceIndex`/`TargetRef` 配对直接 Canvas/SVG 或 source-empty 的稳定宿主；它不读取脚本、评论范围、computed selector 或运行时猜测节点。只有同一精确 source host 在前后两侧都已有同 geometry owner 且同 `box` scope 的 canonical style fact 时，才抑制该候选；同一 outline 内其他元素的事实不得代替它。宿主枚举可以超出捕获预算（至多 `candidateLimit`），以便评论锚定的宿主排在捕获队列最前，再截取回 32 个候选；owner 侧的像素与字节预算按同一优先级顺序消耗。两张静态审阅页就绪后，可信父级向主进程提交冻结 HTML、完整 source SHA、侧别、固定视口和绑定。owner 在脚本前重验源码路径/标签/身份，随后在 isolated world 确认同一运行态宿主及可见 Canvas/SVG paint；每个宿主最多一次 rect 和 PNG。PNG 的字节、尺寸、Hash 和聚合预算重新在可信 renderer 验证。比较结果按候选输出 `{candidateKey, changeId, verdict}`（verdict 为 `changed` 或 `suspected`，其他值整批拒收）；outline 只聚合内容地图和导航，不参与 geometry。
   每侧首个 parser-blocking bootstrap 响应私有携带候选 key 到“元素路径 + 完整窄 fingerprint”的绑定，并在作者脚本前捕获原始 `Element`；后续 bootstrap 响应不含绑定。可信父级使用与评论完全分离的随机 challenge 和私有端口传递结果，端口逐项校验 contract、`sessionId + side + sourceSha256`，不新增 generation owner。候选 key/绑定不进入 authored HTML、DOM 属性、普通 window message 或可重读的 bootstrap，TargetRef 与 PNG 从不进入审阅页。bootstrap 以 `Map<Element, ReviewProjectionFact[]>` 原子替换运行态 facts，并在绘制时与静态 serialized facts 做 canonical 加法合并；empty、无效、超时或 capture 失败都只留下原静态 Diff。目标被替换或断连时只丢弃该精确宿主的运行态框，禁止回退到 outline；source-box 签名只在 parser-blocking 捕获绑定时严格校验，图表库渲染时改写自己宿主的 style/尺寸属性/子节点是正常行为，不再视为目标丢失。
...
3. 字符证据与可读范围在布局时才汇合，并只由一套几何规则决定最终文字框。每个文字 marker 一律用 `Range.selectNodeContents(marker).getClientRects()` 测量；同一侧按 `changeId + semanticOwnerId + geometryOwnerId + operation + tone` 聚合，并保留其中全部 `phraseGroups`。默认以短语生成普通轴对齐圆角矩形；过窄的单字或标点框只在所属实际文字行内扩展到最小可读宽度。任一实际文字行满足“至少三个语义短语组”或“首个到最后一个字符证据的横向跨度达到该行可见文字宽度的 60%”时，统一提升为一个完整行矩形。一个允许提升的 owner 至少有两条实际文字行，且完整行矩形覆盖其中至少 75% 时，统一提升为一个最小段落矩形——一段连续正文的两行不应各自拿一个平行框；稳定文字可以落在这个更大的可读范围内，但永远不会因此成为字符证据。短语矩形在紧行距下会伸进相邻行，两个交叉的框围着同一句话就是噪声；因此相邻两行的短语矩形（计入渲染时的统一内边距）一旦重叠，这两行统一提升为它们各自的完整行矩形。碰撞只是局部排版意外，不算“该行被改写”的语义信号，因此不计入段落矩形的语义提升比例；但当该 owner 的每一行都带证据且全部已提升时，同样统一为一个最小段落矩形——跨行插入若每一个渲染文字行都被改动，就读作一个干净矩形而不是一排平行行框；“每一行都带证据”是硬条件，它保证段落矩形永不跨过一整行未变文字（例如未动的开场或收尾行）。行矩形仍然存在于部分行未带证据的 owner；同一 owner 的相邻行矩形可以共用一条边：每个行框必须向下伸到足以包住自己的绿点，强行要求行框之间留空会把字符证据挤到框外。收并后的单一矩形仍必须完整包含该 marker 的全部字符证据。该规则同样适用于 `replace/insert/delete`，不再保留密度、字符数、稳定句、sentence scope 或事实层 block 决策。段落 owner 只允许 `p/li/td/th/caption/标题/无嵌套块的直接文字 div`；grid、flex、多列、跨 cell、大空白、离散视觉行或嵌套块一律回退为行框或短语框，不能扩大到 card、section、article、form 或 table。每个最终文字框必须是一个干净长方形，禁止 shaped SVG、碎片 union 或锯齿多边形，并且必须完整包含其全部字符证据；不同 semantic owner 永不融合。非文案记录先做一次嵌套收并：同一 `changeId` 内，只有两种情形才收并到外层：外层是新增或删除的结构事实（整个子树本就是新的或没了，内层框只是重复外框已经说过的话），或者内外两者都是结构事实（一层套一层的结构框不再提供定位信息而只是视觉噪声）；独立的视觉事实永远保留自己的框，盒子级视觉 owner 不得吞掉后代自己的呈现变化。包含关系必须 DOM 与几何两者同时成立，因此相邻的独立事实永不被融成一个框或一个透明孔，被收并的事实数量由徽章的「×N」承载。嵌套收并之后，非文案记录仍只有同 owner、同事实且确实相接或重叠时才能再融合。`全部变化`中，新增/删除的 `li/tr` 结构 owner 支配同 semantic owner 的文字子框；文案筛选仍只显示内部文字，结构筛选只显示父元素。
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生坐标、尺寸完全相同的透明孔。遮罩以当前会话、页面侧别和 presentation epoch 唯一的 SVG luminance mask 表示：整页白底保留虚化，每个 canonical path 以黑色挖孔；重叠孔天然取集合并集，绝不能用 `evenodd` 让重叠区重新变暗。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文；作者页的 `svg path`、`mask rect` 或通用 `path/rect` 样式也不得重写受管遮罩原语。
   初次进入审阅时，父级先等待 before/after 的静态 Review 都就绪并投影静态 footprint；随后仅并行请求一个 owner capture pair，运行态补充绝不阻塞静态审阅。owner 的总 deadline 为 4 秒，并在首帧 paint 后等待 1.2 秒 settle 再取样：settle 必须盖过图表库默认约 1 秒的入场动画，否则两侧在不同动画相位取样会把未修改的图表伪造成确认变化。运行态判定是三态的：只有双侧都真实捕获、像素一致或在噪声预算内、且画面不是近似单色时，该宿主才作为“核实未变”参与虚化；双侧捕获并产生超出预算的像素差才可能成为精确宿主的 changed fact，且必须有源佐证：当前 HTML 字节是权威，只有静态 diff 在同一区段也发现了改动时，像素差才作为 `style` 类型并入那条变化；源未改动的区段里的像素差无法归因到本轮编辑，只能进入疑似态，不得凭像素凭空造出一条变化区域、页缘修订条与导航位；捕获缺失、解码失败或双侧近似单色（图表根本没渲染的特征）为“无法核实”。无法核实的宿主一律输出 suspected fact：该宿主在两侧都豁免虚化，修改后页绘制琥珀色虚线“疑似有改动”框，修改前页不画框。评论锚定是下限而不是门槛：被评论的宿主一定出框，其余无法核实的宿主也出框，除非全页过半宿主都无法核实——那是一个页面级成因（图表库被阻断、脚本错误）而不是每个宿主一个成因，此时只保留被评论的那些，以免整页 CDN 图表全部挂满琥珀框。不可核实的宿主无论画不画框都不得被虚化成“已核实未变”的上下文；丢失捕获能力时运行态宿主进入疑似态而不是静默虚化。创建、绑定、取证、PNG 验证或撤销会话失败同样落入疑似态，截止后的结果不追加框、不展示失败原因；不会启动第二个 fresh pair。运行态 Map 的测量复用现有 presentation epoch：隐藏宿主没有旧框，Tab 稳定并提交当前 epoch 后才按仍连接且 fingerprint 未漂移的同一 `Element` 重测。
5. 每一个语义变化组在范围框边只显示一个几个字的说明，不以“当前定位”为显示前提；同组的后续短语或行框不重复标签。说明直接读取该 marker 自身的 operation，不得因为同一 section 另有相反方向的变化而改名：`insert` 统一写“新增内容”，`delete` 统一写“删除内容”，`replace` 的短语或行范围写“文本调整”，段落范围写“段落改写”；移动、结构和一般视觉分别写“位置调整”“结构调整”“视觉调整”，`layout` 明确写“换行调整”。文字范围框在两侧统一使用低强调紫色细实线，只表达受影响范围；结构与视觉框使用同样克制的细虚线。边线只负责圈出范围，不得粗到与正文争夺注意力。修改前的精确删除字符使用红色虚线删除线，修改后的精确新增字符在每个字下显示一个清晰的绿色实点。字符证据不增加背景底色、不改作者颜色、字重、字号、行距或原有装饰，绿色实点不实现成下划线或 `text-emphasis`，并且任何字符证据都不得落在其范围框之外。删除与新增的可见标记样式以「字符证据标记（冻结合同）」为准。`全部变化`仍只会合并同一事实、同一 owner 的非文案重叠记录，或同一 `changeId` 内确实嵌套包含的非文案记录；文字范围框不与结构或视觉记录融合成 shaped 框。同一位置的相邻但不相包含的独立事实（例如 box-style 与 layout）保留各自的框和说明。相邻且说明文字相同的说明会聚合成一个代表徽章并写成「{说明} ×N」，只折叠重复文字：每处变化仍各自保留框、透明孔与字符证据，不同说明永不互相聚合，跨列或相距较远的同类说明也不聚合。N 数的是事实而不是框：一个已经收并了嵌套事实的框，按它代表的事实数计入簇总数。徽章按屏幕尺寸反向缩放，聚合的纵向判定范围随缩放同步放大，因此在「适应」比在「100%」聚合更积极；当前定位所在的变化始终作为其聚合簇的代表徽章。说明徽章数与「变化区域」计数是两个口径：一个变化区域可含多个不同说明的徽章，聚合只影响同类重复徽章的显示，不改变可导航的变化区域数。区段说明（变化区域与内容地图共用的那一句）与徽章是两个层级，判据也不同：整段只在一侧存在时写「新增内容」「删除内容」；两侧都在时，必须有区段内的结构级证据：只有 `added` 结构标记（`insert` 文字足迹可以同向佐证但不能单独成立）才写「新增内容」，只有 `removed` 才写「删除内容」，因为区段内整块插入仍然是插入，只按区段存在性判断会把它降级成「结构调整」。段落里多出几个字不升级整段措辞——那已经由「文本调整」描述，且它旁边的徽章本来就写「新增内容」。一旦同时出现相反方向、`replace`、就地修改或移动证据，就回到按类型拼出的「…调整」，不拿只描述一半的措辞当整段结论；`layout` 在词表里有自己的说明「换行调整」，因此同样否决这个判断，只有 `none` 完全中性。这一句是互斥的：「新增内容」与「…调整」在词表里是替代关系，不并列堆叠。
