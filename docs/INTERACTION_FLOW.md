...
仅由这条评论范围放宽、而没有变化脚本直接引用的宿主，若首轮前后指纹不同，必须在
同一冻结文档的新 frame run 中让两侧各自重复同一指纹；任一侧与首轮不同、重载失败或
超时都不得产生运行态 marker。具备直接脚本因果证据的宿主，以及首轮本就无差异的
评论范围候选，不触发这次额外复采样。
...
运行态图表补充只在桌面 Electron 的受管 owner capture 中启用。两张审阅 iframe 只负责静态审阅，候选作者页永远不能获得运行态取证通道、候选绑定或截图；内联/srcDoc 或纯浏览器审阅只使用静态结果。owner 为每个请求创建新的非持久 `pageroot-preview` 会话和隐藏窗口，拒绝导航、弹窗、下载、webview、权限和非预览请求，完成或失败后立即销毁并撤销该会话。
...
1. 分析器先建立高置信度节点配对，再从实际结果生成类型，禁止仅凭“同标签 + 同位置”把两个区域配成一对。文案先按混合行内节点中的语义文本流计算叶子级精确增删证据，直接文本与嵌套的 `span/strong` 不得断开；事实层明确区分 `insert/delete/replace/layout`，并把每侧的 `evidenceRanges`、`footprintGroups` 与 `anchorOffset` 分开保存。证据负责说明哪些字确实变化，只有 footprint groups 能产生文字框。纯新增只在修改后页显示新增 footprint，修改前页只保留不可见定位锚点；纯删除只在修改前页显示删除 footprint，修改后页只保留不可见定位锚点。定位锚点不得进入框选、遮罩透明孔、标签或变化类型统计。只有前后均有证据的成对 replace 才可能提升为 block；整句单侧变化使用 sentence scope，稳定前后句永远不被密度阈值带入 block。正文相同而 authored `br` 偏移变化时使用 layout operation，并归入既有视觉变化，不生成红绿文字 evidence。文本配对先建立语义层级：表格以 `tr` 为行单元并用逻辑列边界约束 cell，列表以 `li` 为项单元，连续 `br` 文本在每个非空行都具有编号标记时以编号行为单元，普通 `p/标题/直接文字块` 才作为文字单元；直接的无文字媒体、图形、画布和表单控件也是独立原子单元，不能被丢入空文字流。不同父级、单元类型或 HTML 标签不得互配。每组已配对父级内的兄弟单元按 DOM 顺序做有界单调配对，重复类目、重复短句、低置信或多解候选保持 unmatched；超出矩阵预算时只允许有限前瞻的单调退化，不得恢复为全局贪心。未配对的编号行只框该行，未配对的 `li/tr` 以自身为结构 owner，后代不再与其他父级配对。范围计算先按句末、换行和有意义的标点建立语义边界，相同标点前后的独立改动不跨界合并；句内只在共享词或连续短语足以构成稳定锚点时继续细化。超长且无标点的文本超过计算预算时，只能用前后均唯一的连续词组建立递归分块边界，不得把两处修改之间的大段稳定文字一起框选。短中文块可用至少两个共同汉字补充配对证据，但前后不同词组中偶然出现的单个相同汉字不得抵消删除与新增。仅承担排版的行内标签和 `br` 不构成结构事实；文字与结构读取同一 semantic pair graph，视觉事实保持独立但不得越过已配对祖先边界。新增/删除结构本身不得顺带被判成视觉变化。稳定且自身未变化的大区域只作导航锚点，永不直接成为可见框。
   静态分析完成后，只允许对源码为空、身份唯一、两侧可配对且能由本轮变化脚本直接引用的图表宿主补漏；已有静态 marker 覆盖的宿主完全跳过。可信父级先完成两张静态审阅页，再向主进程提交只含冻结 HTML、source SHA、侧别、冻结视口和静态候选绑定的窄请求。owner 在解析原始冻结源码时先重验路径、标签、source-box 签名和身份属性；作者脚本运行后，隔离世界只重验路径、标签和身份属性。未声明属性没有事实权限，缺失、重复、被转移或替换、采样中身份漂移时，该候选静默 unavailable，不拖累其他候选。每个宿主独立获得遍历、节点、值、截图像素和字节预算；owner 每候选采两次事实、只捕获一次 PNG。两侧只比较宿主内可见文字、绘制样式、SVG 几何和 Canvas 像素，并纳入宿主自身真正绘制的背景、边框、阴影、滤镜、整体透明消失以及脚本直接改写的尺寸；每个采样节点都沿祖先链检查到宿主，SVG 矢量节点也不例外。宿主整体透明时只记录稳定消失状态，任一内部透明层的整棵不可见子树也不再取证，但原本可见的内部区域变透明仍会改变宿主最终结果。未绘制空宿主的纯尺寸变化、父容器响应式布局或其他内容挤压造成的间接尺寸变化，都不算图表主体变化。所有几何均相对宿主，页面绝对位置与仅由上方增删造成的整体下移不构成变化。动画、随机抖动、跨域 Canvas、超预算、单侧、超时或迟到结果均静默保留静态结论。多个命中宿主属于同一已有变化时复用其 `changeId`，原来未变化的区域最多新增一项“视觉调整”。
   运行态候选的 key、原始 source-box 基线和定位信息绝不以固定属性留在两侧审阅页，亦不以可被作者页读取的 bootstrap、端口或消息传递。候选绑定仅停留在可信父级和主进程 owner 的请求/隔离世界闭包内；作者读取 HTML、bootstrap、窗口消息或伪造 `MessageChannel` 都得不到 key、路径、基线、截图或运行态结果。路径失效后只允许唯一身份属性匹配；歧义、替换或断连使该候选保留静态 Diff。
...
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生同形透明孔。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文。
   初次进入审阅时，父级先等待 before/after 的静态 Review 都就绪并投影静态 footprint；随后才并行请求 owner 捕获两侧，运行态补充可以晚到但绝不阻塞静态审阅。每一轮 owner pair 的总 deadline 为 1.5 秒；创建、取证、验证或撤销会话失败均只使相关候选保留静态结果，截止后的结果不能追加框、提示或恢复操作。
   所有局部运行态候选都必须接受一次新的 before/after owner session 确认：source SHA、冻结视口、两次事实和每候选一次 PNG 的像素摘要均须与首轮同侧一致，才可框选；确认失败或任一侧不稳定只回退该候选，不影响其他合格候选或静态审阅。
...
进入审阅时同步冻结本轮已保存评论。评论只投影到“修改前”左页的原 TargetRef 位置，常驻一个 30px 的紫底白字“评”标记；鼠标 hover 时展开简洁的只读评论详情气泡，移出后气泡自动消失而“评”保留。标记继续按原评论 TargetRef 锚定，不固定在画布边缘。标记和气泡不提供编辑、回复、删除或其他点击交互，右页不重复显示。评论正文始终保留在可信的 React 宿主层：冻结分析器临时使用不含内容的不透明目标 key 判定局部运行态候选，并从当前 section 内每个非全局目标向上定位最近多图表祖先组，因此评论落在图表标题或说明时也能覆盖相邻图表；它不读取评论正文，也不把全局评论扩展成整页授权。scope 属性只保留到冻结分析结束，序列化前从两页移除；可信父级在 before frame 加载后，才通过独立、受随机 challenge 绑定的私有端口把不含正文的稳定定位描述交给其 bootstrap，定位描述绝不进入两页 HTML 或可抓取的 bootstrap 源码。该描述只允许源 TargetRef 的唯一 `id`、`data-*`、`name` 或 `aria-label`，不允许随同标签兄弟重排漂移的位置路径；不能安全定位或私有端口不可用时不显示该标记。仅修改前 bootstrap 在布局稳定点解析它并回传文档坐标与当时的视口坐标。因此原页 CSS/脚本无法观察到评论属性或定位描述，更不能据此制造结构或视觉变化。有限坐标在千万像素内均为合法长文档位置。滚动期间宿主按已有 `scrollTop` 平移标记层，不重新读取目标几何，原页脚本无法读取或转发评论内容。

实现补充：上述稳定定位描述在有冻结源投影时以内容无关的 `sourceNodeId` 为准，而不局限于 selector。准备 before 文档时，精确目标元素绝不带临时身份；分析器仅把 `sourceNodeId`、child-index 路径与窄静态指纹放入会话私有的首个 bootstrap 响应。桌面受管预览只将该响应交给解析器阻塞式的第一次请求，随即改为无绑定回退源码，私有端口随后才交付评论 key→`sourceNodeId` 映射。故 class-only 的普通目标即使邻近同标签元素被作者重排，只要路径失效后仍唯一匹配私有指纹，仍会显示在原元素上；私有绑定不可用时才回退到唯一 `id`、`data-*`、`name` 或 `aria-label`，绝不走位置路径。绑定缺失、歧义、替换、脱离文档或私有端口失效时不显示标记。作者加载后不能从 HTML 或后续 bootstrap 抓取获得评论正文、评论 key、`sourceNodeId` 或完整定位映射；运行态与评论端口的 challenge 都由首个受管 capture listener 校验并停止传播，因此作者 capture listener 既看不到 challenge，也不能伪造先到端口。
