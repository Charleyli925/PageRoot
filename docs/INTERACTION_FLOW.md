...
- 当前项目入口。
- 当前文件名、版本与保存状态，以及仅在桌面本地文件可用的“在文件夹中打开”快捷入口。
- 当前 HTML 画布。
- 直接编辑工具。
- 评论与本轮要求面板。
- 自动写回状态。
- “一键发送至 QoderWork”主按钮。
- 右上角“关于源页”入口；弹层说明当前版本、运行架构与更新频道，提供“检查更新”和 GitHub 仓库入口，并把检查中、已是最新、新版本下载进度或失败原因原位展示。
- 仅在正式签名的 macOS App 检测到 stable 新版本后显示、位于发送按钮上方的更新状态入口；应用在启动约 5 秒后及持续打开期间每 4 小时检查 stable 更新，新版本自动下载，完成后明确提示“重启并安装”，安装前仍执行同一套安全写入与关闭确认。
- 项目文件与交接详情。
- 版本历史。
