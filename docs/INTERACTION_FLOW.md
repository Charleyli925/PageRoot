...
- 顶部是浏览器式双层骨架：第一层是全局标签栏，第二层是编辑、预览与审阅共用的
  ...
  不复制 Request/Candidate/Version 状态。「+」创建独立开始标签。开始页无卡片外框，优先引导用户
  查看 Registry 已登记项目，其次才从 Finder 打开新的 HTML。
...
- 当前项目入口。
- 标签页承担当前 HTML 的名称与打开状态；第二层工具栏不再重复 HTML 图标、文件标题、打开“+”或其锚定浮层。
  “在文件夹中打开”和“在默认浏览器中打开”是与模式、项目、AI 同一行的图标动作，保存事实以紧凑状态显示。
  外部打开会先围栏刚送达的原生输入并等待该精确编辑 revision 安全写回，写回中保持禁用，失败时不启动浏览器。
  纯浏览器预览没有本地文件权限，因此仅展示禁用态的外部打开图标。Finder 显示源文件、默认浏览器打开、
  经验证项目根、可见 Version Working Copy 和派生 AI任务都是一击一次的本地副作用：失败显示可恢复错误
  但不锁住项目，也不会自行重放；用户再次点击才发起下一次调用。
...
- 第二层工具栏固定为一行：模式切换、审阅工具、项目/上轮处理、文件动作与 AI 入口按功能区排列，
  不因编辑、预览或审阅改变位置。审阅工具在编辑和预览中保留禁用态，进入审阅后原位启用；审阅画布内
  不再出现另一条浮动工具栏。“添加全局评论”仍只放在右侧评论标题行。HTML 图标和更新入口移入全局左侧栏，
  顶栏不再重复品牌块。
- 当前 HTML 画布。
- 直接编辑工具。
- 评论与本轮要求面板。
...
顶部“项目”按钮打开的是**当前项目工作台**：只呈现当前这一个项目的身份、状态、项目规则与版本历史，不再混入其他项目。打开或切换到别的项目由全局左侧栏、开始页或新标签页负责，形成“左侧栏 = 全局项目，项目 = 管理当前”的清晰分工。
...
全局左侧栏提供“打开 HTML”、当前项目、Registry 已登记项目与最近打开列表。它展开时占据产品左侧列，
收起时不保留窄轨道，也不在开始页或其他位置复制第二个开关；唯一开关随系统红黄绿按钮在标题带中切换位置。
...
当前项目工作台不再展示其他项目列表；切换到别的项目统一走全局左侧栏或新标签页。
...
全局左侧栏或开始页选中最近项、已登记项目或“打开 HTML”时，进入同一安全项目切换与导入确认流程。
...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；候选就绪后可按完整 operation/Hash/comment 身份后台预热，未命中缓存时也先挂载同一安全传输合同的前后页面壳，再渐进补齐变化、评论与运行时视觉事实。正式审阅页复用同一条工作台工具栏并移除所有 Demo 标记：工具栏中的“审阅”模式原位选中，全部审阅控件与“返回修改前 / 采纳修改”决定在同一行启用；编辑和预览模式下这些控件仍在原位但禁用。HTML 品牌与“关于源页”入口由全局左侧栏统一承担，审阅画布不复制同形入口，也不提供另一条浮动工具栏或收起手柄。采纳会把项目当前源晋升到新的工作文件路径；浮层仍可见时，两页预览继续持有准备审阅时冻结的源路径与文件标题，不因中途路径切换重建。完整 Session 权威同步发布后立即拆除浮层并显示新 HTML，最终编辑 Canvas 核验与项目资料刷新在后台继续；浮层拆除是采纳过程中唯一一次视觉切换，后续核验失败只锁住已提交页面并提供恢复，不回退为旧审阅源。
