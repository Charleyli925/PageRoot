...
本节用户可见行为是现行合同。内部执行仍是过渡态：Canvas 对同一次已接受编辑
先应用 SourcePatch，再应用语义内核，两边完整 HTML/Hash 必须一致才发布。
内核在同一次 apply 内复用与当前字节对应的已构建 SourceIndex，不因此跳过
Hash、范围、身份或解析完整性检查。后面的“先物化为完整 next HTML”描述产品结果，
不表示生产路径已经只执行一次修改。
