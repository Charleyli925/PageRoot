...
`POST /request` 最多等待 60 秒。若超时发生在请求已经发出之后，客户端不得直接解锁或再次提交，而是读取 `/workspace` 核对持久运行态。核对同样有 15 秒边界；失败时处理面板说明系统仍在自动核对，并每 4 秒继续读取权威状态，不提供重复的手动核对按钮。只有本地服务持续不可用时才提供“重新打开源页”；成功确认没有活动任务后自动恢复编辑。
