...
| Escape、结束编辑、切换目标 | 已成功的普通局部编辑到此结束，不补 Candidate | 只有之前真实失败留下的恢复任务才继续；过期 lease 不得切换 |
...
AI成功采纳按既有冻结Request完成评论、附件与edit event归档，完成后才清空已处理本轮草稿。失败、取消、冲突或no-change不能提前清空。显式本地保存版本只冻结HTML，不消费当前评论附件。
...
“更多 → 找回此前的稿件…”列出旧工作稿或此前当前稿，展示时间、基于Vn和“含评论”，本次不预览HTML。用户明确选择“恢复为当前稿”才创建下一份recovery-copy Version，恢复HTML、评论草稿及附件，并先保留当前稿；不回滚任何旧历史或项目长期规则。

有效旧v4迁移时保留实际活动Working Copy的HTML、评论、附件、ID、原路径与名称，其他旧稿进入此恢复入口。即使最新V6而活动稿基于V2，也不能自动切到V6。普通保存版本、历史创建、恢复都受活动Request/Candidate及身份/Hash围栏限制。
...
8. 成功V9保持同一当前稿身份/路径，快照与当前HTML为已确认字节；画布确认后才声称已打开。
9. 查看 V6 永远只读且打开精确路径。
10. 从历史V5继续工作必须明确创建下一Version并保护被替换稿，不能无版本地覆盖当前内容。
11. 外部冲突重启后仍停留在同一候选和同一 Hash 对比。
12. 每个事务边界崩溃后，不出现半提交状态。
13. 连续两次AI采纳后当前稿路径/身份不变，原始HTML、所有旧Version和被替换稿的恢复字节保持可验证。
14. AI no-change、失败和取消不建版、不替换当前稿。
15. 左侧先规则与当前稿，再默认折叠历史；全部历史行只读，长文件名不重复造成横向滚动。
...
23. V6项目从历史V2创建V7后当前稿仍是同一文件；V2/V6快照不变，当前稿与历史选中态独立。
24. 历史Finder动作明确定位当前工作文件，Candidate Finder只打开既有`AI任务/`派生展示；删除或篡改展示不影响隐藏Candidate，不增加可见附件管理。
