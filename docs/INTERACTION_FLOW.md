...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；候选就绪后可按完整 operation/Hash/comment 身份后台预热，未命中缓存时也先挂载同一安全传输合同的前后页面壳，再渐进补齐变化、评论与运行时视觉事实。正式审阅页复用正式工作台顶栏并移除所有 Demo 标记。顶栏左上角的 HTML 图标在审阅页与编辑页语义完全一致，都是“关于源页”入口；同形控件不得在两个页面里指向不同动作，因此审阅页不再提供“退回本轮处理页”入口，离开审阅只能通过“返回 AI 修改前”或“采纳 AI 修改”这两个明确决定。审阅浮层不得铺满整个工作台去盖住顶栏；收起态审阅工具条可以上探露出手柄，但必须裁切在内容行内，不得拦截顶栏“关于源页”等共享控件的点击。采纳会把项目当前源晋升到新的工作文件路径；完整 Session 权威同步发布后立即拆除浮层并显示新 HTML，最终编辑 Canvas 核验与项目资料刷新在后台继续。浮层拆除是采纳过程中唯一一次视觉切换，后续核验失败只锁住已提交页面并提供恢复，不回退为旧审阅源。
