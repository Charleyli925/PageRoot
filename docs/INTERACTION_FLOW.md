...
| 复制、删除、同父相邻移动 | 先通过 direct-structure-policy 的范围、身份、父级和落点检查，再由现有语义内核一次物化；副本或移动对象获得准确输出选择。只有当前 iframe 的原地计划已验证才接受 | 拒绝时源码、历史、revision、Candidate 和 DOM 均不变。源码已接受而投影失败时保留源码和历史，进入统一画布恢复，不把结果改回“编辑被拒绝” |
