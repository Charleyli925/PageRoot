...
当前文件名右侧始终保留独立的“+”按钮，点击后与文件菜单“打开本地 HTML”共用同一文件选择与项目切换流程。标题的小铅笔仍只在鼠标悬停或键盘聚焦时出现；两个入口的点击热区互不嵌套。用户在文件选择器取消时，当前项目与画布完全不变。若当前编辑、评论、附件或项目规则还没有安全收口，系统只在用户确认会切换项目的动作之后，才沿用项目切换边界完成保存或阻止切换。
