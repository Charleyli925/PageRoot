...
AI 候选通过校验并进入待打开状态后，底部只显示“审阅对比”和“直接打开”两个入口，
默认突出“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；正式审阅页
使用双页完整画布，支持变化筛选、内容地图、同步/独立滚动和缩放，并移除所有 Demo 标记。
“保留当前版本”先提醒用户确认后会回到本轮处理页，候选仍会保留并可随时再次进入审阅；
...
修改前后的完整页面；退出审阅或确认“保留当前版本”只返回本轮处理页，不清除候选。
