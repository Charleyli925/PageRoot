...
- 全部项目（Registry 全量登记项目，不受最近打开条数限制；按最近打开时间排序，从未打开过的项目按项目名接在其后；当前项目、`unavailable` 与 `invalid` 项目灰显不可点击并各自说明原因；可逐条清除某项目的最近打开时间，该项目仍是 Registry 成员、仍留在列表内）。
- 打开本地 HTML。
