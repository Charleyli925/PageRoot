...
变化数量使用统一的两级口径：工具栏与内容地图都以“变化区域”为单位（工具栏显示 `N 个变化区域`，内容地图分组显示 `N/M 个区域有变化`，抽屉标题显示 `M 个区域`），逐处导航的 `当前/总数` 同样按区域计数；画布上的标注徽章是区域内的明细事实，不参与这些数字。修改前页头保持中性标识，AI 修改后页头额外使用紫色顶边与极浅紫底，让横向滚动后仍能立刻分辨哪一侧是新版本。
