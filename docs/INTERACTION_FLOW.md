...
欢迎 HTML 只在文件不存在时写入；模板引用的品牌图同时以受管普通文件写入同级 `PageRoot-assets/`，桌面首次打开不得显示断图。用户对欢迎 HTML 的修改、AI 新版和项目记录在重启后继续保留，应用不得用内置模板覆盖。已有当前项目时仍优先恢复该项目，不额外创建或切换欢迎项目。纯浏览器预览没有桌面文件权限，继续使用只存在于内存中的欢迎内容。
