...
同一目标有多条评论时，标记沿用线上紫色悬浮胶囊并直接显示 `评N`。标签页控制本身被评论时，标记悬浮在标签右上角，不作为标签文字或源码内容嵌入页面。右侧评论栏只显示评论总数，不重复显示“当前标签页”；存在其他标签页评论时，顶部栏提供“其他标签页评论 N”入口。展开内容必须留在同一个顶部评论栏容器内，按标签分组并使用评论卡片未点击、未选中的中性样式；点击任一张卡片会切换到对应标签页并定位该条评论。
