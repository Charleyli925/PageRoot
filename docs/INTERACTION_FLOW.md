...
- 整个岛在一次会话内保持打开；工具栏内部的焦点迁移不会结束编辑。用户点击当前选区之外的页面内容或 App 区域时，系统提交当前 checkpoint，并一次性清除 `contenteditable`、选区和浮动工具栏。Escape、保存、导出、项目切换、关闭和发送同样是明确 checkpoint 边界。
...
- `页面预览`：`双页 / 左页 / 右页`。进入审阅默认保持“双页”；左页按钮以弱提示标明“修改前”，右页标明“修改后”，且不改变三个按钮的既有宽度。“左页”或“右页”只显示对应一页；必须点击明确的“双页”才能回到双页，不使用“再次点击当前单页”的隐藏手势。
- `变化审阅`：`全部变化 / 文案 / 结构 / 视觉`。进入审阅默认选中“全部变化”、开启同步滚动、把上下文可见度设为 18%，并定位第一处变化，而不是停在完整页面。选择任一变化类型都会恢复双页画布并自动聚焦该类第一处变化；某类没有结果时显示明确空态。

内容地图使用稳定的大区域作为导航锚点，但可见变化边界只能来自小粒度 marker，禁止直接给导航区域画框。新增文字不改变作者颜色、字重或装饰；同一文字变化中的相邻片段和多行 rect 合并成一个紫色虚线框。删除文字只保留红色删除线，不增加边框。结构与视觉边界只落在具体受影响节点；当前过滤类型下，变化父级内部未变化的子树也按上下文可见度虚化，不能因为父级命中就整块清晰。地图和画布提示只显示“新增内容”“删除内容”或“文案/结构/视觉变化”等简短文案。“上下文可见度”数值按 0–100 线性映射到 opacity，0 完全不可见，100 完全无遮挡。

内容地图按页面区域与原页面 Tab 分组，同时列出已修改和未修改区域；变化项使用克制的紫色边线与淡底色，未变化项降低对比度。面板从把手右侧展开并贴住画布右边缘；用户点击地图以外的页内画布、顶部栏或 App 区域时自动收起。点击地图项或上一处/下一处定位隐藏 Tab 内区域时，两侧先安全切换到对应页签，再各自定位。双页且同步滚动时，用户在任一侧触发的安全页内动作都镜像到另一侧，包括 Tab、折叠、业务按钮以及 input/select/textarea 的值与选中态；导航、提交、弹窗和下载仍被隔离层阻止。同步滚动的横向位置直接联动，纵向按相同内容区域及区内相对进度联动，不按两页原始像素高度硬对齐；程序化跟随端在本轮同步完成前不能反向成为主导端，页面顶端与底端必须精确钳制。仍可切换独立滚动和适应/100% 缩放。
