...
- `changeFilter`：`全部变化 / 文案 / 结构 / 视觉`，任一时刻恰好选中一个。筛选按钮只改变框选与虚化所读取的变化集合，不强制返回双页、不改变上下文可见度或缩放；三个类型按钮各带一个与画布标注同色的差异色点（文案为删除红与新增绿，结构与视觉同属单一靛紫家族，视觉另带琥珀「疑似」），色值与页内标注同源。切换筛选时，若当前定位目标在新筛选下已不再匹配，则一并把定位目标移到该筛选的第一处变化；当前目标仍匹配时保持用户所在位置不动。没有匹配变化时保留所选筛选、不移动定位并显示空态。
...
变化数量使用统一的两级口径：工具栏与内容地图都以“变化区域”为单位，且同一个筛选下的数字必须可互相对应：全部变化时工具栏显示 `N 个变化区域`，选中某一类型时显示 `N/M 个变化区域`（N 为当前筛选命中、M 为全部），并与逐处导航的 `当前/总数` 共用同一个 N；内容地图分组在全部变化时显示 `N/M 个区域有变化`，筛选时显示 `N/M 个区域含某类变化`，不得拿筛选后的 0 冒充“该分组没有任何变化”；抽屉标题显示 `M 个区域`。画布上的区域说明与页缘修订条是逐区域的画布标注，不参与这些数字。修改前页头保持中性标识，AI 修改后页头额外使用紫色顶边与极浅紫底，让横向滚动后仍能立刻分辨哪一侧是新版本。
...
5. 标注遵循「安静默认 → 聚焦放大」的分层模型，同一事实只在用户伸手时才提高声量。默认态下已确认变化不画范围框：压暗遮罩负责“哪里变了”；说明与修订条以变化的“连续区段”为单位——同一变化纵向相近的事实构成一段，一个横跨全文的变化在它真正触及的每一处各自成段：页面左缘每段一条可点击的靛紫修订条负责位置索引，每段只在其最上方框位显示一枚几个字的说明，不以“当前定位”为显示前提；导航与计数仍以变化区域为单位。说明直接读取 marker 自身的 operation，不得因为同一 section 另有相反方向的变化而改名：`insert` 统一写“新增内容”，`delete` 统一写“删除内容”，`replace` 的短语或行范围写“文本调整”，段落范围写“段落改写”；移动、结构和一般视觉分别写“位置调整”“结构调整”“视觉调整”，`layout` 明确写“换行调整”。一个区段含多种事实时，说明按阅读顺序合写两种以内（如“新增内容 · 视觉调整”），超过两种写“综合调整”。悬停某区段或其修订条时，预览该变化全部精确框（低强调细实线）；定位到该变化时框转为实线声明并附极淡紫底，修订条同步高亮，各段说明升级为该段的逐类事实计数（如“新增内容 ×3 · 视觉调整 ×3”，嵌套收并的事实数计入该计数）；点击修订条或说明即定位该变化。已确认框线与修订条统一使用单一靛紫强调家族（原结构蓝并入，变化种类由说明文字与筛选图例承载），边线只负责圈出范围，不得粗到与正文争夺注意力；疑似框保持琥珀色细虚线且不参与静默——它是“无法核实”的常驻提示，不是已确认事实。修改前的精确删除字符使用红色虚线删除线，修改后的精确新增字符在每个字下显示一个清晰的绿色实点，它们是常驻证据、不随框静默。字符证据不增加背景底色、不改作者颜色、字重、字号、行距或原有装饰，绿色实点不实现成下划线或 `text-emphasis`，并且任何字符证据都不得落在其范围框之外。删除与新增的可见标记样式以「字符证据标记（冻结合同）」为准。`全部变化`仍只会合并同一事实、同一 owner 的非文案重叠记录，或同一 `changeId` 内确实嵌套包含的非文案记录；文字范围框不与结构或视觉记录融合成 shaped 框。同一位置的相邻但不相包含的独立事实（例如 box-style 与 layout）保留各自的框，悬停与聚焦时各自显示；它们同属一个连续区段时共享那一枚区段说明。相邻且说明文字相同的区段，其说明锚点（区段顶缘）同列且落入一枚说明的判定范围内时，聚合成一个代表说明「{说明} ×N」，N 数的是区段：每处变化仍各自保留框、透明孔、修订条与字符证据，不同说明永不互相聚合，跨列或锚点相距较远的同类说明也不聚合。说明按屏幕尺寸反向缩放，区段归类与聚合的锚点判定范围随缩放同步放大，因此在「适应」比在「100%」聚合更积极；当前定位所在变化的各段始终保留自己的说明且不携带簇计数。说明数与「变化区域」计数是两个口径：聚合只影响重复说明的显示，不改变可导航的变化区域数。
