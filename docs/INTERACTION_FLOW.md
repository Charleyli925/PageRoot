...
- 项目身份、Registry、PROJECT.md、Version、Candidate 和历史 HTML 仍由对应事实与工作流持有。当前项目文件夹下固定置顶“长期规则”入口，真实文件名为 `PROJECT.md`，它不属于版本时间线；点击后在工作区打开唯一规则标签页，重复点击只聚焦。规则页支持基础 Markdown、700ms 自动保存和 `⌘S` 立即保存，保存状态只在编辑器原位安静显示。AI 任务开始时由 Request freeze 读取并冻结规则快照，任务期间编辑只影响下一次任务；恢复或切换 HTML 历史版本不回滚该文件。
