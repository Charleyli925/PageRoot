...
| Escape、结束编辑、切换目标 | 合并到最新完整 HTML 的一次 Candidate | 旧画面继续显示；过期 lease 不得切换 |
| 复制、删除、插入、重排 | 立即准备最新 Candidate | 不回滚已接受的源码，不把旧画面宣称为新版本 |
...
新源码仍保留的源码宿主，其 Canvas/SVG 表面必须恢复且尺寸非零后才能切换；
等待沿用既有 Runtime deadline 和用户编辑暂停规则。该检查只保护展示连续性，
不授予编辑权限，不推断任意脚本已全部执行，也不保存/迁移像素或脚本状态。
静态 Candidate 若会删掉这些可见图表，不晋升为空白页面；保留上一次可用预览
为只读，持续说明预览未完整更新、最新 HTML 未回滚，并提供重新加载和导出。
直接不支持运行时的首次打开仍可显示明确标识的静态页。
