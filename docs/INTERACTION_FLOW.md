...
桌面编辑画布加载同目录图片、字体和媒体时，复用一条短期
`pageroot-preview:` 资源会话作为文档基址。该会话只让已知源 HTML
真实路径所在目录内的资源通过 realpath 围栏进入禁脚本编辑 iframe；它不把
作者脚本带回编辑模式，也不改变源码字节。切换源文件或卸载画布时立即撤销
旧会话，旧资源根不得泄漏到新项目。
...
- 选框和浮动编辑工具条只在画布或工具条内部交互期间保持；点击工作台其他
  区域会同时清除选框与工具条。点击 iframe 内不可选择的留白也执行同一
  收口，不能留下已经失去操作上下文的悬浮控件。
...
用户打开“审阅修改”后，修改前与修改后 HTML 并排显示。审阅证据遵循以下
规则：

- 先用稳定语义身份、行列上下文和相邻结构配对目标，再比较文字；重复文案
  不能只凭第一次命中跨表格行或跨卡片配对。无法配对的一侧按真实新增或
  删除保留，不把同文案的其他节点划线。
- 内容地图只统计当前两份渲染文档中实际产生文字、结构或样式标记的区域。
  未检测到证据的条目标记为“未检测到变化”、不可进入，上一处/下一处也会
  跳过；加载和测量完成前显示核对状态，不能先用静态配置宣称发生变化。
- 差异框由该目标全部实际子证据的几何并集生成。字体完成、图片加载或失败、
  区域尺寸变化后都重新测量，因此框必须始终包住当前变化，不能沿用旧坐标。
- 修改前页面在历史评论目标旁只显示紧凑的 `评` 标记。点击后显示一张简约
  的只读评论气泡；同一时间只打开一张，点击标记与气泡之外或按 `Escape`
  后收起。评论浮层只属于审阅呈现，不参与差异配对和源码修改。
...
22. 重复单元格文案只与同一语义行列的对应项配对；一行真实删除不会把另一
    行未变文案划线。
23. 内容地图中没有任何实测差异标记的区域显示为未变化且不可导航；异步
    字体、图片或布局变化后，所有差异框仍完整包含各自证据。
24. 审阅修改前页面的历史评论默认只显示 `评`；点击外部或按 `Escape` 收起
    气泡，且标记不增加差异数量。
25. 点击编辑画布外部同时清除选择框和工具条；桌面编辑状态的同目录图片在
    禁脚本 iframe 中可见，且不能越出已知源 HTML 的资源根。
