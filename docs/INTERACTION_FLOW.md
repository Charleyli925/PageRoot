...
- 顶部是浏览器式双层骨架：第一层是全局标签栏，第二层是编辑、预览与审阅共用的
  ...
  查看 Registry 已导入项目，其次才从 Finder 打开新的 HTML。
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  从活动开始页打开 Finder、最近或已导入项目时，该开始标签原位变成目标文档；只有用户明确点击「+」
...
全局左侧栏只负责导航，提供“打开 HTML”、当前项目与 Registry 已导入项目。它展开时占据产品左侧列，
收起时不保留窄轨道，也不在开始页或其他位置复制第二个开关；唯一开关随系统红黄绿按钮在标题带中切换位置。
“最近打开”只保留在开始页，不在左栏重复。当前项目默认展开；当前项目与已导入项目使用同一棵紧凑的缩进继承树，
全局一次只展开一棵项目树。展开已导入项目只读取只读版本摘要、自动收起当前项目树且不切换当前项目；只有点击具体 HTML 文件才进入既有安全导航流程。
...
版本行的主标题始终是 Bridge 返回的真实 HTML 文件名，不使用用户评论、评论摘要或独立的 `V1/V2` 标题列。版本摘要只携带项目/文档/版本 ID、序号、父版本 ID、真实展示文件名、安全写入时间与当前/最新正式状态；展开左栏不读取 HTML 正文、评论或附件。版本按父版本后紧跟完整子树的顺序展示，连续继承使用垂直线，分支使用不交叉的短横线与直角转折。文件名保持单行省略，确实溢出时才在悬停或键盘聚焦后延迟滚动；提示层只说明初始导入或基于哪个父文件生成，并通过 `aria-describedby` 提供完整身份与继承关系。
...
全局左侧栏或开始页选中最近项、已导入项目或“打开 HTML”时，进入同一安全项目切换与导入确认流程。
...
unavailable` 状态。打开 About 或侧栏立即进入“检测中”并执行当前选中 Agent 的真实
preflight；窗口从外部终端返回时，只要状态尚未 `ready`，`focus` 与 `visibilitychange`
会合并为一次自动复检。检测、复制登录指令和打开 About 不运行 Request、不冻结 HTML、不
...
时显示对应 Agent 的设置入口；点击只打开 About 的 AI Agent 区域并把安装或登录动作聚焦，不调用发送、
不清空对话输入、不关闭侧栏。About 对每个可安装或需登录恢复的 provider 渲染通用
...
对话侧栏不逐条暴露上述内部检查，生命周期仍固定合并成四个用户阶段：
第一阶段按模式显示“启动 Agent”或“正在准备并复制”，其后为“等待 AI 完成”
“校验并保存”“结果”。每轮只允许一个当前阶段；启动或复制失败落在第一阶段，
完成记录缺失落在第二阶段，校验失败或外部冲突落在第三阶段，
成功、无变化和最终选择落在第四阶段。界面只展示当前阶段的一行 PageRoot 状态，不把四阶段
清单同时铺开；完成记录出现前不得把第二阶段显示为完成。
...
PageRoot 当前阶段使用品牌头像和名字签名，并以简短文案说明正在准备、交接、处理、校验或等待决定。
交接摘要必须给出当前文件名以及发送给 Agent 的评论、当前 HTML 与项目规则；不能使用“新版本已准备好”
这类在 Candidate 尚未成立时有歧义的文案。底部操作区不显示冗长阻断说明；Agent 选择器位于左侧，
仅显示 Logo 与名字，当前可用动作位于右侧并与选择器垂直居中。
...
点击文件名旁的“+”或选择最近项目时，系统先只读分类选中的 HTML。已导入项目随后才完成当前编辑 drain，再核对当前
