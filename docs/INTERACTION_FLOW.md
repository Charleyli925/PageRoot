...
本文描述当前产品流程。每个项目只有一个当前稿，全部 Vn 为不可变历史；普通保存、显式保存版本、AI 采纳、历史创建与恢复按 [版本与项目文件产品需求](VERSION_AND_PROJECT_FILES_PRD.md) 区分。等待固定时长不能判定 AI 完成，单项目处理也不锁住其他项目。
...
3. AI 真正完成且产生有效变化后，系统先保留独立 Candidate 并让我审阅；明确采纳才创建下一份不可变版本并更新同一当前稿，替换前稿件先安全保留。
4. 普通编辑自动保存；“保存为新版本”主动留下本地历史。修改前稿件、AI冻结输入和所有旧历史都可保留，同一当前稿继续编辑。
...
Registry 决定项目列表成员，Recent 不授予文件权限。Finder/浏览器动作打开明确的当前工作文件，Candidate 文件夹入口只打开已验证 `AI任务/` 派生目录。项目归档、搜索、克隆、评论审阅包、备份包和资源/可见附件管理不在本次范围。
...
- 左侧栏是全局项目上下文，固定分为标题栏、品牌区、可滚动项目列表和底部固定操作区。项目列表只有一个“项目”分组：当前项目与 Registry 项目按真实 `projectId` 去重后混合展示，并按权威内容更新时间倒序排列；打开项目不会改变排序。每个项目独立维护展开状态，多个项目可以同时展开，展开或收起不会切换当前项目，也不会读取 HTML 正文、评论或附件。每个展开项目依次提供长期规则、当前稿和默认折叠的历史版本；三类入口通过既有安全导航打开。品牌区打开 About，底部齿轮在标签栏中打开设置。
...
- 历史列表按 V1、V2、V3……升序，只显示简短版本标签和生成时间，继承来源留在需要时查看的详情中。全部历史行，包括最新版本，都打开不可变只读快照；不重复完整文件名、当前徽标或分支连线。独立“当前稿”行始终打开实际当前内容，哪怕与最新历史字节相同。规则、当前稿与历史分别选中，不能由基础版本或 Working Copy ID 相等推断历史选中。历史查看不替换当前 DocumentSession，返回当前仅结束历史展示并核对源变化，失败保留原视图及返回出口。
...
4. 导入默认复制，不移动、覆盖或改写用户选择的原始 HTML 字节。只有用户勾选「成功导入后，同意将原文件移至废纸篓。」并且新项目与新画布都已确认，才把该 HTML 移入废纸篓。有效旧 v4 多工作稿按专项 PRD 迁移并保留实际活动稿；v4 以前状态不迁移、不恢复，也不作为读取回退。
...
用户在 Finder 中于同一登记项目根内改名当前 HTML 或同父目录的项目文件夹时，目录事件只作为检查提示。当前 HTML 仍在原处时只核对内容；路径缺失时才按唯一稳定身份、登记成员关系、安全路径和内容证据重绑。跨重启的 device/inode/birthtime 不作为身份门槛；Hash 相同不能认领另一文件。HTML 改名不改变项目名，也不创建 Version；项目目录改名才同步项目显示名。本次不增加顶栏改名或项目管理菜单。

成功扫描配置项目根后确认非活动项目目录不存在，才从普通列表隐藏，保留 Registry 和恢复证据。扫描失败、权限、损坏和重复身份继续显示不可用事实。当前打开项目缺失时保留内存、恢复与导出出口，所有写入暂停，任何后台动作都不得重建旧根；目录返回后重新验证再接续。
...
直接入口只承认独立的 `p`、`h1`—`h6`、简单 `li` 和直接承载行内内容的
`blockquote`，其后代只能是受支持的行内格式；嵌套块/列表、控件、媒体、表格、图表、
SVG/Math、脚本/资源、自定义元素、注释、作者 ID 或需要引用重写的内容都会拒绝。
运行时生成内容、不透明运行表面或程序的容器不显示复制入口，命令层也统一拒绝；
...
Runtime 结构操作在源码身份、当前节点和局部更新均可证明时原地完成；
新的直接结构命令若无法证明原地路径或计划会进入 Candidate，则在宿主接受前拒绝，
不为复杂操作启动重建。带 `is` 的内建自定义元素、无法证明文本/注释插入边界的
混合内容父级，以及 `html`/`head` 作为目标父级，都不纳入直接入口。
`body` 自身不可删除或移动，但可以作为普通源码子元素的合法父级。
作者 Script 程序身份变化仍立即准备 Candidate。
同父重排仅在局部同步不可靠时重建。
...
| 复制、删除、同父相邻移动 | 先通过 direct-structure-policy 的范围、身份、父级和落点检查，再由现有语义内核一次物化；删除只要求源码删除/逆向边界可证明，删除后没有合法落点时清空选择；副本或移动对象获得准确输出选择。只有当前 iframe 的原地计划已验证才接受 | 拒绝时源码、历史、revision、Candidate 和 DOM 均不变。源码已接受而投影失败时保留源码和历史，进入统一画布恢复，不把结果改回“编辑被拒绝” |
| 跨父移动、任意 HTML 插入、复杂结构直接操作 | Canvas 和命令端口不提供这些直接能力；共享源码原语只供历史、恢复和受控内部消费者 | 显式请求只产生一次局部轻提示，不结束 Native Edit，不写入源码或历史 |
...
- 页面应尽量保持当前 Document、共享滚动位置和 Selection。结构修改在源码身份、
  当前节点身份和局部更新结果均可验证时原地完成；无法证明，或必须由
  Script 重新计算的修改，仍在语义操作/checkpoint 边界自动重建。新的直接结构
  命令若计划结果为 Candidate，必须在宿主接受前拒绝；历史回放、权威替换和已接受
  后的投影失败仍沿用原有恢复路径。无法证明文本/注释边界的混合内容插入与移动、
  以及带 `is` 的内建自定义元素，不得报告原地成功。`body` 可以作为普通子元素的目标父级，
  但其自身仍不可作为删除或移动对象。局部同步失败、源码目标失效或已经存在的必要 Candidate 被新源码过期时，仍只能请求
...
保存事实不再由画布上方或顶栏的状态条展示。源文件始终是项目唯一当前稿；首次确认导入后复制到项目内 base.html，AI Candidate 先保持独立，明确采纳后更新同一当前稿。用户只在发生冲突或失败时从全局提示执行恢复动作。
...
保留原 ID，删除后的 ID 不主动复用。共享语义原语仍由历史、恢复和其他
受控内部消费者按父 ID 和 before ID 使用原始 HTML 插入或跨父移动能力；
Canvas 直接入口和测试命令端口不提供这些能力。本阶段不增加元素面板、组件系统、
...
“更多 → 保存为新版本”是独立动作：先收口并保存当前完整 HTML，再验证身份、Hash 和项目空闲条件，保存下一不可变 Version并继续编辑同一当前稿。当前评论附件保留。无内容变化时提示“当前内容已保存在历史版本中”，不增加编号；未知提交结果只查询/重试同一操作。

“导出当前 HTML…”原样复制收口后的完整当前源码，包括 Stable ID；“导出此版本…”复制所见不可变历史。两者不清洗源码、不序列化 Runtime，不导出评论、附件或项目记录。

当前稿导出旁有“同时保存为新版本”，每次打开更多菜单都默认未勾选，切换复选项不关闭菜单。默认导出不建版；勾选后先成功导出，再用该次相同字节/Hash建版。无变化不重复建版，取消或文件写入失败不建版；导出成功但建版失败时保留“文件已导出”事实，只查询/重试建版。历史不提供勾选项。

首次导出默认到 Downloads，随后使用最近一次成功外部目录；取消/失败不更新。项目目录、其他项目、隐藏记录及其路径/硬链接别名禁止覆盖；选中受保护目的地就失败，不另选位置。成功结果提供“显示文件”。当前打开文件、项目、Recent和评论不因导出改变。
...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；候选就绪后可按完整 operation/Hash/comment 身份后台预热，未命中缓存时也先挂载同一安全传输合同的前后页面壳，再渐进补齐静态变化与评论事实。正式审阅页复用同一条工作台工具栏并移除所有 Demo 标记：工具栏中的“审阅”模式原位选中，只承载审阅视图与导航控件。所有“返回修改前 / 采纳修改”决定固定由 AI 会话侧栏底部持有；侧栏收起时顶栏 AI 入口显示“待决定”，但不复制决定按钮。HTML 品牌与“关于源页”入口由全局左侧栏统一承担，审阅画布不复制同形入口，也不提供另一条浮动工具栏或收起手柄。采纳创建下一份不可变历史并更新同一当前稿，身份和路径不变；浮层仍可见时，两页预览继续持有准备审阅时冻结的HTML与文件标题，不因当前内容更新而重建。完整 Session 权威同步发布后立即拆除浮层并显示新 HTML，最终编辑 Canvas 核验与项目资料刷新在后台继续；浮层拆除是采纳过程中唯一一次视觉切换，后续核验失败只锁住已提交页面并提供恢复，不回退为旧审阅源。
AI 候选通过检查并进入待打开状态后，正常连续性候选显示“审阅对比”和“直接打开”两个入口，默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；候选就绪后可按完整 operation/Hash/comment 身份后台预热，未命中缓存时也先挂载同一安全传输合同的前后页面壳，再渐进补齐静态变化与评论事实。合法候选没有可定位事实时也能查看和决定：导航保持总览、focus 为空，不启动首处定位，不制造标记；工具栏原位说明“未定位到可标注的变化，可直接查看前后页面。”，只有核验后的 HTML Hash 相同才显示“前后 HTML 内容相同。”。两者均保留前后页面、页面动作与既有采用／不用本次流程。编辑与预览不显示禁用的审阅子控件；主工具栏高度、模式选择、AI 入口与画布位置保持稳定。审阅内没有变化事实时隐藏无作用的变化筛选组，保留页面预览、滚动、缩放和原位说明。正式审阅页复用同一条工作台工具栏并移除所有 Demo 标记：工具栏中的“审阅”模式原位选中，只承载审阅视图与导航控件。所有“返回修改前 / 采纳修改”决定固定由 AI 会话侧栏底部持有；侧栏收起时顶栏 AI 入口显示“待决定”，但不复制决定按钮。HTML 品牌与“关于源页”入口由全局左侧栏统一承担，审阅画布不复制同形入口，也不提供另一条浮动工具栏或收起手柄。采纳创建下一份不可变历史并更新同一当前稿，身份和路径不变；浮层仍可见时，两页预览继续持有准备审阅时冻结的 HTML 与文件标题，不因当前内容更新而重建。完整 Session 权威同步发布后立即拆除浮层并显示新 HTML，最终编辑 Canvas 核验与项目资料刷新在后台继续；浮层拆除是采纳过程中唯一一次视觉切换，后续核验失败只锁住已提交页面并提供恢复，不回退为旧审阅源。
...
历史 v4 中仍在 processing 的 Request，即使 completion 的原状态是 `no-change`，也在全部校验通过后进入普通 Candidate；completion 原字节不重写。已终态的历史 `no-change` 保持原语义：不建版、不复活活动 Request、保留评论和诊断，沿已封存的 `lastAiTask` 展示“上轮处理”。其 submission receipt 与 outbox 继续按原稳定事件 ID 重放，不重新执行，也不迁移历史记录。该终态不创建下一正式 Version、不替换当前稿，释放候选号并解锁回到 editing。
...
Candidate校验成功只进入审阅，不提前创建正式Version。用户明确采纳后：

```text
重新验证项目、Request/Candidate和已审阅Hash
→ 核对当前源Hash及活动状态
→ 保留被替换前的当前HTML、评论草稿与附件
→ 创建下一份不可变Version
→ 更新同一当前稿及基础版本，保持Working Copy身份和路径
→ 发布已提交事实，完成当前HTML打开/画布确认
```

提交、打开与显示成功是不同状态。任何未知提交结果都查询同一操作；已创建但打开失败只重试打开，不重复建版，也不谎称版本未创建。外部替换或清理期间新增内容必须保留，不能删除唯一恢复字节。

### 9.2 成功与AI前本地编辑

使用连续整数标签，例如“V9”，不使用V1.8。成功后仍编辑同一当前稿。V3后的本地修改会进入冻结AI输入与替换前保留稿；采纳成为V4时，V3快照不变，不偷偷插入一个额外Version。
...
AI 成功采纳按既有冻结 Request 完成评论、附件与 edit event 归档，完成后才清空已处理本轮草稿。显式本地保存版本只冻结 HTML，不消费当前评论附件。任何失败、取消、冲突、no-change 或重启都不能提前清空。
...
5. 未再变化时先保护被替换稿，再更新同一当前稿并继续候选V9的事务；外部修改字节仍有可验证恢复副本。
...
每个项目下依次是“长期规则”“当前稿”“历史版本”。历史默认折叠，展开后按 V1、V2、V3……升序显示简短标签和生成时间；来源信息按需查看，不在每行重复文件名、当前/最新标记和分支连线。展开不导航，多个项目可同时展开，键盘和无障碍名称必须可用。

历史只列正式提交的版本，来源可为初始导入、本地保存、AI采纳、历史创建或保留稿件恢复。临时恢复记录与未采纳Candidate不混入列表。历史评估只用于解释，不拥有当前稿编辑权限。

### 11.2 查看历史

点击任何 Vn，包括最新版本，都设置精确历史身份，读取对应不可变快照并核对Hash。历史使用独立只读预览，不替换当前稿、评论或当前文档会话。

返回横条说明所见历史并提供“回到当前稿”；沿用自动收起行为，键盘聚焦时保持展开。缺失快照或Hash错误不能回退显示当前HTML、另一Version或旧缓存。动态传输失败可提供受限静态显示与刷新，返回出口保留。

历史“导出此版本…”复制所见快照。更多菜单的“在 Finder 中显示当前工作文件”“在浏览器中打开当前工作文件”明确指向当前稿；隐藏快照不作为可编辑用户文件发布。

### 11.3 回到当前稿

“回到当前稿”或独立当前稿行打开项目真实当前HTML，先核对当前身份、源状态和Hash，再按既有会话/画布边界接续。即使当前稿和最新历史完全相同，也不能把二者合并为同一个导航目标；不可用时保留真实恢复状态，不用历史快照冒充当前稿。

### 11.4 从历史创建新版本

历史视图的工具栏“编辑”先确认“基于Vn创建新版本”。取消保持历史与当前稿不变。确认时验证项目空闲、所选Version/快照Hash、当前稿Hash及稳定操作身份；先保护被替换前稿件，再建立下一Version并更新同一当前稿。

最新V6从V2创建得到V7，V7基于V2且前序为V6；V2和V6不变，后续采纳AI得到V8。不能激活一份旧V2 Working Copy，也不能覆盖V2快照。

未知创建结果只查询同一操作；已创建而打开失败提供“打开已创建版本”，只重试打开。跨项目/重启保留结果，更晚版本已接替时旧回执不能夺回当前稿。
...
“更多 → 找回此前的稿件…”列出旧工作稿或此前当前稿，展示时间、基于 Vn 和“含评论”，本次不预览 HTML。用户明确选择“恢复为当前稿”才创建下一份 `recovery-copy` Version，恢复 HTML、评论草稿及附件，并先保留当前稿；不回滚任何旧历史或项目长期规则。

有效旧 v4 迁移时保留实际活动 Working Copy 的 HTML、评论、附件、ID、原路径与名称，其他旧稿进入此恢复入口。即使最新 V6 而活动稿基于 V2，也不能自动切到 V6。普通保存版本、历史创建、恢复都受活动 Request/Candidate 及身份/Hash 围栏限制。
...
| 当前历史查看 | 只读允许 | 经校验的只读历史允许 | 按实际可读状态 | 按该项目状态 |
| 保存新版本/历史创建/恢复稿件 | 身份与Hash验证后允许 | 禁止 | 禁止 | 按该项目状态 |
| 当前项目规则修改 | 允许 | 禁止 | 禁止 | 按该项目状态 |
| 当前导出 | 允许，可选建版 | 已验证HTML可导出，建版禁用 | 已保护HTML可导出，建版禁用 | 按该项目状态 |
...
7. 新的合法同内容输出进入普通 Candidate，明确采用才消耗正式版本号；已终态历史 no-change 不复活，不消耗候选号，评论保留。
8. 成功 V9 保持同一当前稿身份/路径；新历史快照、当前 HTML 与画布 Hash 均为已确认字节，且只有在画布确认后才声称已打开。
9. 查看 V6 永远只读且打开精确路径。
10. 从历史V5继续工作必须明确创建下一Version并保护被替换稿，不能无版本地覆盖当前内容。
11. 外部冲突重启后仍停留在同一候选和同一 Hash 对比。
12. 每个事务边界崩溃后，不出现半提交状态。
13. 连续两次 AI 采纳后当前稿路径/身份保持稳定，原始 HTML、所有旧 Version、第一份工作文件和被替换稿的恢复字节保持可验证；不再生成第二个可编辑工作文件。
14. AI no-change、失败、取消和“不用本次”均不建版、不替换当前稿；新的同内容 Candidate 经明确采用后沿普通 Promotion 创建。
15. 左侧先规则与当前稿，再默认折叠历史；全部历史行只读，长文件名不重复造成横向滚动；点击具体版本仍定位精确历史快照或明确的当前工作文件。
...
23. 旧回执已激活 V2、项目最新为 V6 或从历史 V2 创建 V7 时，当前稿仍保持同一身份/路径；V2/V6 快照不变，左侧保留“基于 V2”“项目最新 V6”“当前编辑基础/有本地修改”事实，历史选中态独立。
24. Version Finder/历史 Finder 动作明确定位可见当前工作文件，Candidate Finder 只打开 `AI任务/` 派生展示；删除或篡改展示不影响隐藏 Candidate，不增加可见附件管理。
...
- 独立保存新版本保留评论附件，无变化不增号；普通自动保存和⌘S不建版。
- 导出默认不建版，复选项每次打开菜单为关；勾选后以本次导出相同字节建版，取消和导出失败不建版。
- 导出成功而建版失败保留成功文件事实，恢复入口只重试同一建版操作。
- 找回稿件恢复HTML、评论附件并创建下一Version，保护被替换稿；不提供HTML预览，不改变旧Vn。
- 旧v4迁移保留实际活动较早稿件的路径、名称和内容；非活动项目明确缺席才隐藏，故障不可伪装删除，旧根不自动重建。
