...
1. 分析器先建立高置信度节点配对，再从实际结果生成类型，禁止仅凭“同标签 + 同位置”把两个区域配成一对。文案先按混合行内节点中的语义文本流计算叶子级精确增删证据，直接文本与嵌套的 `span/strong` 不得断开；事实层明确区分 `insert/delete/replace/layout`，并把每侧的 `evidenceRanges`、`footprintGroups` 与 `anchorOffset` 分开保存。证据负责说明哪些字确实变化，只有 footprint groups 能产生文字框。纯新增只在修改后页显示新增 footprint，修改前页只保留不可见定位锚点；纯删除只在修改前页显示删除 footprint，修改后页只保留不可见定位锚点。定位锚点不得进入框选、遮罩透明孔、标签或变化类型统计。只有前后均有证据的成对 replace 才可能提升为 block；整句单侧变化使用 sentence scope，稳定前后句永远不被密度阈值带入 block。正文相同而 authored `br` 偏移变化时使用 layout operation，并归入既有视觉变化，不生成红绿文字 evidence。文本配对先建立语义层级：表格以 `tr` 为行单元并用逻辑列边界约束 cell，列表以 `li` 为项单元，连续 `br` 文本在每个非空行都具有编号标记时以编号行为单元，普通 `p/标题/直接文字块` 才作为文字单元；直接的无文字媒体、图形、画布和表单控件也是独立原子单元，不能被丢入空文字流。不同父级、单元类型或 HTML 标签不得互配。每组已配对父级内的兄弟单元按 DOM 顺序做有界单调配对，重复类目、重复短句、低置信或多解候选保持 unmatched；超出矩阵预算时只允许有限前瞻的单调退化，不得恢复为全局贪心。未配对的编号行只框该行，未配对的 `li/tr` 以自身为结构 owner，后代不再与其他父级配对。范围计算先按句末、换行和有意义的标点建立语义边界，相同标点前后的独立改动不跨界合并；句内只在共享词或连续短语足以构成稳定锚点时继续细化。超长且无标点的文本超过计算预算时，只能用前后均唯一的连续词组建立递归分块边界，不得把两处修改之间的大段稳定文字一起框选。短中文块可用至少两个共同汉字补充配对证据，但前后不同词组中偶然出现的单个相同汉字不得抵消删除与新增。仅承担排版的行内标签和 `br` 不构成结构事实；文字与结构读取同一 semantic pair graph，视觉事实保持独立但不得越过已配对祖先边界。新增/删除结构本身不得顺带被判成视觉变化。稳定且自身未变化的大区域只作导航锚点，永不直接成为可见框。
...
3. 精确文案证据必须再经过可读范围规划。文字 marker 一律用 `Range.selectNodeContents(marker).getClientRects()` 测量；归组身份同时包含 `changeId + semanticOwnerId + geometryOwnerId + footprintGroup + visualLine`。相隔不超过两个可见字符，或相隔不超过六个字符且合并后变化密度足够高的片段可归入同一短语；句末、分号、换行、完整稳定句或明显稳定间隔必须拆开。单行短语生成一个普通圆角矩形，过窄的单字/标点框有限扩展到所属文字行内的可读宽度，并由当前 geometry owner 的内容边界裁剪；局部变化跨行时每个实际 `Range` 行盒使用独立、干净的矩形并共用一个说明，不再把碎片 union 成锯齿多边形。长度至少 20 个可见字符的成对改写，在综合变化密度达到 45%，或至少五个碎片、密度达到 28% 且变化跨度覆盖文本块 65% 时，才由事实规划提升为最小语义文本块矩形；渲染层不得再按行数或密度二次升级。`insert/delete` 即使跨越更多行也仍逐行显示，稳定前后句永远不被包含进 block。该提升只能停在对应的 `p/li/td/标题/纯 inline 直接文字 div`；多列、grid/flex、跨 cell、大空白或离散视觉行回退为逐行框，不能扩大到外层 card、section、article、form 或 table。不同 semantic owner 永不融合；非文案记录也只有同 owner、同事实且确实相接或重叠时才能融合。`全部变化`中，新增/删除的 `li/tr` 结构 owner 支配同 semantic owner 的文字子框；文案筛选仍只显示内部文字，结构筛选只显示父元素。
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生同形透明孔。遮罩以当前会话、页面侧别和 presentation epoch 唯一的 SVG luminance mask 表示：整页白底保留虚化，每个 canonical path 以黑色挖孔；重叠孔天然取集合并集，绝不能用 `evenodd` 让重叠区重新变暗。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文；作者页的 `svg path`、`mask rect` 或通用 `path/rect` 样式也不得重写受管遮罩原语。
...
5. 每一个语义变化组在框边只显示一个几个字的说明，不以“当前定位”为显示前提；同组的后续行框不重复标签。说明直接读取该 marker 自身的 operation，不得因为同一 section 另有相反方向的变化而改名：`insert` 统一写“新增内容”，`delete` 统一写“删除内容”，`replace` 的局部成对修改写“文本调整”，跨行局部改写写“句子改写”，密集文本块写“段落改写”；移动、结构和一般视觉分别写“位置调整”“结构调整”“视觉调整”，`layout` 明确写“换行调整”。修改前继续使用红色虚线框和红色虚线删除线；修改后继续使用绿色虚线框，不增加下划线或背景底色，并保留作者颜色、字重和装饰。`全部变化`仍保留单类型的语义色，但只会合并同一事实、同一 owner 的重叠记录；同一位置的独立事实（例如 box-style 与 layout）保留各自的框和说明，不用紫色融合框伪造为一个变化。
