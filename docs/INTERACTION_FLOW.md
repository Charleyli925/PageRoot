...
1. 用户停留在“编辑”时，桌面端会自动为源码中为空、但被页面脚本填充的
   图表宿主生成一次只读 PNG 投影，不要求先进入预览。唯一支持范围是直接
   source Canvas/SVG，或具有唯一稳定 `id`/`name`/`aria-label`/`data-*`/class
   绑定的 source-empty 宿主；不为 ECharts 等库写适配器，也不捕获任意 HTML、
   `tbody` 或运行时猜测节点。协议传输有界 PNG 二进制并校验内容摘要、像素
   尺寸和字节预算。新图在离屏解码后才替换旧图，捕获失败只撤销临时投影。
...
运行时图表投影以支持范围内的宿主源码、`base`/`link`/`script`/`style` 源码、
document 和 64px 视口 bucket 作为粗粒度刷新依赖。普通文字输入、撤销/重做
或不改变这些输入的源码位移只把已验证位图重新绑定到当前 `SourceIndex`，不
重新运行截图；编辑/预览和 Tab/模式切换也复用有界最近结果。输入变化时旧图
保持可见，新的 Blob PNG 离屏解码成功后才原位替换；没有任何新增提示或操作
入口。稳定空宿主使用 pointer-transparent 图像，直接 Canvas/SVG 使用可逆背景，
所以点击、选择和评论始终落在原 HTML 宿主。
...
运行态 Snapshot 补充只在桌面 Electron 的受管 `RuntimeSnapshotOwner` 中启用。两张审阅 iframe 只负责静态审阅，候选作者页永远不能获得取证通道、候选绑定或截图；内联/srcDoc 或纯浏览器审阅只使用静态结果。owner 为每个请求创建新的非持久 `pageroot-preview` 会话和隐藏窗口，拒绝导航、弹窗、下载、webview、权限和非预览请求，完成或失败后立即销毁并撤销该会话。
...
1. 分析器先建立高置信度节点配对，再从实际结果生成类型，禁止仅凭“同标签 + 同位置”把两个区域配成一对。文案先按混合行内节点中的语义文本流计算叶子级精确增删证据，直接文本与嵌套的 `span/strong` 不得断开；事实层明确区分 `insert/delete/replace/layout`，并把每侧的 `evidenceRanges`、`footprintGroups` 与 `anchorOffset` 分开保存。证据负责说明哪些字确实变化，只有 footprint groups 能产生文字框。纯新增只在修改后页显示新增 footprint，修改前页只保留不可见定位锚点；纯删除只在修改前页显示删除 footprint，修改后页只保留不可见定位锚点。定位锚点不得进入框选、遮罩透明孔、标签或变化类型统计。只有前后均有证据的成对 replace 才可能提升为 block；整句单侧变化使用 sentence scope，稳定前后句永远不被密度阈值带入 block。正文相同而 authored `br` 偏移变化时使用 layout operation，并归入既有视觉变化，不生成红绿文字 evidence。文本配对先建立语义层级：表格以 `tr` 为行单元并用逻辑列边界约束 cell，列表以 `li` 为项单元，连续 `br` 文本在每个非空行都具有编号标记时以编号行为单元，普通 `p/标题/直接文字块` 才作为文字单元；直接的无文字媒体、图形、画布和表单控件也是独立原子单元，不能被丢入空文字流。不同父级、单元类型或 HTML 标签不得互配。每组已配对父级内的兄弟单元按 DOM 顺序做有界单调配对，重复类目、重复短句、低置信或多解候选保持 unmatched；超出矩阵预算时只允许有限前瞻的单调退化，不得恢复为全局贪心。未配对的编号行只框该行，未配对的 `li/tr` 以自身为结构 owner，后代不再与其他父级配对。范围计算先按句末、换行和有意义的标点建立语义边界，相同标点前后的独立改动不跨界合并；句内只在共享词或连续短语足以构成稳定锚点时继续细化。超长且无标点的文本超过计算预算时，只能用前后均唯一的连续词组建立递归分块边界，不得把两处修改之间的大段稳定文字一起框选。短中文块可用至少两个共同汉字补充配对证据，但前后不同词组中偶然出现的单个相同汉字不得抵消删除与新增。仅承担排版的行内标签和 `br` 不构成结构事实；文字与结构读取同一 semantic pair graph，视觉事实保持独立但不得越过已配对祖先边界。新增/删除结构本身不得顺带被判成视觉变化。稳定且自身未变化的大区域只作导航锚点，永不直接成为可见框。
   静态分析完成后，`SourceHostResolver` 只从 `SourceIndex`/`TargetRef` 配对直接 Canvas/SVG 或 source-empty 的稳定宿主；它不读取脚本、评论范围、computed selector 或运行时猜测节点。已有静态 footprint 的宿主不产生额外运行态候选。两张静态审阅页就绪后，可信父级向主进程提交冻结 HTML、完整 source SHA、侧别、固定视口和绑定。owner 在脚本前重验源码路径/标签/身份，随后在 isolated world 确认同一运行态宿主及可见 Canvas/SVG paint；每个宿主最多一次 rect 和 PNG。PNG 的字节、尺寸、Hash 和聚合预算重新在可信 renderer 验证。候选 key、绑定、TargetRef 和 PNG 都不进入审阅页、bootstrap、端口或窗口消息；歧义、替换、断连、预算失败或 capture 失败只使该候选静默保留静态 Diff。相同 outline 的命中复用既有 `changeId`，否则最多加入一项“视觉调整”。
...
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生同形透明孔。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文。
   初次进入审阅时，父级先等待 before/after 的静态 Review 都就绪并投影静态 footprint；随后仅并行请求一个 owner capture pair，运行态补充绝不阻塞静态审阅。owner 的总 deadline 为 1.5 秒；创建、绑定、取证、PNG 验证或撤销会话失败都只让该候选保留静态结果，截止后的结果不追加框、提示或恢复操作。两侧都捕获且 PNG Hash、尺寸或字节数不同才可加入 marker；不会启动第二个 fresh pair，也不展示动画、随机或失败原因。
