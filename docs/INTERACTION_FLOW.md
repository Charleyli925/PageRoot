...
6. 首次确实无法加载动态内容、且没有上一张可用动态画面时，画布提示“部分动态内容未加载”；用户可关闭提示，临时加载、执行或恢复失败时还可点“重新加载动态内容”，成功后提示自动消失；页面脚本本身不支持或运行环境不可用时不显示无效重试按钮。普通编辑触发的新画布重建采用“上一张可用画面 + 最新修改优先”：慢候选或失败候选只会被丢弃，不清空图表、不跳回顶部，也不阻止再次双击改字。文字输入和中文输入法尚未完成时不会切换 iframe，系统会先等最新完整 HTML 形成。
