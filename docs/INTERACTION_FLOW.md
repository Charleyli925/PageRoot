...
- 左侧栏是全局项目上下文，固定分为标题栏、品牌区、可滚动项目列表和底部固定操作区。项目列表包含“当前项目”和 Registry 中的其他已导入项目；每个项目按真实 `projectId` 独立维护展开状态，多个项目可以同时展开，展开或收起不会切换当前项目，也不会读取 HTML 正文、评论或附件。当前项目行在已有状态事实时显示“未修改”或“已修改”，该文案只来自 `userDiffersFromBase`，不把 Stable ID 物化造成的字节差异显示为用户修改；版本树摘要仍可从左侧展开并进入既有安全导航。品牌区打开 About，底部齿轮在标签栏中打开设置。
- 第二层工具栏只承担模式、审阅筛选/视图、文件动作和 AI 入口。顶栏不提供“项目”按钮、项目管理入口，也不显示保存状态点、写入中、冲突或失败文字；保存/冲突事实继续由 Document/Project 工作流、标签状态点和全局错误提示承载。更多菜单中的导出动作必须明确区分“导出 PageRoot 工作副本”和“导出干净 HTML”。刷新与更多图标保持 20px 视觉盒、30px 点击区，顺序为“刷新 → 上轮处理（有时）→ AI 助手 → 更多”。
- “在文件夹中打开”和“在默认浏览器中打开”仍是当前 HTML 的文件动作。纯浏览器预览没有本地文件权限，因此只展示禁用态。失败显示可恢复的全局错误，不通过顶栏重复呈现。
- 项目身份、Registry、PROJECT.md、Version、Candidate 和历史 HTML 仍由对应事实与工作流持有。PROJECT.md 在 AI 任务冻结时继续读取；用户可从安全工作流编辑或恢复规则，但不再有面向用户的项目管理面板、版本详情面板、Finder/导出面板或独立规则编辑界面。
...
工作副本导出、撤销/重做、版本对比和发给 AI 的基础文件始终是完整、原样的权威 HTML；
“导出干净 HTML”是明确的只读派生动作，只移除真实 HTML 属性中的 Stable ID，不改变其他源码字节；
...
新 generation 并重新授权。保存、Version、工作副本导出和 AI 输入始终读取完整源码
HTML，永不序列化 Runtime DOM；干净 HTML 导出只在导出的副本上按真实属性范围
移除 Stable ID，不改变该权威源码。
...
1. **导出 PageRoot 工作副本**：复制当前完整 Working Copy HTML，保留
   `data-pageroot-id` Stable ID，适合继续交回 PageRoot 编辑。
2. **导出干净 HTML**：复制当前完整 HTML，只移除解析器识别出的真实
   start tag 属性 `data-pageroot-id`。Script、Style、注释和普通文本中的同名
   字符串不是属性，必须原样保留。

两种动作都先读取当前已确认的完整源码，再交给系统保存对话框；取消不产生
副作用。成功或失败都不改变 Working Copy、Version、评论、Registry、当前
项目或 Recent，也不创建新版本。导出期间若当前项目正处于不可安全读取的
状态，动作按既有工作流拒绝或延后，不绕过保存、Hash/CAS、冲突和恢复边界。
...
[导出 PageRoot 工作副本] [预览外部版本] [采用磁盘版本]
```
...
基于 V4 · 前一正式版本 V7
优化指标卡片层级与标题间距
最新正式版本 · 当前编辑基础 · 已修改（仅真实用户内容/样式/结构变化）
[只读查看] [在文件夹中打开]
```
...
界面、正式 Version 与可见工作文件都使用同一连续 `Vn` ordinal；左侧版本树行必须在侧栏可用宽度内自动省略，不能要求用户横向滑动才能读完文件名。历史预览在画布中完成，Working Copy 的“未修改/已修改”只是一项由 `userDiffersFromBase` 派生的可读投影，不写入不可变 Version；技术层的 `differsFromBase` 仍可因 Stable ID 物化而为真。
...
- 源 HTML 写入失败与评论记录失败共用一个全局持久恢复提示：源文件问题优先，否则由评论问题提供“重试记录评论”。右侧评论栏和 Toast 不再重复同一错误。
...
| 当前导出（工作副本/干净 HTML） | 允许 | 禁止 | 禁止 | 按该项目状态 |
...
13. 连续两次 AI 成功后，外部原始 HTML 和隐藏快照逐字节不变；可见工作文件按各自 Version 物化 Stable ID，项目当前路径指向第二份工作文件。
...
23. V6 的历史 V2 继续编辑后，左侧版本树与历史仍保留“基于 V2”“项目最新 V6”“当前编辑基础/已修改”事实；该状态来自 `userDiffersFromBase`，顶栏不显示保存状态，历史只读不改变当前编辑目标。
...
27. 首次导入只物化 Stable ID 时，侧栏显示“未修改”；技术 `differsFromBase` 可以为真，但 `userDiffersFromBase` 必须为假。用户修改文字、样式或结构并重启后，侧栏显示“已修改”。
28. 更多菜单明确提供“导出 PageRoot 工作副本”和“导出干净 HTML”；前者保留 Stable ID，后者只移除真实 HTML 属性且保留 Script、Style、注释和普通文本字面量，导出不改变任何项目事实。
