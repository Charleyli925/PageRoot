...
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生坐标、尺寸完全相同的透明孔。遮罩以当前会话、页面侧别和 presentation epoch 唯一的 SVG luminance mask 表示：整页白底保留虚化，每个 canonical path 以黑色挖孔；重叠孔天然取集合并集，绝不能用 `evenodd` 让重叠区重新变暗。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文；作者页的 `svg path`、`mask rect` 或通用 `path/rect` 样式也不得重写受管遮罩原语。
   初次进入审阅时，父级先等待 before/after 的静态 Review 都就绪并投影静态 footprint；随后仅并行请求一个 owner capture pair，运行态补充绝不阻塞静态审阅。owner 的总 deadline 为 3 秒，并在首帧 paint 后等待 800 毫秒 settle 再取样，以免把异步初始化、仍在入场动画中的图表拍成空白或半成品。运行态判定是三态的：只有双侧都真实捕获、像素一致或在噪声预算内、且画面不是近似单色时，该宿主才作为“核实未变”参与虚化；双侧捕获并产生超出预算的像素差才加入精确宿主的 changed fact；捕获缺失、解码失败或双侧近似单色（图表根本没渲染的特征）则输出 suspected fact：该宿主在两侧都豁免虚化，修改后页绘制琥珀色虚线“疑似有改动”框，修改前页不画框；丢失捕获能力时全部运行态宿主进入疑似态而不是静默虚化。创建、绑定、取证、PNG 验证或撤销会话失败同样落入疑似态，截止后的结果不追加框、不展示失败原因；不会启动第二个 fresh pair。运行态 Map 的测量复用现有 presentation epoch：隐藏宿主没有旧框，Tab 稳定并提交当前 epoch 后才按仍连接且 fingerprint 未漂移的同一 `Element` 重测。
