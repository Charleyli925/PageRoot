...
4. 可读范围规划产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费完全相同的一组矩形；每个短语、行或文本块框都产生同形透明孔。不得用精确字词 marker 另算遮罩，也不得再通过 DOM 祖先/后代 opacity 另算一份上下文。
   初次进入审阅时，静态 footprint 与合格的运行态图表补充在同一次投影提交中生效；快照若在 iframe `load` 前完成则先缓存，待 frame 注册并完成可信端口握手后再交付。补充的 500ms 硬截止从实际 frame load 启动，截止后只使用静态结果，页面可交互后不再迟到插入新框，也不增加提示或恢复操作。
