...
- `页面预览`：`整页 / 左页 / 右页`。进入审阅默认选中“整页”，以无差异标记的修改前/AI 修改后双页完整画布展示；“左页”或“右页”只显示对应一页；必须点击明确的“整页”才能回到双页，不使用“再次点击当前单页”的隐藏手势。
- `变化审阅`：`全部变化 / 文案 / 结构 / 视觉`。选择任一变化类型都会恢复双页画布并自动聚焦该类第一处变化；某类没有结果时显示明确空态。

“全部变化”只在变化区域外围使用稀疏虚线，不叠加每一处细节填色；“文案”先按句组呈现，再精确标记修改前删除和修改后新增片段；“结构”包含新增、删除与同层区域移动；“视觉”覆盖 inline style、class、可视属性和适用 CSS 规则，并使用蓝紫虚线边界。“上下文可见度”只调节非聚焦区域，从整页视图直接拖动时先自动进入全部变化并聚焦第一处变化。

内容地图按页面区域与原页面 Tab 分组，同时列出已修改和未修改区域；定位隐藏 Tab 内的区域时先触发对应页签并揭示面板，再进行滚动。同步滚动的横向位置直接联动，纵向按相同内容区域及区内相对进度联动，不按两页原始像素高度硬对齐；仍可切换独立滚动和适应/100% 缩放。

两侧审阅画布在无同源、无表单提交、无导航、无弹窗、无下载、无宿主 IPC 的隔离沙箱中运行原页面交互，因此原页面 Tab、折叠区和纯页内控件可点击审阅，但运行态变化不会写回 HTML、候选 Version 或当前项目。“返回 AI 修改前”先提醒用户这次 AI 返回不会被采用、当前 HTML 保持为 AI 修改前版本，并说明返回本轮处理页后 AI 候选仍保留、可再次进入审阅；“接受全部并打开”也必须先确认项目将切换到完整 AI 候选，同时说明修改前版本仍保留。只有确认后的“直接打开”或“接受全部并打开”进入 9.1 的正式激活事务。
...
修改前后的完整页面；通过左上角项目标识退出审阅或确认“返回 AI 修改前”只返回本轮处理页，不清除候选。
