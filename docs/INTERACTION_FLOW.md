...
未确认的评论输入支持多条草稿。切换目标或关闭输入框时，正文、附件和 TargetRef 自动进入右栏“未完成评论”；附件上传尚未结束时，关闭评论、切换项目和关闭应用会等待归档完成。用户点击发送时，系统先收起当前输入并汇总未完成数量，只提供“查看未完成评论”这一条真实路径，直接打开第一条；不会要求用户先“处理第一条”再去点另一个全局入口。
