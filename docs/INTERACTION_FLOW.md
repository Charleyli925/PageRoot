...
安全同级重排优先原位更新。Runtime 页的文字和常见样式在源码修改、
当前 DOM 同步和目标身份校验都成功后就完成，不留下待刷新标记；Escape、
切换选中元素、等待和普通保存都不得为这类已成功的修改补跑 Candidate。
Runtime 结构操作、同级重排和作者 Script 程序身份变化立即准备 Candidate；
...
可能未完成；资源或 bootstrap 失败仍不得伪装成这一状态。
...
| 复制、删除、插入 | 立即准备最新 Candidate | 不回滚已接受的源码；动态候选失败则转入可编辑的最新静态页 |
...
等待分成 activation 的 4 秒故障上限和 activation 完成后的 12 秒关键内容上限；
用户正在编辑的时间不计入任一阶段。该检查只保护展示连续性，
