...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；正式审阅页
使用双页完整画布，支持变化筛选、完整页面内容地图、同步/独立滚动和缩放，并移除所有 Demo 标记。点击“变化”“文案”“结构”或“视觉”会自动聚焦该类第一处变化，修改前文字使用删除标记、修改后文字使用新增标记，结构和视觉变化使用各自边界；“上下文可见度”只调节非聚焦区域，从整页视图直接拖动时先自动聚焦第一处变化。某类没有结果时显示明确空态。内容地图按页面区域与原页面 Tab 分组，同时列出已修改和未修改区域。修改前、修改后版本卡可切换为单页查看，再次点击回到并排对比。

两侧审阅画布在无同源、无表单提交、无导航、无弹窗、无下载、无宿主 IPC 的隔离沙箱中运行原页面交互，因此原页面 Tab、折叠区和纯页内控件可点击审阅，但运行态变化不会写回 HTML、候选 Version 或当前项目。“返回 AI 修改前”先提醒用户这次 AI 返回不会被采用、当前 HTML 保持为 AI 修改前版本，并说明返回本轮处理页后 AI 候选仍保留、可再次进入审阅；
只有“直接打开”或“接受全部并打开”进入 9.1 的正式激活事务。
