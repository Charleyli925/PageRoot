...
默认突出“审阅对比”；连续性为 `attention` 时只显示“审阅对比”。审阅只读取冻结 HTML 与不可变候选，不切换项目当前源；正式审阅页复用正式工作台顶栏并移除所有 Demo 标记。顶栏左上角的 HTML 图标在审阅页与编辑页语义完全一致，都是“关于源页”入口；同形控件不得在两个页面里指向不同动作，因此审阅页不再提供“退回本轮处理页”入口，离开审阅只能通过“返回 AI 修改前”或“采纳 AI 修改”这两个明确决定。
