...
已核对的编辑画布时，左下角出现一张非模态「快速开始」卡片，约 340px 宽、
距边 20px，不抢焦点、不遮罩；卡片正文不拦截画布点击，只有「知道了」和关闭
可点。单击画布不关闭。文案固定为：单击选择、
双击文字直接编辑、点击“预览”查看最终效果。按钮为「知道了」和关闭。
`Esc` 关闭卡片且不清选择。

卡片按**本机安装**记住，不按项目。连续可见约 800ms 才写入 `presented`；
中途切走仍为 `pending`，下次符合条件还会出现。点「知道了」、关闭或 `Esc`
写入 `dismissed` 后不再出现。大版本可用 `guideGeneration` 重置。欢迎页、
纯浏览器预览、预览模式、历史、AI 审阅、导入确认、冲突、处理锁定和画布
尚未 `verified` 时不显示。
