...
1. 文案 footprint 是叶子级语义文本块中的精确增删范围；结构 footprint 是无法再由更小后代解释的变化节点；视觉 footprint 是实际受样式或呈现属性影响的节点。稳定的大区域只作导航锚点，永不直接成为可见框。
2. 指定筛选下，清晰区域等于该类型 footprint；其余整页内容都是上下文并按当前可见度虚化。变化父级中的未变化子树和文字范围也属于上下文，不能因祖先命中而保持清晰。
3. 可见虚线框从同一 footprint 派生。相邻或跨行但连续的 rect 用连通区域融合；相隔列、大片空白或不同区域不跨越合并。同一变化内被更小框完整解释的祖先框删除，避免嵌套与重复。
4. `全部变化`先取文案、结构、视觉 footprint 的并集，再把同一位置的多类型边界融合成一个紫色框，并用“文案、结构、视觉变化”的类型并集说明；不得把同处的多色框全部叠出。单类筛选保留对应类型色。删除文字在修改前页使用红色删除线，不额外制造大外框。
5. `navigationTarget`只决定活动标签和滚动位置。活动目标之外的其他匹配变化不得被虚化；框外若没有当前筛选变化就必须虚化，框内若只是未变化后代也必须虚化。
