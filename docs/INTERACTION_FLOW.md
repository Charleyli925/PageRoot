...
3. 文案相邻或跨行但连续的 rect 必须在同一变化、同一语义色内融合为一个连续的阶梯形框：既避免每行单独框相互重叠，也不能用包含大片空白的外接矩形跨行截断。视觉框只有 `changeId + ownerKey` 相同才可融合，邻近兄弟组件不得因间距小而连成一框；盒子级视觉变化保持完整矩形，不把内部子元素重新拼成阶梯形。相隔列、大片空白或不同区域不跨越合并。`全部变化`只在 footprint 强重叠时融合不同类型，避免把邻近但独立的变化并成大框。
4. 分析器产出的最终 footprint 是唯一投影事实。指定筛选下，框与虚化遮罩必须消费这份完全相同的边界；遮罩以该边界的 union path 为透明孔，因此只有框实际包围的内容保持清晰，阶梯转角之外的空白仍按 `contextVisibility` 虚化。不得再通过 DOM 祖先/后代 opacity 另算一份上下文。
5. 每一个最终可见框都在框边显示一个几个字的说明，不以“当前定位”为显示前提。整块新增统一写“新增内容”，不继续区分新增图表或新增表格；整块删除写“删除内容”，成对的文字改动写“文本调整”，移动、结构和视觉分别写“位置调整”“结构调整”“视觉调整”。新增文字本身继续保留作者颜色、字重和装饰。`全部变化`中的单类型框仍保留语义色，同处多类型只显示一个紫色融合框和简短类型并集说明，不叠出重复框。
