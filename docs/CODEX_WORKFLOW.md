...
The policy waits 30 seconds after the observed final completion, allowing the associated threads to arrive before classifying them. It blocks active, non-outdated P0/P1 findings because they represent user-impact risk. A `CHANGES_REQUESTED` review is blocking only when it is explicitly P0/P1, regardless of reviewer; P2, P3 and unclassified findings are retained as non-blocking debt. The P0/P1 boundary does not weaken deterministic source fidelity, IPC, dependency, security or release checks: those remain hard gates in their owning tests and policy jobs.
...
`review-debt.yml` runs only from trusted scheduled/manual default-branch workflow code. It scans current deferred review findings and updates one weekly rolling issue; it does not check out PR heads, change code or merge anything. This is the regular batch queue for P2/P3/unclassified findings.
