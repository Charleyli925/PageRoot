...
6. Update the final head onto current `main`, freeze it and mark the PR Ready once. Ready is the sole final-review trigger; never post an unmarked Draft review command or a second review command while promotion is running. A marked `github-actions[bot]` Draft-phase probe request is allowed, but `review-policy` refuses every completion while its exact-head marker is open, so the probe cannot satisfy the final gate. The probe closes its own marker once the round settles while the PR is still Draft; never mark the PR Ready while a probe round is still open, and a manual close is refused while the PR is Ready on an unsettled head. `review-policy` binds the final Codex response to the exact live head/base, requires post-Ready exact-commit evidence, waits a 30-second settle window, blocks user-impact P0/P1 findings and P0/P1 `CHANGES_REQUESTED` reviews, and writes P2/P3/unclassified findings to a machine-readable debt artifact regardless of reviewer. The branch/dependency baseline and test lanes run in parallel with that review.
