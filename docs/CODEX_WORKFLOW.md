...
The final request is a promotion attestation, not a replacement for Draft iteration. The workflow intentionally ignores a thumbs-up attached to the PR itself: GitHub does not expose which Codex invocation produced that reaction. A clean final review therefore settles when Codex reacts to this exact final request comment; a submitted Codex review or completion comment may also settle it because those records carry the reviewed commit identity. There is one stricter invocation-ambiguity fence: if any trusted review command in the PR history was edited or cannot be canonically bound to the same exact head/base pair—including a missing or mismatched marker—commit-only review/comment records cannot identify which invocation produced them and are ignored. Only a Codex thumbs-up on each new exact request is then usable; normally the operator should instead update the branch onto a changed base, producing a new head, and restart Draft review.
