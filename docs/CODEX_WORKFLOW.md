...
The final request is a promotion attestation, not a replacement for Draft iteration. The workflow intentionally ignores a thumbs-up attached to the PR itself: GitHub does not expose which Codex invocation produced that reaction. A clean final review therefore settles when Codex reacts to this exact final request comment; a submitted Codex review or completion comment may also settle it because those records carry the reviewed commit identity. There is one stricter base-drift fence: if canonical requests ever name the same head with different bases, commit-only review/comment records cannot identify which base invocation produced them and are ignored. Only a Codex thumbs-up on each new exact request is then usable; normally the operator should instead update the branch onto the new base, producing a new head, and restart Draft review.
