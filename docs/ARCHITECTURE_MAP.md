...
| Review and Candidate | Repository owns immutable Candidate HTML, runtime seal and source-identity report; `VersionSession` owns only the renderer projection | Repository validates/normalizes full-HTML Candidate; `VersionWorkflow` prepares Review and accepts | `bridge/project-file-repository/candidate-identity.mjs`, `app/workbench/AiReviewWorkspace.tsx` |
