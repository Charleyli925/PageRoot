...
| Semantic source editing foundation | immutable semantic document state and lineage | pure `SemanticOperationKernel`; SourcePatch is its internal materializer | `app/lib/semantic-operation-kernel.js`, `app/lib/source-patch-engine.js`, `schemas/semantic-operation.v1.schema.json` |
