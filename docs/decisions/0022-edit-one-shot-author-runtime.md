...
with non-zero geometry. Other classic-script pages retain static Edit.
...
- The bootstrap runs parser-first, records mutations and author async/event
  resources, blocks network/navigation/worker/form/media APIs, then removes
  timers, listeners and observers after two frames. It rejects source identity,
  text or geometry changes outside approved hosts and disables interaction on
  runtime nodes.
...
- React/Vue root reconstruction, module/dynamic-import/worker/network pages,
  layout-changing pages and pages with continuing runtime work fail closed to
  static Edit.
