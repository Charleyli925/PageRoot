...
5. Ensure the Draft review has a completion signal, not a Codex environment/setup error, and no other PR is being promoted; then mark this reviewed head Ready once. `review-settled` proves both the pre-Ready Draft completion and the Ready-triggered final Codex completion, then waits a 180-second settle window. Only after that may `baseline-policy` run the unchanged advisory threshold and packaged-runtime closure check.
