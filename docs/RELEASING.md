...
5. Ensure the Draft review has a completion signal, not a Codex environment/setup error, and no other PR is being promoted; then mark this reviewed head Ready once. After GitHub records Ready, wait at least one second and post the canonical final exact-SHA request from `docs/CODEX_WORKFLOW.md`. `review-settled` proves the pre-Ready Draft completion plus the post-Ready final request and its comment-bound or commit-bound Codex completion, then waits a 180-second settle window. PR-level reactions are not evidence. Only after that may `baseline-policy` run the unchanged advisory threshold and packaged-runtime closure check.
