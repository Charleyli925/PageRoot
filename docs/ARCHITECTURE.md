...
| Workbench visual cascade | `app/globals.css` is import-only; `app/styles/*.css` load in fixed order (tokens/base, browser shell, legacy shell, review V5/V5.1/V5.2 canvas, comment hierarchy, project resources, about/chrome, top toolbar, final browser ownership). Later layers override earlier ones; the two `:root` blocks stay split |
