...
| Canvas one-shot runtime frame identity, frozen-result verification and author-paint retention | `app/components/html-canvas-frame.ts`; `HtmlCanvasEditor` still owns iframe mount/reload |
| Native deferred-command arbitration (user-explicit vs system, lease matching, stale drain) | `app/components/html-canvas-native-commands.ts`; the editor supplies the live session/lease and still retires the queue before host replacement |
...
`html-canvas-frame.ts`. Native deferred-command arbitration lives in
`html-canvas-native-commands.ts`. Comment-target geometry lives in
