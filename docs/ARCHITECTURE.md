...
| Current source bytes, Hash, revisions, persistence projection, source-write single flight, verified recovery/export protection evidence and Canvas authority generation | `app/application/document-session.js` and `app/application/document-workflow.js`; failed/conflict remains visible even when exact protection permits reversible detach |
