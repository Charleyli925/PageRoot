...
| External OS/QoderWork HTML-open delivery, opaque request deduplication, whole project-open transition ordering, deferred safe-switch retry and stale-result fencing | `desktop/external-file-open.mjs`, `desktop/project-open-queue.mjs`, `app/application/external-file-open-session.js` |
