...
| Trusted-local Qoder installation discovery, read-only diagnosis, package/version/login/model preflight, error classification and ACP launch descriptor | `bridge/agent/providers/qoder-provider.mjs`; diagnosis uses only version/model-list commands and does not open ACP. Historical Request `qoder-acp` is decoded only by the Agent Delivery codec; public session `driver` remains a compatibility projection |
>>>>>>> origin/main
