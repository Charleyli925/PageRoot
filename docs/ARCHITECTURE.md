...
- Production Edit remains script-disabled. Authored inline SVG stays
  source-backed and arbitrary runtime-only Canvas/SVG stays in Preview. ADR 0020
  permits one bounded declarative exception: PageRoot validates an explicitly
  authored fixed-size JSON Chart Spec, synchronously renders a disposable SVG
  and mounts it once inside the original source-empty host's Shadow DOM before
  that Canvas generation becomes ready. A chart-bearing frame waits only until
  HTML parsing reaches `interactive`, not for external resources. Initial
  hidden Tabs are prepared in the same pass and later reuse the same nodes. The
  iframe owns their lifetime; no
  runtime snapshot session, cache, IPC request, observer, bitmap projection,
  Blob URL or chart-specific generation is created.
...
| Edit Chart Spec grammar, fixed ECharts mapping, validated SVG SSR and one-shot same-slot projection | `app/domain/edit-chart-spec.js`, `app/lib/edit-chart-svg.js`, `app/lib/edit-chart-projection.js`, composed by `HtmlCanvasEditor` inside the existing Canvas generation |
