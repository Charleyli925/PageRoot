...
| Frozen execution/discussion filesystem policy and permission-separated Host Ports | `scripts/agent/policies/execution-policy.mjs`, `scripts/agent/policies/discussion-policy.mjs`, `scripts/agent/hosts/execution-host.mjs` and `scripts/agent/hosts/discussion-host.mjs`; the legacy client adapts generic errors to its established name/code/copy without owning a second policy brand |
