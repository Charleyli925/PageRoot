...
审阅显示以单一互斥状态保存，界面分为两组同样式控件：`页面预览（双页 / 左页·修改前 / 右页·修改后）` 与 `变化审阅（全部变化 / 文案 / 结构 / 视觉）`。默认“双页”为无差异标记的修改前/AI 修改后完整画布；左右页只显示对应单页，明确点击“双页”恢复双页。任一变化模式必须恢复双页并聚焦第一处匹配变化；无匹配时显示空态。导航区域与可见变化 marker 必须分离；全部变化只融合相邻的小粒度 marker；文案按标点短句做字符级新增/删除标记且无外层大框；结构标出具体新增、删除、移动或属性节点；视觉把可视属性、inline style 与适用 CSS 规则映射到具体元素，并提供可读变化说明。

正式审阅页还必须提供包含已修改与未修改区域、并按原页面 Tab 分组的完整内容地图；定位隐藏面板前先在两侧安全触发对应页签。同步模式下，一侧安全 Tab 操作镜像到另一侧，普通链接、表单和业务按钮不镜像。非聚焦上下文可见度按 0–100 线性映射到 opacity（0 完全不可见，100 完全无遮挡）；适应/100% 缩放、同步/独立滚动保持可用。同步横向滚动联动位置，纵向滚动按内容区域 ID 与区内进度对齐并显式钳制顶/底边界；程序化跟随事件不得回传形成反向跳动。原页面 Tab、折叠区和纯页内控件可在无同源、无导航、无提交、无弹窗、无下载、无宿主 IPC 的隔离沙箱中交互，运行态不得写回 HTML、Version 或项目状态。

审阅页点击“返回 AI 修改前”必须先逐行展示确认提醒：不会采用本次 AI 返回；继续以修改前版本为基线重新修改；AI HTML 仍保留且整句可打开本轮文件夹。确认后把该 active run 以 `declined-ai-candidate-after-review` 结束并令 runtime 回到 `editing`，直接恢复原 HTML 编辑状态；评论、编辑记录、候选 Version、working HTML 与本轮审计记录均不删除。“继续审阅”为紫色建议操作，“返回修改前版本”为灰色操作。通过左上角项目标识退出审阅仍只返回本轮处理页，不结束待打开状态。

“打开 AI 修改后”必须先确认项目将切换到完整 AI 候选，同时说明修改前版本与本轮记录仍保留；最终按钮为“确认并打开”。确认后处理抽屉保持关闭，先恢复编辑画布挂载，再于下一动画帧进入激活事务，禁止闪现等待 AI 页面。用户点击“直接打开”或在审阅页确认“打开 AI 修改后”后：
