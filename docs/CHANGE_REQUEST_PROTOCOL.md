...
导航区域与可见变化 marker 必须分离。分析器为文案精确范围、最小结构节点和实际视觉节点生成唯一 canonical footprint；虚化是该 footprint 在整页范围内的补集，框选由同一 footprint 投影。相邻连续 rect 以连通区域融合，跨列或大片空白不合并，能被更小后代完整解释的祖先框删除。`全部变化`对三类 footprint 求并集，同一位置只显示一个紫色融合框和类型并集说明；单类筛选只显示该类。新增文字保留作者原有颜色、字重和装饰；删除文字继续使用红色删除线。变化父级中的未变化后代也必须虚化，导航焦点不得把其他匹配变化变成上下文。
