...
`ready-to-open` 的正式处理页必须同时提供“审阅对比”和“直接打开”，其中“审阅对比”为默认强调操作。“审阅对比”读取冻结基础 HTML 与不可变候选 Version，不调用激活事务；正式审阅页不得带 Demo 标记，并提供双页完整画布、按文案/结构/视觉自动聚焦的变化筛选、包含已修改与未修改区域并按原页面 Tab 分组的完整内容地图、非聚焦上下文可见度、修改前/修改后单页切换、缩放与同步/独立滚动。原页面 Tab、折叠区和纯页内控件可在无同源、无导航、无提交、无弹窗、无下载、无宿主 IPC 的隔离沙箱中交互，运行态不得写回 HTML、Version 或项目状态。

审阅页点击“返回 AI 修改前”必须先展示确认提醒，明确这次 AI 返回不会被采用、当前 HTML 保持为 AI 修改前版本、确认后返回本轮处理页、候选仍会保留且可再次进入审阅。通过左上角项目标识退出审阅同样返回本轮处理页，不释放候选。用户点击“直接打开”或在审阅页点击“接受全部并打开”后：
