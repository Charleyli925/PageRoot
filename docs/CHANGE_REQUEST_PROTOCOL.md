...
审阅显示以单一互斥状态保存，界面分为两组同样式控件：`页面预览（双页 / 左页·修改前 / 右页·修改后）` 与 `变化审阅（全部变化 / 文案 / 结构 / 视觉）`。默认状态为“双页 + 全部变化”，同步滚动开启，上下文可见度为 18%，并聚焦第一处变化；左右页只显示对应单页，明确点击“双页”恢复双页。任一变化模式必须恢复双页并聚焦第一处匹配变化；无匹配时显示空态。导航区域与可见变化 marker 必须分离。新增文字保留作者原有颜色、字重和装饰，只用一个可跨多行合并的紫色虚线框标示；删除文字继续使用红色删除线，不增加外框。结构与视觉 marker 只落在具体变化节点，过滤器还必须在变化区域内部把该类型未变化的子树按当前上下文可见度虚化，不能因父级有变化就整块保持清晰。内容地图和画布提示只显示“新增内容”“删除内容”或“文案/结构/视觉变化”等简短说明，不展开属性清单。

正式审阅页还必须提供包含已修改与未修改区域、并按原页面 Tab 分组的完整内容地图；变化项以克制的紫色边线和底色显示，未变化项降低对比度。地图面板从把手右侧展开，右边缘贴住画布，并在用户点击地图以外的页内画布、顶部栏或 App 区域时自动收起。点击地图项或上一处/下一处时，定位隐藏面板前先在两侧安全触发对应页签。同步模式下，一侧的安全页内动作都镜像到另一侧，包括 Tab、折叠、业务按钮以及 input/select/textarea 的值与选中态；导航、提交、弹窗、下载和宿主 IPC 仍由沙箱阻止。非聚焦上下文可见度按 0–100 线性映射到 opacity（0 完全不可见，100 完全无遮挡）；适应/100% 缩放、同步/独立滚动保持可用。同步横向滚动联动位置，纵向滚动按内容区域 ID 与区内进度对齐并显式钳制顶/底边界；程序化跟随事件不得回传形成反向跳动。运行态不得写回 HTML、Version 或项目状态。
