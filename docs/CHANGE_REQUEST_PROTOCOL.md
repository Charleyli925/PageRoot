...
桌面正式审阅的运行态图表补充仍以静态 footprint 为主，并只接纳源码为空、身份唯一、两侧可配对且未被静态 marker 覆盖的宿主。无评论宿主必须由本轮变化脚本直接引用稳定身份；宿主本身或祖先若是冻结的非全局评论目标，则该局部用户意图可让它优先进入同一有界前后视觉指纹比对，即使配色、数据或配置变化位于未直接引用宿主的独立脚本。全局评论不授权整页候选，普通同 section 共址也不构成因果证据；候选超出 128 个时优先保留评论范围，后续稳定性、身份、预算、截止与安全校验完全不放宽。
