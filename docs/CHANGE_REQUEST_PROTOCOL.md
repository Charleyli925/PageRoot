...
审阅显示以单一互斥状态保存，界面分为两组同样式控件：`页面预览（整页 / 左页 / 右页）` 与 `变化审阅（全部变化 / 文案 / 结构 / 视觉）`。默认“整页”为无差异标记的修改前/AI 修改后双页完整画布；左右页只显示对应单页，明确点击“整页”恢复双页。任一变化模式必须恢复双页并聚焦第一处匹配变化；无匹配时显示空态。全部变化只显示稀疏外围边界；文案支持句组和精确新增/删除标记；结构包含纯移动；视觉覆盖可视属性、inline style 与适用 CSS 规则。

正式审阅页还必须提供包含已修改与未修改区域、并按原页面 Tab 分组的完整内容地图；定位隐藏面板前先安全触发对应页签。非聚焦上下文可见度、适应/100% 缩放、同步/独立滚动保持可用；同步横向滚动联动位置，纵向滚动按内容区域 ID 与区内进度对齐。原页面 Tab、折叠区和纯页内控件可在无同源、无导航、无提交、无弹窗、无下载、无宿主 IPC 的隔离沙箱中交互，运行态不得写回 HTML、Version 或项目状态。

审阅页点击“返回 AI 修改前”必须先展示确认提醒，明确这次 AI 返回不会被采用、当前 HTML 保持为 AI 修改前版本、确认后返回本轮处理页、候选仍会保留且可再次进入审阅。“接受全部并打开”也必须先确认项目将切换到完整 AI 候选，同时说明修改前版本仍保留。通过左上角项目标识退出审阅同样返回本轮处理页，不释放候选。用户点击“直接打开”或在审阅页确认“接受全部并打开”后：
...
- active run；其中 `ready-to-open` 必须保留候选 Version 与 transaction 身份，直到用户确认打开、显式取消或进入可审计错误。仅退出审阅或“返回 AI 修改前”不得释放候选。
