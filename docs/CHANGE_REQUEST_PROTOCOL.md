...
可见范围框再由真实布局统一规划：默认短语矩形；同一实际文字行至少三个变化短语，或首尾证据横跨该行可见文字宽度 60% 时提升为完整行矩形；同一 owner 至少两行且至少 75% 的行已提升时，再提升为最小段落矩形；一段连续正文的两行不应各自拿一个平行框。短语矩形在紧行距下会伸进相邻行，两个交叉的框围着同一句话就是噪声；因此相邻两行的短语矩形（计入渲染时的统一内边距）一旦重叠，这两行统一提升为它们各自的完整行矩形。碰撞只是局部排版意外，不算“该行被改写”的语义信号，因此不计入段落矩形的语义提升比例；但当该 owner 的每一行都带证据且全部已提升时，同样统一为一个最小段落矩形——跨行插入若每一个渲染文字行都被改动，就读作一个干净矩形而不是一排平行行框；“每一行都带证据”是硬条件，它保证段落矩形永不跨过一整行未变文字（例如未动的开场或收尾行）。行矩形仍然存在于部分行未带证据的 owner；同一 owner 的相邻行矩形可以共用一条边：每个行框必须向下伸到足以包住自己的绿点，强行要求行框之间留空会把字符证据挤到框外。收并后的单一矩形仍必须完整包含该 marker 的全部字符证据。范围框两侧统一为低强调紫色细实线，结构与视觉框为同样克制的细虚线，边线不得粗到与正文争夺注意力；每个框必须是干净的轴对齐长方形并包含全部字符证据；不得使用密度/稳定句的第二套判断，不得把字词 rect 连成锯齿多边形或提升到外层卡片。结构事实来自未配对/移动/非视觉结构属性，视觉事实只来自已配对既有节点的呈现属性或实际命中样式规则；新增结构不得自动派生视觉变化。最终 canonical footprint 同时供框和整页遮罩消费，遮罩直接以坐标和尺寸完全相同的最终矩形为透明孔，因此框内完整清晰、框外按可见度虚化。每个语义变化组只显示一个标签；文字范围不与结构或视觉事实融合为 shaped 框。非文案 footprint 先做一次嵌套收并：同一 `changeId` 内，只有两种情形才收并到外层：外层是新增或删除的结构事实（整个子树本就是新的或没了，内层框只是重复外框已经说过的话），或者内外两者都是结构事实（一层套一层的结构框不再提供定位信息而只是视觉噪声）；独立的视觉事实永远保留自己的框，盒子级视觉 owner 不得吞掉后代自己的呈现变化。包含关系必须 DOM 与几何两者同时成立，因此相邻的独立事实永不被融成一个框或一个透明孔，被收并的事实数量由标签的「×N」承载。嵌套收并之后，其他类型仍只融合强重叠且同事实、同 owner 的 footprint。导航焦点不得改变变化集合或遮罩。

桌面正式审阅的运行态图表补充仍以静态 footprint 为主，并只接纳源码为空、身份唯一、两侧可配对且未被静态 marker 覆盖的宿主。无评论宿主必须由本轮变化脚本直接引用稳定身份；宿主本身或祖先若是冻结的非全局评论目标，则该局部用户意图可让它优先进入同一有界前后视觉指纹比对，即使配色、数据或配置变化位于未直接引用宿主的独立脚本。分析器还可在当前高置信度审阅 section 内，从该评论目标向上选择首个至少包含两个可配对空宿主的祖先，并把其中其他宿主作为最近图表组的相邻候选；不得跨 section、按纯视觉距离扩张或解析评论正文。全局评论不授权整页候选，组外同 section 共址也不构成因果证据；候选超出 128 个时依次优先保留直接评论目标、评论目标内部和最近组相邻图表，后续稳定性、身份、预算、截止与安全校验完全不放宽。

正式审阅页还必须提供包含已修改与未修改区域、并按原页面 Tab 分组的完整内容地图；变化项以克制的紫色边线和底色显示，未变化项降低对比度。地图面板从把手右侧展开，右边缘贴住画布，并在用户点击地图以外的页内画布、顶部栏或 App 区域时自动收起。修改前/后文档的 panel 与 action key 必须成对建立稳定映射；AI 改动控件文案或顺序时，优先依据显式目标、同 panel 控件类型与语义位置匹配。任一侧的安全页内动作始终镜像到另一侧，包括 Tab、折叠、业务按钮以及 input/select/textarea 的值与选中态；该合同不受同步/独立滚动控制，匹配失败时静默保持当前侧，不显示额外提示。导航、提交、弹窗、下载和宿主 IPC 仍由沙箱阻止。同步滚动联动横向位置；纵向以当前直接输入侧为唯一主控，按布局稳定点缓存的共同内容区域及区内进度建立连续单调映射，跟随侧每帧只应用最新目标。换侧必须让旧代次失效；短页边界只钳制短页，不得强拉长页到顶/底。滚动事件内不得扫描锚点、重建框选或重测评论，布局映射在手势空闲后更新。独立滚动只关闭滚动跟随。页内动作和布局变化后必须自动刷新框选与虚化，内容地图选择不是刷新前置条件。运行态不得写回 HTML、Version 或项目状态。
