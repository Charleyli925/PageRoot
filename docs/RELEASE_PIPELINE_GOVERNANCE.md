...
| Review debt | Trusted weekly default-branch scheduled/manual run | Rolling P2/P3/unclassified review issue and machine-readable snapshot | Check out PR code, modify a PR, merge or turn debt into a required check |
...
- `review-debt.yml` runs from scheduled/manual trusted default-branch code with `contents: read`, `pull-requests: read` and `issues: write`. It only refreshes one rolling issue from active non-blocking findings; it has no `pull_request` trigger, no PR-head checkout and no merge permission.
