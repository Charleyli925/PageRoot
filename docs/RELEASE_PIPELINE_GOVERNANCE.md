...
| Review policy | A frozen Pull Request transitions to Ready once | Live head/base equality, exact-commit Codex review/clean-comment completion submitted after the frozen base commit (Draft-phase included, post-Ready fallback without base date), post-Ready root `+1`, 30-second settle window, active P0/P1 and P0/P1 `CHANGES_REQUESTED` blocking, P2/P3/unclassified debt artifact | Reuse a review written before the frozen base existed or a pre-Ready root reaction, make review priority weaken deterministic safety checks or block a candidate solely for P2/P3 debt |
...
- `review-policy` is a final-candidate barrier that runs in parallel with deterministic work. It revalidates live head/base and the latest Ready event, accepts an exact-commit Codex review with `Reviewed commit` evidence or an unedited exact-commit clean Codex comment submitted after the frozen base commit existed (including while Draft; without the base commit date it falls back to post-Ready), then waits 30 seconds. Root `+1` reactions carry no commit identity and must still follow the latest Ready transition. It blocks active non-outdated P0/P1 threads and P0/P1 `CHANGES_REQUESTED` reviews; GitHub review history is reduced to each reviewer's latest explicit decision on the final head, so only a later approval or dismissal retracts an earlier request. P2/P3/unclassified findings are written to non-blocking debt regardless of reviewer. `release-gate` repeats the evidence check in immediate revalidation mode after all source lanes and before exact-tree attestation.
...
- One macOS `electron-renderer` job builds the Electron renderer once per candidate run on the target OS and uploads a run-ID-stable artifact. Linux-to-macOS build output therefore never becomes a variable, and both Electron lanes test the exact same renderer bytes.
- Native Electron and deterministic AI run as separate jobs that download the shared renderer artifact. A failure can be rerun independently.
- The native lane first runs a product-independent synthetic Electron environment preflight. It proves that the hosted window is visible and that renderer timers and animation frames advance before PageRoot code or assertions begin; the AI lane reuses those same renderer bytes without repeating the preflight.
- Browser shards and real HTML keep retries at zero. The native Electron lane retries a test once in CI only, to absorb a transient Electron launch/hydration stall; local runs stay retry-free and every retried failure retains trace, video and screenshot evidence. Reliability remains anchored in deterministic readiness and better evidence, not blanket retrying.
