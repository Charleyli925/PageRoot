...
- `review-settled` is the first promotion barrier. It revalidates the live head and base against the Ready event plus the latest Ready timeline event, requires a trusted exact-head/base request and completion made in Draft, then waits for the Ready-triggered substantive exact-commit review or clean PR reaction. Empty/unmarked reviews and `EYES` are ignored. It holds three minutes from the real completion and rejects active non-outdated P0-P2 threads. `release-gate` repeats that live evidence check after all source lanes and before exact-tree attestation.
