...
- 内容地图导航区域与 canonical change footprint 分离。分析器先建立高置信度节点配对，再从叶子文案增删、未配对/移动结构和已配对节点的呈现差异生成事实，禁止用同标签/位置兜底制造变化。框选和全页遮罩共同读取融合后的同一组矩形，框内完整清晰、框外虚化；新增文案绿框、删除文案红框、结构蓝框、视觉紫框。全部变化只融合强重叠的多类型框，连续 rect 不跨列或大片空白，祖先重复框删除；内容地图包含未修改区域并按页面 Tab 分组；
