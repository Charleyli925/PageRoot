...
- 内容地图导航区域与 canonical change footprint 分离。分析器先建立高置信度节点配对，再按混合行内节点的语义文本流生成精确文案增删证据，纯增删在另一页保留对应上下文框；可见文案 footprint 另按短语、行和最小语义文本块归组，密集改写提升而稳定句拆分，不把细碎 rect 描成锯齿轮廓。仅排版的行内标签与 `br` 不算结构事实。其余结构事实只来自未配对/移动结构，视觉事实只来自已配对节点的呈现差异，禁止用同标签/位置兜底制造变化。静态 footprint 未覆盖的空图表宿主仍走有界运行态指纹补充；冻结的非全局评论目标范围优先进入候选，允许识别与宿主绑定脚本分离的配色、数据或配置变化，未评论宿主仍要求变化脚本直接引用，且全局评论不放宽整页范围。框选和全页遮罩共同读取最终同一组矩形，框内完整清晰、框外虚化；新增文案保留绿色虚线框且无下划线，删除文案保留红色虚线框和红色虚线删除线，结构蓝框、视觉紫框。每个语义变化组只显示一个标签；全部变化只融合强重叠的多类型框，连续 rect 不跨列或大片空白，祖先重复框删除；内容地图包含未修改区域并按页面 Tab 分组；
