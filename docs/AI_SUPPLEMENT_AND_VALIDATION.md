...
- “审阅对比”只读取冻结 HTML 与不可变 AI 候选，不激活候选；正式审阅页使用一个互斥七状态模型：`双页 / 左页·修改前 / 右页·修改后` 页面预览和 `全部变化 / 文案 / 结构 / 视觉` 变化审阅。默认“双页”是无标记双页画布，左右页只显示一侧，明确点击“双页”恢复双页；任一变化模式会恢复双页并自动聚焦匹配变化；
- 正式审阅页把内容地图导航区域与可见 marker 分离：全部变化融合相邻的小粒度 marker，文案按有意义短句做字符级新增/删除且不画外层大框，结构/视觉落到具体节点并提供可读说明；完整内容地图包含未修改区域并按页面 Tab 分组；
- 内容地图、上一处/下一处和同步模式中的安全 Tab 点击会让两侧一起切换隐藏面板，普通链接或业务操作不镜像；非聚焦上下文可见度按 0–100 线性映射，0 完全不可见、100 完全无遮挡；适应/100% 缩放和同步/独立滚动保持可用，同步纵向滚动按相同内容区域与区内进度对齐并阻止跟随端反向回传；
