...
- 内容地图导航区域与 canonical change footprint 分离。分析器先建立高置信度节点配对，再按混合行内节点的语义文本流生成精确文案增删证据，纯增删在另一页保留对应上下文框；可见文案 footprint 另按短语、行和最小语义文本块归组，密集改写提升而稳定句拆分，不把细碎 rect 描成锯齿轮廓。仅排版的行内标签与 `br` 不算结构事实。其余结构事实只来自未配对/移动结构，视觉事实只来自已配对节点的呈现差异，禁止用同标签/位置兜底制造变化。静态 footprint 未覆盖的受支持源码宿主可走有界 Runtime Snapshot 补充：SourceHostResolver 仅从 SourceIndex/TargetRef 配对直接 Canvas/SVG 或 source-empty 稳定宿主，不分析脚本因果、computed selector、评论范围或运行时猜测节点。静态页就绪后，可信父级向唯一 owner 提交冻结 HTML、完整 source SHA、侧别、视口和绑定；owner 先重验源码绑定，再在 isolated world 确认同一宿主及可见 Canvas/SVG paint，并至多拍摄一张 PNG。单个 before/after pair 的 PNG Hash、尺寸或字节差异可补一个已有 outline 的视觉 marker；任何 unavailable、超时、迟到、预算或验证失败都静默保留静态结果，没有第二轮 fresh-pair confirmation 或用户状态。评论定位是独立私有能力：可信父级保留不含正文的定位描述，并只在 before frame 加载后经独立、受随机 challenge 绑定的私有端口交给其 bootstrap，绝不写入两页 HTML 或可抓取的 bootstrap 源码。定位描述来自冻结 TargetRef 的私有 sourceNodeId binding，无法安全定位或私有端口不可用时不显示该评论标记。评论定位不会参与 Runtime Snapshot 候选发现，因此作者 CSS/脚本既不能读取评论描述，也不能用它制造视觉差异。框选和全页遮罩共同读取最终同一组矩形，框内完整清晰、框外虚化；新增文案保留绿色虚线框且无下划线，删除文案保留红色虚线框和红色虚线删除线，结构蓝框、视觉紫框。每个语义变化组只显示一个标签；全部变化只融合强重叠的多类型框，连续 rect 不跨列或大片空白，祖先重复框删除；内容地图包含未修改区域并按页面 Tab 分组；
...
- 运行态 Snapshot 身份补充：候选 key、源码宿主 TargetRef、路径和稳定身份属性只保存在可信父级与主进程 owner 的窄请求/isolated-world 闭包中；HTML、review bootstrap、作者可枚举 DOM、窗口消息、端口和截图均不暴露这些值。owner 先用原始冻结源码验证路径、标签和身份属性，作者脚本运行后只接受同一路径/标签/身份与可见 Canvas/SVG paint 的结果；缺失、歧义、替换或断连时仅该候选静默保留静态审阅。可信 renderer 验证 PNG bytes、尺寸、SHA-256 和预算后，只比较一次 before/after pair；没有确认 session、状态 UI 或 Retry。
