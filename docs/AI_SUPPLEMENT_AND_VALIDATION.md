...
- 运行态候选身份补充：候选 host 的 key 与原始 source-box 基线同样只经首个 bootstrap 响应中的私有路径/指纹/基线绑定交给受管 bootstrap；HTML、后续 bootstrap 读取以及作者可枚举 DOM 均没有临时身份、runtime-host/source-box 属性、key 或定位路径。该响应被使用一次后立即由无绑定回退源码替换，运行态取证随后只读闭包映射；路径只能回退到唯一私有指纹，缺失、歧义、替换或断连时整批运行态补充回退为静态审阅。确认帧对会话重新创建，因此各自的首个 bootstrap 请求安全获得新的单次绑定。
