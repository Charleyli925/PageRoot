...
- 原始评论和本地编辑继续锁定并保留；
- 重启应用后仍恢复“可打开”状态；
- 本轮处理页固定显示“审阅对比”和“直接打开”两个入口，并默认突出“审阅对比”；
- “审阅对比”只读取冻结 HTML 与不可变 AI 候选，不激活候选；正式审阅页提供双页完整画布、变化筛选、内容地图和同步/独立滚动；
- 审阅页不显示 Demo 标记；“保留当前版本”必须先明确提示会返回本轮处理页，而且 AI 候选仍会保留、可以再次进入审阅；
- 只有用户点击“直接打开”或在审阅页点击“接受全部并打开”，项目当前路径和左侧画布才切换；
