...
- 内容地图导航区域与 canonical change footprint 分离。文案使用叶子级精确范围，结构/视觉落到最小具体节点；框选和全页虚化共同读取该 footprint。全部变化把同一位置的多类型框融合成一个，连续 rect 合并但不跨列或大片空白，祖先重复框删除；内容地图包含未修改区域并按页面 Tab 分组；
