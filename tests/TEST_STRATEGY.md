...
- `WorkbenchTabsSession/Workflow`：Node 直接证明 `projectId + documentId` 去重、多开始页、
  24 标签上限、状态投影、关闭不删业务权威、close/activate 竞态 fail-closed，以及 Start 和文档
  ...
  Home/End 的 roving focus、Start 冷重启抑制 activePath、Registry 标题恢复及 unmounted outlet 安全关闭。
- 外部打开 FIFO：Main mailbox Node 测试证明队首在 renderer 显式 ack 前不消费、不发送后继；renderer
  ...
  的回执顺序。preload 合同只暴露 opaque requestId 的 accept/ack，不暴露路径权威。
