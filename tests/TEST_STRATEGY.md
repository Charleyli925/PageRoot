...
- 交互预览：Node 分别证明短期自定义协议的资源边界和
  ...
  编辑后只保留当前 Tab 且仍能进入原有文字编辑岛。桌面编辑画布还必须
...
- AI 审阅：Node 用重复文字、重排行列和上下文相似度证明确定性节点配对；
  Browser 用实际差异 DOM 和几何作为独立 oracle，证明内容地图跳过零证据
  区域、差异框在异步尺寸变化后重新包围全部子证据，以及修改前历史评论的
  `评` 标记、只读气泡和外部点击收起不参与差异计数。
