...
- `WorkbenchTabsSession/Workflow`：Node 直接证明 `projectId + documentId` 去重、多开始页、
  状态投影、关闭不删业务权威，以及文档激活严格按 `ProjectWorkflow.prepareSwitch`
  的 drain/fence → Registry 打开 → Controller 身份确认 → active commit 顺序。持久化测试拒绝
  path/HTML/Hash 和未知字段，验证原子替换与无效文件 fail-closed；Electron 证明只有
  一个业务文档 DOM 挂载，快捷键和重启恢复不绕过现有 drain。
