...
- 完整 HTML 工作流性能诊断：`npm run benchmark:html-workflows -- --app
  ...
  Canvas/SVG Runtime 绘制仍为零变化。段落两字或五处修改均为一个阅读块框和一个标签；两个段落是两个 focus group，切换时只显示一个。同一 CSS 规则命中的六张卡在满足 75% 与 grid/flex/list/repeated-card 条件时提升为一个容器框，单卡仍框自身。50%–200% 缩放、resize 与字体变化后，前后页保持同一 `activeFocusGroupId`，框与遮罩孔保持同一 canonical `pathData`，
