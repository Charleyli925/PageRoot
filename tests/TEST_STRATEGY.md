...
`PAGEROOT_REAL_HTML_DIR` 指向该语料目录后，运行 `npm run test:real-html:electron`；缺少目录或空目录直接失败，完整结果与截图写入系统临时目录，不进入 Git。该入口对每个文件至少选取页面上、中、下部且尽量不同标签的多个真实编辑宿主，使用鼠标/键盘输入执行段首、段尾、连续空格、删除/撤销重做、连续加粗/斜体/下划线、元素复制与删除。同时覆盖文档内可见页签、预览/编辑切换、完整来源重载、保存后重开及变更后的持续可编辑性。文档特有图表等行为仍需按本次改动追加场景。
