...
test('bounds the existing Inspector owner and preserves Network events', async () => {
  calls = []; const f = fixture(); const result = await boundFrozenInspectorCache(f.page, '1.63.0');
...
test('new owners are bounded before their first enable, while retired owners can leave', async () => {
  ...
  const result = await boundFrozenInspectorCache(f.page, '1.63.0');
...
test('failed new-owner initialization cannot be silently accepted', async () => {
  const f = fixture(async () => {}), result = await boundFrozenInspectorCache(f.page, '1.63.0');
...
test('unknown versions, missing owners, empty sessions and failed commands cannot pass', async () => {
  ...
  await assert.rejects(boundFrozenInspectorCache({}, '1.63.0'), /ADAPTER_UNAVAILABLE/);
  const empty = fixture(); empty.sessions.clear();
  await assert.rejects(boundFrozenInspectorCache(empty.page, '1.63.0'), /EMPTY_SESSIONS/);
  await assert.rejects(boundFrozenInspectorCache(fixture(async () => { throw new Error('CDP_REJECTED'); }).page, '1.63.0'), /CDP_REJECTED/);
});
