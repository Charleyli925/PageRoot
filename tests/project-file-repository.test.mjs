...
test("save publishes only into an unoccupied source path after staging the verified bytes", async (t) => {
  const value = await fixture(t);
  const imported = await importSource(value, "save-atomic-boundary.html");
  const externalHtml = html("external edit at save publication boundary");
  const repository = new ProjectFileRepository({
    projectsRoot: value.projects,
    failpoint: async (name) => {
      if (name === "save-source-staged") {
        await writeFile(imported.target.exactSourcePath, externalHtml, "utf8");
      }
      return false;
    },
  ...
  await assert.rejects(
    repository.saveWorkingCopy({
      target: imported.target,
      html: html("PageRoot save that must not replace a late external write"),
      expectedSourceSha256: imported.target.sourceSha256,
      editRevision: 1,
    }),
    (error) => error instanceof ProjectFileRepositoryError
...
test("save restores an external write through an already-open Working Copy handle", async (t) => {
  const value = await fixture(t);
  const imported = await importSource(value, "save-open-handle.html");
  const externalHtml = html("external write through retained handle");
  ...
  await assert.rejects(
    repository.saveWorkingCopy({
      target: imported.target,
      html: html("PageRoot save that yields to the retained external handle"),
      expectedSourceSha256: imported.target.sourceSha256,
      editRevision: 1,
    }),
    (error) => error instanceof ProjectFileRepositoryError
...
test("save fault injection recovers a complete durable state or a retained old state", async (t) => {
  for (const failpoint of [
    ...
    const expectedHtml = ["save-prepared", "save-source-staged"].includes(failpoint)
      ? html("V1")
      : nextHtml;
